...
var nn_audio_cfg = require('nn_audio_cfg_jlb');
...
cc.Class({
    ...
    onLoad() {
        this.setClubAudioCfg();
        const baseScore = [0, 10, 20, 50, 100];
        nn_data.Instance().m_nBaseScore = baseScore[RoomMgr.Instance()._Rule.baseScore];
    ...
    */
    ...
    initMaxBet() {
        ...
        switch (maxBank) {
            case 1:
                opbank.children[2].active = false;
                opbank.children[3].active = false;
                opbank.children[4].active = false;
                break;
            case 2:
                opbank.children[3].active = false;
                opbank.children[4].active = false;
                break;
            case 3:
                opbank.children[3].active = true;
                opbank.children[4].active = false;
                break;
        }

        //TODO：换加倍按钮图片
        var opbet = this.playerUI[0].op_list[1].getChildByName('btns');
        if (maxBet != 8) {
            if (maxBet == 3)
                opbet.children[0].active = false;
            for (var i = 1; i < 4; i++) {
                opbet.children[i].getChildByName('font').getComponent(cc.Label).string = maxBet / 3 * i + ' 倍';
            }
        }
    },
    ...
    initRoomInfo() {
        ...
        var rule_str = this.getRuleString(RoomMgr.Instance()._Rule);
        cc.find('room_info/base_score', this.node).getComponent(cc.Label).string = score_str;
        cc.find('room_info/rule', this.node).getComponent(cc.Label).string = rule_str;
    },

    getRuleString(rules) {
        const baseScore = [0, 10, 20, 50, 100];
        var str = baseScore[rules.baseScore] * 20 + '积分准入';
        //str += '  初始分:' + (scoreTypeStr[rules.scoreType]);
        if (rules.gameType == 1) {
            str += '\n最大抢庄' + rules.maxGrub + '倍';
            str += '  最大下注' + rules.maxBet + '倍';
        }
        if (rules.gameType == 1) {
            str += '\n抢庄';
        }
    ...
    */
    ...
    showResult() {
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            else if (view != null && view > -1 && view < 5) {
