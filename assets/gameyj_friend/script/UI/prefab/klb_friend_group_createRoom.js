...
cc.Class({
    ...
    onLoad: function () {
        if (this.rule) {
            this.rule.active = !cc._useChifengUI;
        }
    },
     ...
     * 显示游戏列表
     */
    showGameList: function (gameId, data) {
        if (cc._useChifengUI) {
        ...
        this.cardRichTxt.string = data.type == 0 ? '剩余房卡' + data.cards + '张' : '';
        ...
        cc.dd.ResLoader.loadPrefab(prefab_config.KLB_FG_GAMETAGE, function (prefab) {
            for (var i = 0; i < data.length; ++i) {
                ...
                if (gameData) {
                    ...
                    callback();
                    //方正填大坑
                    if (gameData.gameid == Define.GameType.TDK_COIN && AppConfig.GAME_PID != 10004 && AppConfig.GAME_PID != 10009 && AppConfig.GAME_PID != 10011 && AppConfig.GAME_PID != 10013 && AppConfig.GAME_PID != 10014) {
                        itemData = game_List.getItem(function (item) {
    ...
    setCreateRoomRule: function (ui, gameid) {
        ...
        cc.dd.UIMgr.openUI(ui, function (node) {
            ...
            cc.find("commonRule/proxy", node).active = false;
