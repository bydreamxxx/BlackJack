...
cc.Class({
    ...
    setData: function (roomData) {
        ...
        if (gameInfo) {
            ...
            var str = '房号:' + roomData.roomId + '      ' + this.convertTimeDay(roomData.timestamp) + '   ' +
                this.convertTimeDate(roomData.timestamp) + '       回放码:' + roomData.historyId;// + '  比赛创建人ID:' + roomData.createUserId;
            ...
            if (cc._useChifengUI) {
                if (roomData.dissolveRoleId) {
                    if (roomData.dissolveRoleId == -1) {//群主解散
    ...
    checkBtnCallBack: function () {
        ...
        cc.log("此处设置了club_game_wafanum，如果有问题，记得检查");
        cc.sys.localStorage.setItem('club_game_wafanum', clubMgr.getLastRoomID());
        if (cc._chifengGame || cc.dd.CheckGames.isHeLongMJ(this._data.gameType)) {
    ...
    onClickShare(){
        ...
        // let originParent = this.node.parent;
        //
        // let canvas = cc.find('Canvas');
        // this.node.parent = canvas
        // this.node.x = -canvas.width / 2 + this.node.width / 2;
        // this.node.y = -canvas.height / 2 + this.node.height / 2;

        cc.dd.native_wx.SendCustomNodeShotToWechat(this.node, 0xFFFF, ()=>{
            // this.node.parent = originParent;
            // this.node.x = originX;
            // this.node.y = originY;
        });
    }
