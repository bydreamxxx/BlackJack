...
let brnn_audio_cfg = require('brnn_audio_cfg_jlb');
...
const chipList = [[10, 20, 30, 50], [30, 50, 100, 200], [100, 300, 500, 1000]];
...
cc.Class({
    ...
    getChipSprite(betnum, isSelf) {
        ...
        let idx = values.indexOf(betnum);
        if (idx != -1) {
            return this.splist_chip[idx];
        }
        return null;
    },

    getChipSpriteDi(betnum) {
        let values = [];//下注值列表，去重加排序
        for (var i = 0; i < 3; i++) {
            for (var j = 0; j < 4; j++) {
                if (values.indexOf(chipList[i][j]) == -1) {
    ...
    initChipList(coin, bet) {
        ...
        for (var i = 3; i > -1; i--) {
            ...
            var btnSprite = this.getChipSpriteDi(chipList[this._chipGroup][i]);
            cc.find('bottom/chips/chip_' + (i + 1), this.node).getComponent(cc.Sprite).spriteFrame = btnSprite;
            cc.find('bottom/chips/chip_' + (i + 1), this.node).getComponent(cc.Button).normalSprite = btnSprite;
            cc.find('bottom/chips/chip_' + (i + 1), this.node).getComponent(cc.Button).pressedSprite = btnSprite;
            cc.find('bottom/chips/chip_' + (i + 1), this.node).getComponent(cc.Button).hoverSprite = btnSprite;
    ...
    onExit(event, custom) {
        ...
        if (brnn_Data.getMyBetTotal() > 0) {
            ...
            cc.dd.DialogBoxUtil.show(0, '强退将暂时扣除500000金币，用于本局结算，结算后自动返还剩余金币，是否退出？', '是', '否', this.sendLeaveRoom, null, '退出游戏');
        }
        else {
            this.sendLeaveRoom();
        }
    ...
    */
    ...
    //单个筹码飞出
    singleBet(num, pos, sprite, delay) {
        ...
        node.getChildByName('num').getComponent(cc.Label).string = numToStr(num);
        sp.enabled = false;
        sp.spriteFrame = sprite;
    ...
    playerBet(msg) {
        ...
        if (result != null) {//自己
            ...
            if (result == 0) {
                ...
                if (sp) {
                    ...
                    while (chips.length) {
                        ...
                        this.singleBet(chip[0], bet_pos, sp);
                    }
                }
            }
            else if (result == -1) {
            ...
            while (chips.length) {
                ...
                var chip = chips.splice(random_idx, 1);
                var sp = this.getChipSprite(chip[0], false);
                this.singleBet(chip[0], bet_pos, sp, true);
            }
        }
    ...
    showPlayerList(event, custom) {
        ...
        scp.setDataProvider(plist, 0, function (itemNode, index) {
            ...
            p1_node.getChildByName('coin').getComponent(cc.Label).string = p1_data.score.toString();
            if (p2_data) {
                ...
                p2_node.getChildByName('coin').getComponent(cc.Label).string = p2_data.score.toString();
                p2_node.active = true;
            }
            else {
    ...
    updateBankerList() {
        ...
        cc.find('info/max_bet', this.node).getComponent(cc.Label).string = bankerlist.length == 0 ? '50000' : (bankerlist.length * 50000).toString();
    ...
    updateReqBankNum() {
        ...
        if (isBanker) {
            ...
            cc.find('info/bank_gold', this.node).getComponent(cc.Label).string = '500000';
