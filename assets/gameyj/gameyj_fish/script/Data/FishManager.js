...
var Cbullet = require('BulletNode').CBullet;
...
const data_fishroom = require('fishroom');
var fishSender = require('gameyj_fish_sender');
var data_fishpath = require('fishpath');
const data_fishtype = require('fishtype');
const data_fisharraymin = require('fisharraymin');
...
const defaultBetDir = [Math.PI / 2, -Math.PI / 2, -Math.PI / 2, Math.PI / 2];
...
var FishManager = cc.Class({
    ...
    setRoomType: function (roomType) {//设置房间配置id
        this.m_nRoomType = roomType + 13800;
    },
    ...
    getFishToLock: function (fishId) {//获取锁定的鱼
        if (fishId && fishId > 0) {
            ...
            if (fish) {
                ...
                if (rectContainsPoint(this.m_mainUI.getFishPoolNode(), fishArPos)) {
    ...
    /////////////////////////////////////////////////////////////////////////////翻转切换Begin////////////////////////////////////////////
    toClientSite: function (serverSite) {//座位号翻转为客户端:以1做为起始
        if (this.m_bFilp)
            return (serverSite - 1 + 2) % 4 + 1;
        else
            return serverSite;
    },
    ...
    setBetTimes: function (ty) {//发送加减档位消息
        var myData = this.getMyData(); //获取自己档位信息
        if (myData == null)
            return;
        if (ty == 2) {
            if (myData.bet <= this.m_tRoomCfg.bet_min) //达到最小档位
                return;
        } else {
            if (myData.bet >= this.m_tRoomCfg.bet_max) //达到最大档位
                return;
        }
        fishSender.betSender(ty);//发送信息上去
    },
    /////////////////////////////////////////////////////////////////////////////鱼创建相关begin////////////////////////////////////////
    ...
    createFish: function (fishID, dataID, pathID, offset, stopScale, moveTime) {//创建散鱼
        var fish = CFish.createFish(fishID, dataID, pathID, offset, stopScale, moveTime, this.m_mainUI.getFishPoolNode()); //鱼工具创建鱼
        if (fish) {
    ...
    createLineForm: function (msg) {
        for (var k = 0; k < msg.fishIdsList.length; k++) {
            var v = msg.fishIdsList[k];
            this.createFish(v, msg.fishDataId, msg.pathId, null, k);
        }
    },
        ...
        if (fishCfg) {
            if (buffID == FishType.FishBuff.TypeBomb) {//同类炸弹
                ...
                if (fishes.length > 0) {
                    ...
                    for (var v of fishes) {
                        this.m_mainUI.createFishNetEffect(false, v.pos);//创建网
                    }
                }
    ...
    showBulletCoinEffect: function (userId, bullet, bullet_id) {//击中金币
        ...
        for (var fish of hitFish) {
            if (fish.worldPos)
                this.m_mainUI.showGoldNum(fish.worldPos, fish.win_gold); //金币值显示
            totalGoldNum += fish.win_gold;
            totalSrcCoinNum += fish.fish_bet_times; //总倍率
        }
        ...
        playerData.coin = playerData.coin + totalGoldNum;
    ...
    Bet: function (dir, fishID) {//创建子弹
        ...
        if (own) {
            ...
            if (playerData) {
                ...
                if (this.getMyBulletNums() > 25) { //判定屏幕中子弹数量是否太多
                    cc.log('子弹太多,创建不了子弹');
                ...
                var bulletPos = this.getBulletPos(this.toSeverSite(siteIndex) - 1);//客户端座位号以1起始的

                var bThree = playerData.bet > this.m_tRoomCfg.bet_max / 2; //是否为三发炮弹
                var bulletNode = Cbullet.createBullet(bulletId, bulletPos, this.toSeverDir(dir), fishID, siteIndex, bThree, playerData.buff_id > 0, playerData.buff2_id);//创建子弹
                this.playAudio(7006, false, 0.6);
                ...
                if (bulletNode) {
                    ...
                    this.m_mainUI.getFishPoolNode().addChild(bulletNode.node); //将子弹添加到鱼池节点
    ...
    on_fish_bet: function (msg) {//创建子弹
        ...
        if (player) {
            var playerData = player.getPlayerGameInfo(); //获取游戏数据
            if (playerData) {
                playerData.coin = msg.coin; //玩家金币更新
                ...
                if (msg.userId != cc.dd.user.id) {//不是玩家自己
                    var fishInfo = this.getFishToLock(fishID); //获取锁定鱼信息
                    if (fishInfo) {
                        ...
                        if (fish)
                            ...
                            bulletDir = Math.max(defaultBetDir[siteIndex - 1] - Math.PI / 2, bulletDir)
                            bulletDir = Math.min(defaultBetDir[siteIndex - 1] + Math.PI / 2, bulletDir)
                        }
                    ...
                    var bThree = playerData.bet > this.m_tRoomCfg.bet_max / 2; //是否为三发炮弹
                    var bulletNode = Cbullet.createBullet(msg.bulletId, bulletPos, this.toSeverDir(bulletDir), fishID, siteIndex, bThree, playerData.buff_id > 0, playerData.buff2_id);//创建子弹
                    this.playAudio(7006, false, 0.6);
    ...
    onBulletHitFish: function (bulletId, fishId) {//击中鱼
        ...
        if (bullet && bullet.obj) {
            ...
            if (bullet.nSite == this.m_nSite || bullet.isBot) {
                ...
                fishHit.onHitBullet(bullet.buffId);
    ...
    on_msg_fish_hit_2c: function (msg) {//击中鱼
        ...
        var bHitByBullet = (bullet.hitFishs == null);
        ...
        for (var v of msg.fishsList) {
            if (v.hit != 1) {//未打死
                ...
                if (fish) {
                    if (msg.userID != cc.dd.user.id && bullet.isBot == false)
                        fish.onHitBullet(bullet.buffId); //显示打中鱼的效果
                }
            } else {//打死鱼
                var fishInfo = {};
                fishInfo.fish_bet_times = v.bet; //打鱼倍率
                fishInfo.win_gold = v.win; //打鱼获取的金币
        ...
        if (bullet.hitFishs.length > 0) {
            //生成金币动画
            this.showBulletCoinEffect(msg.userId, bullet, msg.bulletId);
            bullet.hitFishs = [];
        }
    },
    ...
    on_msg_fish_appear: function (msg) {//生成鱼
        ...
        if (msg.form == 1) {//鱼路径生成类型
            ...
            for (var v of msg.fishIdsList) {
                ...
                this.createFish(v, msg.fishDataId, msg.pathId, pos, null) //创建鱼
                k++;
            }
        } else {
            ...
            if (data) {
                ...
                if (dataID > 0) {
                    if (fishIndex < msg.fishIdsList.length) {
                        var fishId = msg.fishIdsList[fishIndex];
                        var fish = CFish.createFishEx(fishId, dataID, fishArrayCfg, k, null, birthPlacePos, this.m_mainUI.getFishPoolNode()); //创建鱼阵的鱼
                        if (fish != null) {
                            if (this.checkFishExistence(fishId)) {
                                cc.log('鱼阵出现重复的鱼');
                            }
