...
var defaultBetDir = [Math.PI / 2, -Math.PI / 2, -Math.PI / 2, Math.PI / 2];
...
var FishPlayerManager = cc.Class({
                ...
                if (player) {
                    ...
                    if (player.userId != cc.dd.user.id) {
                        if (own && this.playerInfo[index].getPlayerCommonInfo().pos == null) {
                            this.playerInfo[index].getPlayerCommonInfo().pos = this.toClientSite(index + 1);
                            this.playerInfo[index].enterGame();
                        }
                    } else if (own.getPlayerCommonInfo().pos == null) { //自己位置未设置
                        cc.log('comm_seat==========' + own.getPlayerCommonInfo().seat);
                        if((own.getPlayerCommonInfo().seat + 1) == 2 || (own.getPlayerCommonInfo().seat + 1) == 3)//位置翻转判定
                            this.m_bFilp = true;
                        this.playerInfo[index].getPlayerCommonInfo().pos = this.toClientSite(own.getPlayerCommonInfo().seat + 1);
                        this.playerInfo[index].enterGame();
    ...
    setPlayerGameData: function(player){
        ...
        if(playerInfo){//查询得到玩家房间管理器数据
            ...
            player.betDir = defaultBetDir[playerInfo.pos]; //方向弧度
