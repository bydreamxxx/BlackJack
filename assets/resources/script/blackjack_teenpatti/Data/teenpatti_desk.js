...
const teenpatti_config = require('teenPatti');
const dsz_userState = require('dsz_config').DSZ_UserState;

var Teenpatti_DeskEvent = cc.Enum({
    TEENPATTI_DEDSK_SEND_POKER: 'dsz_desk_send_poker',//发牌
    ...
    TEENPATTI_DEDSK_SHOW_RESULE: 'dsz_desk_show_result', //显示总结算
    TEENPATTI_DEDSK_DISSOVlE: 'dsz_desk_dissovle', //解散消息
    TEENPATTI_DEDSK_DISSOVLE_RESULT: 'dsz_desk_dissovle_reslut', //解散结果
    New_CHECK_PLAYER_ALL_READY:'dsz_chck_player_all_ready', //朋友场断线重连检查玩家是否全部准备
    TEENPATTI_DEDSK_ERROR:'dsz_desk_op_error',//玩家操作出错
    TEENPATTI_DEDSK_QUICK_RECHARGE: 'dsz_desk_quick_recharge', //快速充值
    TEENPATTI_DEDSK_RECONNECT: 'dsz_desk_reconnect', //断线重连
    TEENPATTI_RESETPLAYERUI: 'dsz_desk_resetplayerui', //重新设置玩家ui
});

var Teenpatti_DeskED = new cc.dd.EventDispatcher();

var Teenpatti_Desk_Data = cc.Class({
    s_teenpatti_desk_data: null,
    statics: {
        Instance: function () {
            if (!this.s_teenpatti_desk_data) {
                this.s_teenpatti_desk_data = new Teenpatti_Desk_Data();
            }
            return this.s_teenpatti_desk_data;
        },

        Destroy: function () {
            if (this.s_teenpatti_desk_data) {
                this.s_teenpatti_desk_data.clear();
                this.s_teenpatti_desk_data = null;
            }
        },
    },
    ...
    initDeskData: function(data){
        ...
        if(this.gameId == 186){
            ...
            this.playModle = this.getConfigData().play_type;
            this.playRule = this.getConfigData().play_rule.split(';');
            this.coinRoomId = this.configId % 18600;
        }

        if(this.isReconnectTag){
            this.m_oReconnectData = data;
            Teenpatti_DeskED.notifyEvent(Teenpatti_DeskEvent.TEENPATTI_DEDSK_RECONNECT);
            return;
        }
        //玩家游戏数据信息
        ...
        data.otherInfosList.forEach(function(otherUser, index){
            ...
            if(index == data.otherInfosList.length - 1 && data.userInfo.userState != 1 && !this.isReconnectTag){
                setTimeout(function() {
                    Teenpatti_DeskED.notifyEvent(Teenpatti_DeskEvent.TEENPATTI_DEDSK_SEND_POKER);
                }.bind(this), 300);
            }
        }.bind(this));
    },
        ...
        this.m_oReconnectData.otherInfosList.forEach(function(otherUser, index){
            ...
            if(index == this.m_oReconnectData.otherInfosList.length - 1  && this.isReconnectTag){
                setTimeout(function() {
                    ...
                    Teenpatti_DeskED.notifyEvent(Teenpatti_DeskEvent.New_CHECK_PLAYER_ALL_READY);
                }.bind(this), 1500);
            }
        }.bind(this));
    },
   ...
   setCurRound: function(round){
    ...
    getConfigData: function(){
        //通过id查询数据
        var self = this;
        const data = teenpatti_config.getItem(function(item){
...
module.exports = {
    Teenpatti_Desk_Data : Teenpatti_Desk_Data,
    Teenpatti_Desk_Ed: Teenpatti_DeskED,
    Teenpatti_Desk_Event: Teenpatti_DeskEvent,
};