...
cc.Class({
    ...
    properties: {
        ...
        p_rule: { default: [], type: cc.Toggle, tooltip: '规则', },
    ...
    onSelectRule: function (target, data) {
        ...
        switch (data) {
            case "js1":
                ...
                group = this.p_js;
                this.m_objRuleIndex.rule[2] = false;
                break;
            case "js2":
            ...
            case "js4":
                ...
                group = this.p_js;
                if (target.isChecked)
                    this.m_objRuleIndex.rule[2] = true;
                break;
            case "fd1":
            ...
            case "fd4":
                ...
                break;
            case "fd5":
                this.m_objRuleIndex.fd = 5;
                group = this.p_fd;
                break;
            case "fd6":
                this.m_objRuleIndex.fd = 6;
                group = this.p_fd;
                break;
            case "rs2":
            ...
            case "gps":
                ...
                break;
            case "rule1":
                this.m_objRuleIndex.rule[0] = target.isChecked;
                group = this.p_rule;
                break;
            case "rule2":
                this.m_objRuleIndex.rule[1] = target.isChecked;
                group = this.p_rule;
                break;
            case "rule3":
                this.m_objRuleIndex.rule[2] = target.isChecked;
                group = this.p_rule;
                if (target.isChecked)
                    this.p_rs[0].check();
                else {
                    if (this.m_objRuleIndex.js == 1)
                        this.p_js[0].check();
                }
                break;
        }

        if (group === this.p_gps || group === this.p_yy || group === this.p_rule) {
    ...
    setSpeRule() {
        this.initToggleColor(this.p_rule);
        this.setCheckedToggle(this.p_rule[0], this.m_objRuleIndex.rule[0]);
        this.setCheckedToggle(this.p_rule[1], this.m_objRuleIndex.rule[1]);
        //this.setCheckedToggle(this.p_rule[2], this.m_objRuleIndex.rule[2]);
    },
    ...
    setFD: function () {
        ...
        switch (this.m_objRuleIndex.fd) {
            ...
            case 5:
                ...
                break;
            case 6:
                this.setCheckedToggle(this.p_fd[5], true);
                break;
        }
    },
    ...
    setRS: function () {
        ...
        switch (this.m_objRuleIndex.rs) {
            ...
            case 4:
                ...
                break;
            case 5:
                this.setCheckedToggle(this.p_rs[3], true);
                break;
        }
    },
    ...
    */
    ...
    getRules(data, isProxy) {
        ...
        var pbRule = new cc.pb.room_mgr.suoha_rule_info();
        var rule = new cc.pb.room_mgr.xl_game_rule();

        pbCommon.setGameType(63);
        if (isProxy) {
        ...
        pbRule.setPlayerNum(data.rs);//人数
        if (data.rule[2])
            pbRule.setMaxScore(7);
        else
            pbRule.setMaxScore(data.fd);//封顶
        //pbRule.setGameType(1);//抢庄
        //pbRule.setScoreType(3);//不限
        pbRule.setGiveUp(data.rule[0]);//超时弃牌
        pbRule.setShowCard(data.rule[1]);//亮底牌
    ...
    setRoomRule(rule) {
        ...
        this.m_objRuleIndex = {
            ...
            club: true,
            //规则 超时弃牌 亮底牌
            rule: [_rule.giveUp, _rule.showCard, false],
        };
    ...
    setRoundCost(player_num) {
        ...
        var conf = cost_room_cards_config.getItemList(function (item) {
            return item.id == cc.dd.Define.GameType.SH_FRIEND && item.player_num == player_num;
        });
