...
cc.Class({
    ...
    properties: {
        ...
        m_nIndex: 0,
        m_nGetIndex: 0,
        m_nSendRecordIndex: 0,
        m_oFailSprite: cc.SpriteFrame, //惜败资源
    },
    ...
    showRecrodInfo: function(){//游戏记录详细信息
        ...
        infoData.rewardsList.forEach(function(reward, index){//结果
            if(reward.value == 0)
                this.setIcon(this.m_tResultNodeList[index], reward.id);
            else if(reward.value == 4)
                this.m_tResultNodeList[index].getComponent(cc.Sprite).spriteFrame = this.m_oFailSprite;
            else
                this.setIcon(this.m_tResultNodeList[index], 12);
