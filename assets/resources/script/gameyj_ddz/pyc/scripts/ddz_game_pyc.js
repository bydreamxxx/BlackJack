...
cc.Class({
     ...
     */
    startFenluo(userId, timeout) {
        var fenluo = cc.find('fenluo', this.node);
        fenluo.active = true;
        var view = this.idToView(userId);
        this._uiComponents[view].showFenluo(timeout);
    },
    ...
    onStackRet(msg) {
        ...
        var stack = msg.stack;
        var view = this.idToView(userId);
        this._uiComponents[view].hideFenluo();
        cc.dd.ShaderUtil.setGrayShader(cc.find('fenluo/stack' + stack.toString(), this.node));
        cc.find('fenluo/stack' + stack.toString(), this.node).tagname = 'choosed';
    },
    ...
    onZhanji(msg) {
        ...
        var totalRound = DDZ_Data.Instance().deskInfo.curCircle;
        ...
        for (var i = 0; i < playList.length; i++) {
            ...
            cc.find('dizhuNum', pNode).getComponent(cc.Label).string = playList[i].landholderTimes;
            cc.find('nongminNum', pNode).getComponent(cc.Label).string = playList[i].farmerTimes;
            cc.find('winNum', pNode).getComponent(cc.Label).string = playList[i].winTimes.toString();
            cc.find('loseNum', pNode).getComponent(cc.Label).string = playList[i].loseTimes.toString();
            if (score > -1) {
    ...
    onInvaite: function (event, custom) {
        ...
        if (cc.sys.isNative|| cc.sys.platform == cc.sys.MOBILE_BROWSER) {
            ...
            wanFa.push('封顶:' + (rules.maxScore == 0x7FFFFFFF ? '不封顶' : rules.maxScore.toString() + '分'));
    ...
    getRuleString: function () {
        ...
        str += '  封顶:' + (rules.maxScore == 0x7FFFFFFF ? '不封顶' : rules.maxScore.toString() + '分');
        if (rules.doubleScore == 1) {
            str += '  自由加倍';
        }
        else if (rules.doubleScore == 2) {
    ...
    initInviteButton: function () {
        var curJushu = DDZ_Data.Instance().curJushu ? DDZ_Data.Instance().curJushu : 1;
        if (curJushu > 1) {
            this.btn_invite.active = false;
            this.btn_friend_invite.active = this.btn_invite.active && RoomMgr.Instance().isClubRoom();
        }
    ...
    */
    ...
    initRoomInfo: function () {
        ...
        if (Rule.cardHolder) {
            cc.find('top/dipai_info/jipaiqi', this.node).active = true;
        }
        ...
        cc.find('fengding_num/lbl', detialNode).getComponent(cc.Label).string = Rule.maxScore == 0x7FFFFFFF ? '不封顶' : Rule.maxScore.toString() + '分';
        cc.find('layout/jiabei', detialNode).getComponent(cc.Label).string = Rule.doubleScore == 1 ? '自由加倍' : (Rule.doubleScore == 2 ? '农民优先' : '不能加倍');
        cc.find('layout/bijiao', detialNode).active = Rule.beenCallScore;
        cc.find('layout/exchange', detialNode).active = Rule.exchangePoker;
        cc.find('layout/extra2', detialNode).active = !Rule.canDaiExtra2;
        cc.find('layout/stack', detialNode).active = Rule.stackGenPoker;
        cc.find('layout/hander', detialNode).active = Rule.cardHolder;
        cc.find('layout/juetou', detialNode).active = Rule.isJuetou;
    ...
    onGameOpening: function () {
        ...
        for (var i = 0; i < fenluo.childrenCount; i++) {
            cc.dd.ShaderUtil.setNormalShader(fenluo.children[i]);
            fenluo.children[i].tagname = 'free';
    ...
    handPoker: function (data) {
        ...
        cc.find('fenluo', this.node).active = false;
    ...
    reconnect: function (data) {
        for (var i = 0; i < this._uiComponents.length; i++) {
            cc.find('gps_bj', this._uiComponents[i].headnode).active = false;
            this._uiComponents[i].hideFenluo();
        }
        this.initGame();
        this.initPlayer();
        cc.find('fenluo', this.node).active = false;
        ...
        switch (DDZ_Data.Instance().deskInfo.deskStatus) {
            ...
            case roomStatus.STATE_CALL_SCORE:
                ...
                break;
            case roomStatus.STATE_STACK_POKER:
                var curTime = data.timeout;
                var stackedList = data.stackedList;
                for (var i = 0; i < stackedList.length; i++) {
                    cc.dd.ShaderUtil.setGrayShader(cc.find('fenluo/stack' + stackedList[i].toString(), this.node));
                    cc.find('fenluo/stack' + stackedList[i].toString(), this.node).tagname = 'choosed';
                }
                var curPlayer = data.curPlayer;
                for (var i = 0; i < this._uiComponents.length; i++) {
                    this._uiComponents[i].hideFenluo();
                }
                this.startFenluo(curPlayer, 15);
                break;
            case roomStatus.STATE_CHANGE_POKER:
                var curTime = data.timeout;
                DDZ_Data.Instance().maxScore = data.curCallScore;
                var times = data.bottomPokersTimes;
                DDZ_Data.Instance().bottomPokersTimes = times;
                ...
                var finished = function () {
                    cc.find('exchange', this.node).getComponent(cc.Animation).off('finished', finished, this);
                    for (var i = 0; i < playerInfo.length; i++) {
                        if (playerInfo[i].isChangePoker != true) {
                            this._uiComponents[this.idToView(playerInfo[i].userId)].showChangeCard(15);
                        }
                        else {
                            this._uiComponents[this.idToView(playerInfo[i].userId)].onExchangeRet(data.changePokerList, true);
                        }
                    }
                }.bind(this);
                cc.find('exchange', this.node).getComponent(cc.Animation).on('finished', finished, this);
                this.playExchangeAnimation('exchange_reset');
    ...
    initUiScript: function (bool) {
        this._uiComponents = [];
        if (bool) {
            this._uiComponents.push(this.node.getComponentInChildren('ddz_pyc_down_ui'));
            this._uiComponents.push(this.node.getComponentInChildren('ddz_pyc_right_ui'));
            this._uiComponents.push(this.node.getComponentInChildren('ddz_pyc_left_ui'));
        }
    },
