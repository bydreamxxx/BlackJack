...
cc.Class({
    ...
    showBottomCard: function (cardlist) {
        ...
        var playFinished = (event) => {
            ...
            self.scheduleOnce(function () {
                ...
                for (var i = 0; i < cards.length; i++) {
                    //cc.find('top/dipai_info/dipai_' + (i + 1).toString(), self.node.parent).scale = 0;
                    self.setDipai(cc.find('top/dipai_info/dipai_' + (i + 1).toString(), self.node.parent), cardlist[i]);
                }
            }, 0.2);
        };
        cc.find('top/dipai_info/bottomcard_ani', this.node.parent).getComponent(cc.Animation).off('finished', playFinished);
        cc.find('top/dipai_info/bottomcard_ani', this.node.parent).getComponent(cc.Animation).on('finished', playFinished);
    ...
    showOutCard: function (cardlist, isLord) {
        ...
        if (cardlist.length == 0) {
            ...
            switch (type) {
                ...
                case 7://飞机不带
                case 8://飞机
                    cc.find('Canvas/root').getComponent('ddz_game_jbc').playAirplaneAnimation();
                    break;
                case 10://炸弹
                    cc.find('Canvas/root').getComponent('ddz_game_jbc').playBombAnimation('zhu');
                    break;
                case 11://火箭
                    cc.find('Canvas/root').getComponent('ddz_game_jbc').playRocketAnimation('1');
                    break;
                default:
                    break;
            }
    ...
    */
    ...
    resetUI: function () {
        ...
        cc.find('offline', this.headnode).active = false;
        cc.find('weak', this.headnode).active = false;
    },

    //设置底牌
    setDipai: function (prefab, cardValue) {
        this.node.parent.getComponent('ddz_game_jbc').setDipai(prefab, cardValue);
    },

    //设置牌值
    setPoker: function (prefab, cardValue) {
        this.node.parent.getComponent('ddz_game_jbc').setPoker(prefab, cardValue);
    },
