...
cc.Class({
    ...
    showBottomCard: function (cardlist) {
        ...
        let sendingCard = this.node.parent.getComponentInChildren('ddz_bsc_down_ui')._sendingCard;
        if (sendingCard) {
            this.calSpecialBottom(cardlist);
            for (var i = 0; i < cards.length; i++) {
                cc.find('top/dipai_info/dipai_' + (i + 1).toString(), this.node.parent).scale = 1;
                this.setDipai(cc.find('top/dipai_info/dipai_' + (i + 1).toString(), this.node.parent), cardlist[i]);
                cc.find('top/dipai_info/bottomcard_ani', this.node.parent).children.forEach(child => {
                    child.active = false;
                });
            }
        }
            ...
            this.scheduleOnce(function () {
                cc.find('top/dipai_info', this.node.parent).getComponent(cc.Animation).play('dipai_ani_jbc_1');
                this.calSpecialBottom(cardlist);
                for (var i = 0; i < cards.length; i++) {
                ...
                switch (type) {
                    case 5://顺子
                        ...
                        if (cards.length > ONE_LINE_NUM) {
                            posX = -cardWidth + 30 - ONE_LINE_NUM / 2 * CARD_OFFSET;
                        }
                        else {
                            posX = -cardWidth + 30 - cards.length / 2 * CARD_OFFSET;
                        }
                        if (tongtianshun) {
                            this.tongtianshun_node.active = true;
                    ...
                    case 8://飞机
                        ...
                        break;
                    case 10://炸弹
                        cc.find('Canvas/root').getComponent('ddz_game').playBombAnimation('you');
                        break;
                    case 11://火箭
                        cc.find('Canvas/root').getComponent('ddz_game').playRocketAnimation('2');
                        break;
                    default:
                        break;
                }
