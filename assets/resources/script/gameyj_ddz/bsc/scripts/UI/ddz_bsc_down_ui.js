...
cc.Class({
    ...
    getHandCardList() {
        var cards = [];
        for (var i = 0; i < this.hand_node.childrenCount; i++) {
            cards.push(parseInt(this.hand_node.children[i].name));
        }
        return cards;
    },
    ...
    refreshHandCards: function (time) {
        ...
        cc.log('刷新手牌,' + duration + 's' + this.getHandCardList());
        if (duration > 0) {
            this.canTouch = false;
    ...
    clearHandCards: function () {
        cc.log('清除手牌,' + this.getHandCardList());
    ...
    playSendCards: function (cards, time) {
        ...
        cc.log('发牌开始,' + duration + 's');
        cc.find('op_jiaofen', this.node).opacity = 0;
        this.remainCard = cards.length;
        var showCompleted = function () {//发牌动画完成回调
            this.canTouch = true;
            this.clearTouchList();
            cc.find('op_jiaofen', this.node).opacity = 255;
        }.bind(this);
        this.clearHandCards();
        ...
        var showFunc = function () {
            ...
            cc.log('发牌完成,手牌:' + this.getHandCardList());
            this._sendingCard = false;
        }.bind(this);
        if (duration > 0) {
            this._sendingCard = true;
    ...
    showCallScoreOp: function (time, maxScore, curtime) {
        ...
        cc.find('op_jiaofen', this.node).getComponentInChildren('ddz_timer').play(time, function () { ddz_send_msg.callScore(0); }, curtime);
    },
    ...
    showBottomCard: function (cardlist) {
        ...
        for (var i = 0; i < cards.length; i++) {
            var dulp = false;
            for (var j = 0; j < this._handCards.length; j++) {
                if (this._handCards[j] == cards[i]) {
                    dulp = true;
                    break;
                }
            }
        ...
        var downCard = function () {
            ...
            cc.log('插入底牌' + cards);
            cc.log('插入后手牌' + this.getHandCardList());
        }.bind(this);
    ...
    showDouble: function (time, curtime) {
        ...
        cc.find('op_jiabei', this.node).getComponentInChildren('ddz_timer').play(time, function () { ddz_send_msg.callDouble(false); }, curtime);
    },
    ...
    showPlaying: function (time, curtime) {
        ...
        if (DDZ_Data.Instance().lastCards && DDZ_Data.Instance().lastCards.length > 0 && DDZ_Data.Instance().lastPlayer != 0) {
            if (ddz.calCards(DDZ_Data.Instance().lastCards, this._handCards)) {
                ...
                if (cc.find('tuoguan_node', this.node).active != true) {
    ...
    showOutCard: function (cardlist, isLord) {
        ...
        if (cardlist.length == 0) {
            ...
            switch (type) {
                ...
                case 7://飞机不带
                case 8://飞机
                    cc.find('Canvas/root').getComponent('ddz_game').playAirplaneAnimation();
                    break;
                case 10://炸弹
                    cc.find('Canvas/root').getComponent('ddz_game').playBombAnimation('zhu');
                    break;
                case 11://火箭
                    cc.find('Canvas/root').getComponent('ddz_game').playRocketAnimation('1');
                    break;
                default:
                    break;
            }
    ...
    */
    ...
    onSelectCardChanged: function () {
        ...
        if (ddz.compareCards(DDZ_Data.Instance().lastCards, selectCards)) {
    ...
    setDipai: function (prefab, cardValue) {
        this.node.parent.getComponent('ddz_game').setDipai(prefab, cardValue);
    },

    //设置牌值
    setPoker: function (prefab, cardValue) {
        this.node.parent.getComponent('ddz_game').setPoker(prefab, cardValue);
    },
    ...
    onCallScore: function (event, data) {
        if (event.target.parent.opacity == 0)
            return;
