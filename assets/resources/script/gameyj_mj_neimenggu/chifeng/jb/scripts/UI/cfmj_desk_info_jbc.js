var baseDeskInfo = require("cfmj_desk_info");
...
var baseDeskInfoJB = cc.Class({
    ...
    onLoad: function () {
        cc.log("cfmj_desk_info_jbc onLoad");
        this._super();
    ...
    initDeskData: function () {
        if (DeskData.Instance().isJBC()) {
            var deskDataJBC = cfmj_desk_data_jbc.getInstance();
    ...
    start: function () {
        if (cfmj_desk_data_jbc.getInstance().getIsReconnect()) {
    ...
    onDestroy: function () {
        ...
        cfmj_desk_data_jbc.getInstance().destroy();
        this._super();
        //RoomMgr.Destroy();
    },
    ...
    on_room_replace: function (msg) {
        if (msg.retCode !== 0) {
            ...
            this.playMatchAnim();
            cfmj_desk_data_jbc.getInstance().setIsMatching(true);
            this.tuo_guan(false);
            cc.find("Canvas/desk_node/jlmj_zhishiqi").active = false;
        }
    },
    ...
    on_room_enter: function () {
        cc.log("cfmj jbc desk on_room_enter--->" + (DeskData.Instance().isGameStart));
    ...
    recover_desk: function () {
        cc.log("cfmj jbc desk recover_desk------执行0");
        this.on_room_enter();
        //this.recoverDesk();
    },
        ...
        if (msg.retCode !== 0) {
            ...
            switch (msg.retCode) {
                case 1:
                    ...
                    this.popupEnterHall(str, cfmj_util.enterRoomList.bind(cfmj_util));
                    break;
                case 2:
    ...
    tuo_guan: function (isShow) {
        if (isShow) {
            ...
            if (this.menu_list) {
                this.menu_list.getComponent("cfmj_game_menu_list").closeMenuAndOptions();
            }
    ...
    clean_tuo_guan: function () {
        if (DeskData.Instance().isoffline) {
            ...
            cc.gateNet.Instance().sendMsg(cc.netCmd.chifengmj.cmd_chifeng_req_update_deposit, req, "chifeng_req_update_deposit", true);
        }
    },
    ...
    onEventMessage: function (event, data) {
        ...
        switch (event) {
            ...
            case DeskEvent.RECOVER_DESK:
                cc.log("cfmj jbc recover_desk------ 开始0");
                this.recover_desk();
                break;
