...
cc.Class({
        ...
        switch (data) {
            case "rs1":
                ...
                var conf = cost_room_cards_config.getItemList(function (item) {
                    return item.id == cc.dd.Define.GameType.CFMJ_FRIEND && item.player_num == 4;
                }.bind(this));
            ...
            case "rs2":
                ...
                var conf = cost_room_cards_config.getItemList(function (item) {
                    return item.id == cc.dd.Define.GameType.CFMJ_FRIEND && item.player_num == 3;
                }.bind(this));
            ...
            case "rs3":
                ...
                var conf = cost_room_cards_config.getItemList(function (item) {
                    return item.id == cc.dd.Define.GameType.CFMJ_FRIEND && item.player_num == 2;
                }.bind(this));
    ...
    onCreate: function (event, custom) {
        /************************游戏统计 start************************/
        cc.dd.Utils.sendClientAction(cc.dd.clientAction.CFMJ_FRIEND, cc.dd.clientAction.T_HALL.CREATE_ROOM);
        /************************游戏统计   end************************/
    ...
    sendCreateRoom: function (data, isProxy) {
        ...
        if(cc._chifengGame){
            cc.sys.localStorage.setItem(cc.dd.user.id + '_cfmj_create', JSON.stringify(data));
            cc.sys.localStorage.setItem(cc.dd.user.id + '_last_enter_game', cc.dd.Define.GameType.CFMJ_FRIEND);
    ...
    initData: function () {
        ...
        if(cc._chifengGame){
            let result = cc.sys.localStorage.getItem(cc.dd.user.id + '_cfmj_create');
    ...
    getRules(data, isProxy){
        ...
        var conf = cost_room_cards_config.getItemList(function (item) {
            return item.id == cc.dd.Define.GameType.CFMJ_FRIEND && item.player_num == data.rs;
        }.bind(this));

        qsOrJs = conf[data.qs].circle_num

        var gameReq = new cc.pb.room_mgr.msg_create_game_req();
        var pbData = new cc.pb.room_mgr.chifeng_req_createdesk();
        ...
        commonGame.setGameType(cc.dd.Define.GameType.CFMJ_FRIEND);
        if (isProxy) {
    ...
    setRoomRule(rule){
        ...
        let _rule = rule.rule.mjChifengRule;

        let qsOrJs = 0;
        var conf = cost_room_cards_config.getItemList(function (item) {
            return item.id == cc.dd.Define.GameType.CFMJ_FRIEND && item.player_num == _rule.usercountlimit;
        }.bind(this));
