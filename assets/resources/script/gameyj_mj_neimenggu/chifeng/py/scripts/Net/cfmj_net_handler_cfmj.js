...
var cc_handler = cc.Class({
    ...
    ctor: function () {
        cc.log("cfmj_net_handler_cfmj 父类");
    },
    ...
    on_chifeng_ack_game_opening: function( msg ) {
        ...
        if(DeskData.Instance().isFriend()){
            cc.find('Canvas/desk_info').getComponent('cfmj_desk_info').cleanJieSuan()
        }else if(DeskData.Instance().isJBC() || DeskData.Instance().isMatch()){
            cc.find('Canvas/desk_info').getComponent('cfmj_desk_info_jbc').cleanJieSuan()
        }
    },

    on_chifeng_ack_game_deal_cards: function( msg ) {
        ...
        if(RoomMgr.Instance()._Rule.paofen == -2){
            DeskED.notifyEvent(DeskEvent.SHOW_PAO_FEN, RoomMgr.Instance()._Rule.paofen);
    ...
    on_chifeng_ack_roomInit: function( msg ) {
        ...
        //     if(msg.playersList[i].playercard.composecardList.length > 0){
        //
        ...
        //                 animation.stop();
        //                 cc.find("Canvas/desk_node").getComponent('cfmj_scene').jlmjPlayerDownSortShouPai();
        //             }
        //         }
        //
        //         break
    ...
    on_chifeng_ack_game_overturn: function( msg ) {
        if (!this.headerHandle(msg)) return;
        ...
        if (!UserPlayer.hasMoPai()) {
            //吃,碰,点杠
            if (menu_list) {
                menu_list.getComponent("cfmj_game_menu_list").setMenus(UserPlayer);
            }
    ...
    on_chifeng_ack_game_send_out_card: function( msg ) {
        ...
        if (!this.headerHandle(msg)) {
            ...
            if(msg.userid == cc.dd.user.id && msg.card && this.realCardID && msg.card.id != this.realCardID[msg.userid]){
                ...
                if(player.shoupai.indexOf(msg.card.id) == -1) {
                    var player_down_ui = cc.find("Canvas/desk_node/jlmj_player_down_ui").getComponent('cfmj_player_down_ui');
    ...
    on_chifeng_ack_update_deposit: function( msg ) {
        if (!this.headerHandle(msg)) return;
    ...
    on_chifeng_ack_chi: function( msg ) {
        if (!this.headerHandle(msg)) return;
        //被吃的牌，移除
        ...
        if(!cc.replay_gamedata_scrolling){
            ...
            if(play_list){
                play_list.getComponent('cfmj_player_list').playerStopChuPaiAni();
            }
            cc.log('停止出牌动画-吃牌');
        }
    ...
    on_chifeng_ack_game_act_peng: function( msg ) {
        if (!this.headerHandle(msg)) return;
        //被碰的牌，移除
        var playerOut = playerMgr.Instance().getPlayer(msg.useridout);
        if (playerOut) {
    ...
    on_chifeng_ack_game_act_gang: function( msg ) {
        if (!this.headerHandle(msg)) return;
        var playerOut = playerMgr.Instance().getPlayer(msg.useridout);
        if (msg.isrob) {
    ...
    on_chifeng_ack_game_act_bugang: function( msg ) {
        if (!this.headerHandle(msg)) return;
        //玩家补杠
    ...
    on_chifeng_ack_game_act_guo: function( msg ) {
        if (!this.headerHandle(msg)) return;
        var player = playerMgr.Instance().getPlayer(msg.userid);
        if(player){
            player.guo();
    ...
    on_chifeng_ack_game_ting: function( msg ) {
        if (!this.headerHandle(msg)) return;
        //听牌
        var player = playerMgr.Instance().getPlayer(msg.userid);
        player.ting();
    ...
    on_chifeng_ack_game_dabao: function( msg ) {
        if (!this.headerHandle(msg)) return;
    ...
    on_chifeng_ack_game_changbao: function( msg ) {
        if (!this.headerHandle(msg)) return;
    ...
    on_chifeng_ack_reconnect: function( msg ) {
        ...
        if(msg.deskrules.desktype == cc.dd.Define.GameType.CFMJ_GOLD){   //金币场
        ...
        let func = ()=>{
            ...
            if(!noUpdateShouPai){
                ...
                DeskED.notifyEvent(DeskEvent.SHOW_PAO_FEN, playerHasPaoFen);
            }
    ...
    on_chifeng_ack_remain_majiang: function( msg ) {
        if (!this.headerHandle(msg)) return;
    ...
    on_chifeng_ack_change_majiang: function( msg ) {
        if (!this.headerHandle(msg)) return;
     ...
     * @param msg
     */
    on_chifeng_game_ack_act_hu: function( msg ) {
        if ( !this.headerHandle( msg ) ) {
        ...
        cc.find('Canvas/desk_info').getComponent('cfmj_desk_info').gameready.active = false;
    ...
    on_chifeng_ack_send_current_result: function( msg ) {
        ...
        this.waitTime = this.waitTime == 0 ? 500 : this.waitTime;
    ...
    on_chifeng_ack_operator: function( msg ) {
        if (!this.headerHandle(msg)) return;
    ...
    on_chifeng_ack_reloading_ok: function( msg ) {
        if (!this.headerHandle(msg)) return;
    ...
    on_chifeng_ack_ready: function( msg ) {
        ...
        if (player) {
            ...
            if(msg.header.userid == cc.dd.user.id){
                ...
                if(DeskData.Instance().isFriend()){
                    cc.find('Canvas/desk_info').getComponent('cfmj_desk_info').cleanJieSuan();
                    cc.find('Canvas/desk_info').getComponent('cfmj_desk_info').setLastJieSuanActive();
                }else if(DeskData.Instance().isJBC() || DeskData.Instance().isMatch()){
                    cc.find('Canvas/desk_info').getComponent('cfmj_desk_info_jbc').cleanJieSuan()
                }
            }
    ...
    on_chifeng_ack_sponsor_dissolve_room: function( msg ) {
        if (!this.headerHandle(msg)) return;
        DeskData.Instance().sponsorDissolveRoom(msg);
    },

    on_chifeng_ack_response_dissolve_room: function( msg ) {
        if (!this.headerHandle(msg)) return;
        DeskData.Instance().responseDissolveRoom(msg);
    },

    on_chifeng_ack_dissolve_room: function( msg ) {
        if (!this.headerHandle(msg)) return;
        if (cc.director.getScene().name != 'cfmj_py_game') {
            return;
        }
        DeskData.Instance().dissolveRoom(msg);
    },

    on_chifeng_ack_exit_room: function( msg ) {
        if (!this.headerHandle(msg)) return;
        DeskData.Instance().exitRoom(msg);
    },

    on_chifeng_ack_finally_result: function( msg ) {
        cc.log('【数据】战绩消息接收 开始');
        if (!this.headerHandle(msg)) return;
        cc.dd.UIMgr.destroyUI(jlmj_prefab.JLMJ_JIESAN);
    ...
    on_chifeng_ack_fen_zhang: function( msg ) {
        if (!this.headerHandle(msg)) return;
        cc.log('【数据】分张消息接收');
        DeskData.Instance().fenzhang();
    },

    on_chifeng_ack_rob_remove_card: function( msg ) {
        if (!this.headerHandle(msg)) return;
    ...
    on_chifeng_game_ack_act_huangzhuangpais: function( msg ) {
        if (!this.headerHandle(msg)) return;
        cc.find('Canvas/desk_info').getComponent('cfmj_desk_info').gameready.active = false;
     ...
     * @param msg
     */
    on_chifeng_ack_bao: function (msg) {
        DeskData.Instance().setBaoPai( msg.card );
        if(!cc.replay_gamedata_scrolling){
            cc.find("Canvas/desk_info").getComponent('cfmj_desk_info').update_bao_pai();
        }
    },
    ...
    on_chifeng_ack_paofen: function(msg){
        var player = playerMgr.Instance().getPlayer(msg.userid);
        if(player){
    ...
    stopFaPaiAni(){
        ...
        if(!cc.dd._.isNull(desk_node)){
            ...
            desk_node.getComponent('cfmj_scene').jlmjPlayerDownSortShouPai();
        }
    }
});
