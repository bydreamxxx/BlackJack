...
const chupai_offset = 25;
const restPt_y = () => {
    if (DeskData.Instance().getIs2D()) {
        if (DeskData.Instance().get2DPai() != 'green') {
            return -287.9;
        } else {
            return -292;
        }
...
cc.Class({
    extends: require('cfmj_player_base_ui'),
    ...
    onLoad: function () {
        cc.log('cfmj_player_down_ui onLoad');
        ...
        this.zhanshi_pai = cc.find("Canvas/desk_node/zhanshi_pais/jlmj_zhanshi_pai_down").getComponent('nmmj_pai');     //展示牌
        ...
        this.menuList = cc.find("Canvas/game_menu_list").getComponent("cfmj_game_menu_list");
    ...
    initShouPai: function () {
        for (var i = 0; i < 13; ++i) {
            ...
            var jlmj_user_shoupai = pai_node.getComponent("nmmj_user_shoupai");
    ...
    initModepai: function () {
        ...
        if (!jlmj_user_shoupai) {
            cc.error("摸牌未挂nmmj_user_shoupai组件");
        }
        jlmj_user_shoupai.setTouchAble(true);
    ...
    touchStart: function (event) {
        ...
        if (!UserPlayer.hasMoPai() || this.menuList.isOpen) {
            return;
        }
        ...
        if (pai_touched) {
            ...
            if (this.touchCardMode == TouchCardMode.DA_TING_PAI) {
                this.closeJiaoInfo();
                cc.log("cfmj touchStart openJiaoInfo");
    ...
    touchMove: function (event) {
        ...
        if (this.pai_touched &&                          //没有选择牌
            ...
            if (new Date().getTime() - this.isCanMove > 50 || this.isCanMove == null) {
                ...
                this.yidong_pai.node.x = event.touch.getLocationX() - this._offsetX;
                this.yidong_pai.node.y = event.touch.getLocationY() - this._offsetY;
                if (this.yidong_pai.node.y > -267 * this._node_scale_y) {
    ...
    touchEnd: function (event) {
        ...
        if (!this.pai_touched) {
            ...
            if (this.touchCardMode == TouchCardMode.DA_TING_PAI) {
                this.closeJiaoInfo();
                cc.log("cfmj touchEnd openJiaoInfo");
                this.openJiaoInfo(this.pai_touched.cardId);
            }
        }
        ...
        if (this.yidong_pai.node.active) {
            if (this.yidong_pai.node.y > -267 * this._node_scale_y) {
                ...
                if (UserPlayer.isTempBaoTing) {
                    ...
                    if (UserPlayer.hasMoPai()) {
                        // if(this.touchCardMode == TouchCardMode.DA_TING_PAI){
                        //     cc.log("cfmj touchEnd sendOutCard");
                        //     UserPlayer.setJiaoInfo(this.yidong_pai.cardId);
                        // }
            ...
            if (!pai_touched.doubleSelected) {
                if (event.touch.getLocationY() - this.yd_y > chupai_offset) {
        ...
        if (this.canTouchPaiAni) {
            ...
            UserPlayer.dapai(DeskData.Instance().sendCard, false);
        }
    },
    ...
    biaojiPai: function (selectId) {
        ...
        for (var i in userList) {
            var com = userList[i].getComponent('cfmj_player_down_ui');
    ...
    sendTingPai: function (playCardId, tingType) {
        ...
        cc.gateNet.Instance().sendMsg(cc.netCmd.chifengmj.cmd_chifeng_req_tingpai_out_card, msg, "cmd_chifeng_req_tingpai_out_card");
    ...
    sendOutCard: function (cardID) {
        ...
        this.quxiaoBiaoji();
        var msg = new cc.pb.chifengmj.chifeng_req_game_send_out_card();
        msg.setCardid(cardID);
        cc.gateNet.Instance().sendMsg(cc.netCmd.chifengmj.cmd_chifeng_req_game_send_out_card, msg, "chifeng_req_game_send_out_card");
        cc.dd.NetWaitUtil.net_wait_start();
    },
    ...
    dapai: function (userplayer, chupai_idx_in_shoupai) {
        ...
        var jlmj_pai = pai_node.getComponent("nmmj_pai");
        if (!jlmj_pai) {
