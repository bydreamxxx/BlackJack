...
var PlayerHead = cc.Class({
    ...
    initUI: function (player) {
        ...
        if (!cc.dd._.isUndefined(player.isbanker) && !player.bready) {
            if (player.isbanker && !this.banker.active) {
    ...
    showPochan: function () {
        if (this.curr_coin == 0 && !this.pochanNode.node.active) {
            ...
            AudioManager.playSound("gameyj_mj_neimenggu/common/audio/mj_effect/pochan");
        }
        if (this.curr_coin > 0 && this.pochanNode.node.active) {
    ...
    playMagicProp: function (idx, fromId, toId) {
        ...
        let heads = cc.find("Canvas").getComponentsInChildren("cfmj_playerHead");
    ...
    setPoChan: function (isshow) {
        ...
        if (RoomMgr.gameId != cc.dd.Define.GameType.CFMJ_FRIEND) {
