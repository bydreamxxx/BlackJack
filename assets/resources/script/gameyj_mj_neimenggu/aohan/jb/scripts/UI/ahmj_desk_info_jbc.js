var baseDeskInfo = require("ahmj_desk_info");
...
var baseDeskInfoJB = cc.Class({
    ...
    onLoad: function () {
        cc.log("ahmj_desk_info_jbc onLoad");
        this._super();
    ...
    initDeskDataUI: function () {
        cc.find("Canvas/desk_node/beforeGame/majiang").active = !DeskData.Instance().isMatch();

        cc.find("Canvas/toppanel/layer_base_score").active = !DeskData.Instance().isMatch();
        cc.find("Canvas/toppanel/btn_match").active = !DeskData.Instance().isMatch();
        ...
        cc.find("Canvas/toppanel/gweixin").active = DeskData.Instance().isMatch() && !this.weixinNotShow;
    ...
    initDeskData: function () {
        if (DeskData.Instance().isJBC()) {
            var deskDataJBC = ahmj_desk_data_jbc.getInstance();

            if (deskDataJBC.getBaseScore() === 0) {
                // cc.find("Canvas/toppanel/layer_base_score").active = false;
                cc.find("Canvas/toppanel/btn_match").active = false;
                return;
            }
            //显示房间信息
            //场次类型
            cc.find("Canvas/toppanel/layer_base_score/sprite_base_score/name").getComponent(cc.Label).string = deskDataJBC.getTitle();
            //底分
            cc.find("Canvas/toppanel/layer_base_score/sprite_base_score/base_score").getComponent(cc.Label).string = Text.TEXT_MJ_JBC_0 + deskDataJBC.getBaseScore();
            //右上角底分
    ...
    start: function () {
        if (ahmj_desk_data_jbc.getInstance().getIsReconnect()) {
    ...
    onDestroy: function () {
        ...
        ahmj_desk_data_jbc.getInstance().destroy();
        this._super();
        //RoomMgr.Destroy();
    },
    ...
    onMatch: function () {
        ...
        let match = cc.find("Canvas/toppanel/btn_match");
        if (match)
    ...
    playMatchAnim: function () {
        // cc.find("Canvas/toppanel/layer_base_score").active = true;
    ...
    on_room_replace: function (msg) {
        if (msg.retCode !== 0) {
            ...
            this.playMatchAnim();
            ahmj_desk_data_jbc.getInstance().setIsMatching(true);
            this.tuo_guan(false);
            cc.find("Canvas/desk_node/jlmj_zhishiqi").active = false;
        }
    },
    ...
    on_room_enter: function () {
        cc.log("ahmj jbc desk on_room_enter--->" + (DeskData.Instance().isGameStart));
        ...
        this.gameready.active = !DeskData.Instance().isGameStart;

        cc.find("Canvas/toppanel/layer_base_score").active = !DeskData.Instance().isMatch();
        cc.find("Canvas/toppanel/btn_match").active = !DeskData.Instance().isGameStart && !DeskData.Instance().isMatch();
    ...
    recover_desk: function () {
        cc.log("ahmj jbc desk recover_desk------执行0");
        this.on_room_enter();
        //this.recoverDesk();
    },
        ...
        if (msg.retCode !== 0) {
            ...
            switch (msg.retCode) {
                case 1:
                    ...
                    this.popupEnterHall(str, ahmj_util.enterRoomList.bind(ahmj_util));
                    break;
                case 2:
    ...
    tuo_guan: function (isShow) {
        if (isShow) {
            ...
            if (this.menu_list) {
                this.menu_list.getComponent("ahmj_game_menu_list").closeMenuAndOptions();
            }
    ...
    clean_tuo_guan: function () {
        if (DeskData.Instance().isoffline) {
            ...
            cc.gateNet.Instance().sendMsg(cc.netCmd.aohanmj.cmd_aohan_req_update_deposit, req, "aohan_req_update_deposit", true);
        }
    },
    ...
    game_opening: function () {
        ...
        ahmj_desk_data_jbc.getInstance().setIsStart(true);
        ahmj_desk_data_jbc.getInstance().setIsMatching(false);
    ...
    onEventMessage: function (event, data) {
        ...
        switch (event) {
            ...
            case DeskEvent.RECOVER_DESK:
                cc.log("ahmj jbc recover_desk------ 开始0");
                this.recover_desk();
                break;
            ...
            case BSC_Event.IS_SHARED:
                ...
                cc.find("Canvas/toppanel/gweixin").active = DeskData.Instance().isMatch() && !data;
                break;
            default:
                break;
        }
    ...
    onScoreShare(event, data) {
        if (!this._jiesuan) {
            var node = cc.find('Canvas/share_hongbao');
            node.active = true;
            cc.WxShareType = 1;
            cc.dd.native_wx.SendNodeShotToWechat(node, 1);
            node.active = false;
        }
    },
