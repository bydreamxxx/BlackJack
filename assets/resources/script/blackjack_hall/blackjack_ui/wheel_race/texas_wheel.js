...
cc.Class({
    ...
    onRunCode(code, callback) {
        ...
        for (let i = 0; i < this.numList.length; i++) {
            ...
            this.numList[i].stopImmediate = (i < (this.numList.length - this.luckyScore.length)-1)
        };
        AudioManager.playSound('blackjack_common/audio/wheelrace', false);
    ...
    checkRunEnd() {
        ...
        if (this.runEndNum < this.luckyScore.length+1){
            AudioManager.playSound('blackjack_common/audio/wheelrace', false);
        }
        if (this.runEndNum == this.numList.length) {
