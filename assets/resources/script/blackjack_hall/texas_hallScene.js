...
//    },
    ...
    onLoad: function () {
        ...
        this.find_room_items();
        this.req_game_hall_data();
        this.initHall();
        this.showHall();
    },
...
//                    number += dataInfo.fangjianrenshu
                    var item = this.roomItems[i];
                    item.tagname = i + 1;
//                    item.parent = this.contentNode;
                    item.getComponent('BlackJack_Hall_RoomItem').init(dataInfo, data.hallGameid, this.onClickRoomItemCallBack.bind(this));
                }
            }
    },
     ...
     * 选中游戏消息回调
     */
    onClickRoomItemCallBack: function(clickId){
        for(var i = 0; i < this.roomItems.length; i++){
            this.roomItems[i].getComponent('BlackJack_Hall_RoomItem').setSelect(false);

            if(this.roomItems[i].tagname == clickId){
                this.roomItems[i].getComponent('BlackJack_Hall_RoomItem').setSelect(true);
                this.selectItem = this.roomItems[i].getComponent('BlackJack_Hall_RoomItem');
            }
        }
    },
    ...
    req_game_hall_data(){
        ...
        protoNewRoomList.setHallGameid(202);
        ...
        cc.dd.NetWaitUtil.show('waiting...');
    },

    find_room_items(){
        for(var i=0; i<5; i++){
            this.roomItems[i] = this.gameScrollView.getChildByName('BlackJack_Common_Room_Item_' + i);
        };
    },
    ...
    onclickQuickStart(){
        ...
        var entermin = 0;
        for (var i = this.roomItems.length - 1; i >= 0; --i) {
            var item_node = this.roomItems[i];
    ...
    checkUpdate(){
        ...
        if (cc.sys.isNative && this.updater) {
            ...
            com_game_download.setUpdateFinishCallback(this.onUpdateEnd.bind(this));
