...
cc.Class({
    ...
    init: function () {
        //底分
        cc.find('ruleNode/room_info/baseScore', this.node).getComponent(cc.Label).string = gdyData.Instance().getScore();
        //玩家信息
    ...
    initChat: function () {
        ...
        for (var i = 0, len = cfg.length; i < len; ++i) {
            var item = cc.instantiate(this.chat_item);
            item.active = true;
            item.tagname = i;
            item.x = 0;
            item.getComponentInChildren(cc.Label).string = cfg[i];
    ...
    onButtonClick: function (event, data) {
        switch (data) {
            ...
            case 'rank'://退出
                ...
                } else if (gdyData.Instance().IsMatching) {
                    // 取消匹配状态
                    this.leave_game_req();
                } else if (gdyData.Instance().isSettlement) {
                    this.leave_game_req();
                } else
                    this.backToHall();
                break;
    ...
    */
    ...
    initZhuoBu() {
        var idx = 0;
        var json = cc.sys.localStorage.getItem('gdy_zhuobu_' + cc.dd.user.id);
        if (json) {
            var sprite = null;
            ...
            if (sprite) {
                ...
                cc.sys.localStorage.setItem('gdy_zhuobu_' + cc.dd.user.id, this.zhuobu[0]._name);
            }
        }
        else {
            cc.find('bg', this.node).getComponent(cc.Sprite).spriteFrame = this.zhuobu[0];
    ...
    nextZhuobu() {
        if (!this.changingZhuobu) {
            if (this.zhuobu[this.idxZm + 1]) {
                ...
                cc.sys.localStorage.setItem('gdy_zhuobu_' + cc.dd.user.id, this.zhuobu[this.idxZm]._name);
     ...
     */
    showRule: function () {
        cc.find('ruleNode/room_info', this.node).active = false;
        cc.find('ruleNode/rule_bg', this.node).active = true;
    },
    ...
    onInvaite: function (event, custom) {
        ...
        if (cc.sys.isNative) {
            // cc.dd.native_wx.SendAppContent(room, "干瞪眼", title + ",不要走,决战到天亮!", Platform.GetAppUrl(AppCfg.GAME_PID, AppCfg.PID));
            //TODO，记得完善wanFa, playerName, usercount, jushu
            let wanFa = [];
            ...
            cc.dd.native_wx.SendAppInvite(info, "干瞪眼", title + ",不要走,决战到天亮!", Platform.wxShareGameUrl[AppCfg.PID]);
        }
        cc.log("邀请好友：", title);
    },
