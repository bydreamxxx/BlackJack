...
const replayProto = {
    ...
    joinPlayingDeskRsp: 'cmd_TdkJoinPlayingDeskRsp', //桌子
    csendPoker: 'cmd_TdkCSendPoker', //发牌
    cbetrsp: 'cmd_TdkCBetRsp',      // 操作
    openPoker: 'cmd_TdkCOpenPoker', //开牌
    croundEnd: 'cmd_TdkCRoundEnd',  //小结算
...
cc.Class({
    ...
    handlerMsg(msg) {
        ...
        switch (id) {
            case proto_id[replayProto.joinPlayingDeskRsp]://桌子
                ...
                break;
            case proto_id[replayProto.csendPoker]://发牌
                cc.gateNet.Instance().excuteReplayMsg(id, msg.content);
                break;
            case proto_id[replayProto.cbetrsp]://操作
                cc.gateNet.Instance().excuteReplayMsg(id, msg.content);
                break;
            case proto_id[replayProto.openPoker]://开牌
                cc.gateNet.Instance().excuteReplayMsg(id, msg.content);
                break;
            ...
            case proto_id[replayProto.croundEnd]://小结算
                cc.gateNet.Instance().excuteReplayMsg(id, msg.content);
                break;
            case proto_id[replayProto.finalResult]: //大结算
                cc.gateNet.Instance().excuteReplayMsg(id, msg.content);
                break;
        }
    },
    ...
    initData(data) {
        ...
        for (var i = 0; i < playerList.length; ++i) {
            tdkRoomData.Instance().addPlayer(playerList[i]);
        }
        var haveSelf = false;
        for (var i = 0; i < playerList.length; i++) {
    ...
    */
    ...
    stepBy(msg) {
        ...
        switch (id) {
            case proto_id[replayProto.joinPlayingDeskRsp]://桌子
                ...
                tdkRoomData.Instance().recordDeskData(msg.content);
                break;
            case proto_id[replayProto.csendPoker]://发牌
                this.playingData.playStatus = replayProto.csendPoker;
                tdkRoomData.Instance().recordSendPoker(msg.content);
                break;
            case proto_id[replayProto.cbetrsp]://操作
                this.playingData.playStatus = replayProto.cbetrsp;
                tdkRoomData.Instance().recordBetRsp(msg.content);
                break;
            case proto_id[replayProto.openPoker]://开牌
                this.playingData.playStatus = replayProto.openPoker;
                tdkRoomData.Instance().recordopenPoker(msg.content);
                break;
            case proto_id[replayProto.croundEnd]://小结算
                this.playingData.playStatus = replayProto.croundEnd;
                tdkoperationData.Instance().setCRoundEndMsgData(msg.content);
                break;
        }
    },
    ...
    onDestroy: function () {
        ...
        CPlayerData.Instance().isRecord = false;
