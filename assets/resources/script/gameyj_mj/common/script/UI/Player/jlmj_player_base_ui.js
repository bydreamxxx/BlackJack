...
const analysts = require('jlmj_pai_analysts');
...
cc.Class({
    ...
    properties: {
        head: { default: null, type: require("jlmj_playerHead"), tooltip: '头像', },              //头像
        zhanshi_pai: { default: null, type: require('jlmj_pai'), tooltip: '出牌展示牌' },                   //展示牌
    ...
    getPaiToID: function (cardID) {
        ...
        for (var i = 0; i < this.chuPai.length; ++i) {
            var pai = this.chuPai[i].getComponent("jlmj_pai");
    ...
    getAllPaiToID: function (cardID, isuser) {
        ...
        if (this.shouPai && isuser) {
            for (var i = 0; i < this.shouPai.length; ++i) {
                var pai = this.shouPai[i].getComponent("jlmj_pai");
    ...
    reset_zhanshi_pai: function () {
        ...
        if (use2D) {
            ...
            this.zhanshi_pai.value.node.y = -5;
        } else {
    ...
    chi: function (data) {
        ...
        this.resetBaiPai(player);
        this.updateShouPai(player);
        mj_audio.playAudioBySex("chi", player.sex);
    ...
    peng: function (data) {
        ...
        this.resetBaiPai(player);
        this.updateShouPai(player);
        mj_audio.playAudioBySex("cha", player.sex);
    ...
    hu: function (player, isZiMo) {
        ...
        huInfoList.push({ type: HuType.GANG_HUA_HU, ani: ['hu', 'huXS'], audio: 'dianpao_hu' });
        ...
        zimoHuList.push({ type: HuType.GANG_HUA_HU, ani: ['zimo', 'zimoXS'], audio: 'zimo' });
    ...
    hupaiAni: function (maxHu, isZimo) {
        ...
        //         this.jbdga_ani.playAnimation(maxHu.ani,1);
        //         cc._pauseLMAni = true;
        //         setTimeout(function () {
        //             this.onHuAniFinished();
        //         }.bind(this),2000);
        //         break;
        //     case HuType.PIAO_HU:
        //         this.db_hu_2.node.active = true;
        //         this.db_hu_2.playAnimation(maxHu.ani,1);
        //         cc._pauseLMAni = true;
        //         setTimeout(function () {
        //             this.onHuAniFinished();
        //         }.bind(this),2000);
        //         break;
        //     case HuType.QI_DUI:
        //         this.db_hu_3.node.active = true;
        //         this.db_hu_3.playAnimation(maxHu.ani,1);
        //         cc._pauseLMAni = true;
        //         setTimeout(function () {
        //             this.onHuAniFinished();
        //         }.bind(this),2000);
        //         break;
        //     default:
        //         this.db_hu_1.node.active = true;
        //         this.db_hu_1.playAnimation(maxHu.ani,1);
        //         cc._pauseLMAni = true;
        //         setTimeout(function () {
        //             this.onHuAniFinished();
        //         }.bind(this),2000);
    ...
    angang: function (data) {
        ...
        mj_audio.playAudioBySex("anDan", player.sex);
    ...
    _setAngGang: function (pais, baipai, player) {
        ...
        for (var idx = 0; idx < pais.length; ++idx) {
            ...
            if (idx == 0) {
                ...
                else if (player.kaipai_an_gang_list.length > 0) { //结算时打开其他玩家暗杠
                    jlmj_pai.cfgArrObj = use_kaipai_cfg ? this.pai3dCfg.shoupai_kaipai_up_cfg : this.pai3dCfg.baipai_open_up_cfg;
                    jlmj_pai.kaipai(jlmj_pai.cfgArrObj['frame_' + (mj_index + 1)], jlmj_pai.cfgArrObj['value_' + (mj_index + 1)], jlmj_pai.cfgArrObj, jlmj_pai.cfgArrObj['liangzhang_' + (mj_index + 1)], jlmj_pai.cfgArrObj['hunpai_' + (mj_index + 1)]);
                    for (var id in player.kaipai_an_gang_list) {
                        var gang_data = player.kaipai_an_gang_list[id];
                        if (gang_data.show) {
                            jlmj_pai.setValue(gang_data.value);
                            gang_data.show = false;
                            break;
                        }
                    }
    ...
    _setbaipaiValue: function (pais, baipai, player) {
        ...
        pais.forEach(function (jlmj_pai, idx) {
            ...
            if (player.viewIdx == 0 && use2D && player.state != PlayerState.HUPAI) {
                if (pais.length == 5) {
    ...
    setZFB_chi: function (baipai, cardList, mj_index, player) {
        ...
        if (analysts.Instance().isZFB(arr)) {
    ...
    bugang: function (data) {
        ...
        mj_audio.playAudioBySex(getName(pai_id), player.sex);
    ...
    getFullSoundPath: function (cfgName, fileCount) {
        ...
        if (RoomMgr.Instance().isTuiDaoHuMJ()) {
            soundPath = "gameyj_mj/tuidaohu/py/audio/"
        }
        var randomIdx = null;
        if (fileCount > 1) {
    ...
    onEventMessage: function (event, data) {
        ...
        switch (event) {
            ...
            case PlayerEvent.DAPAI:
                ...
                break;
            case PlayerEvent.BEICHI:
                this.beichi(player);
                break;
            case PlayerEvent.CHI:
                this.chi(data);
                break;
            case PlayerEvent.BEIPENG:
                this.beipeng(data);
                break;
            case PlayerEvent.PENG:
                this.peng(data);
                break;
            case PlayerEvent.BEIGANG:
                this.beigang(data);
                break;
            case PlayerEvent.FGANG:
                this.fgang(data);
                break;
            case PlayerEvent._19GANG1:
                this._19gang1(data);
                break;
            case PlayerEvent._19GANG9:
                this._19gang9(data);
                break;
            case PlayerEvent.ZFBGANG:
                this.zfbgang(data);
                break;
            case PlayerEvent.BUGANG:
                this.bugang(data);
                break;
            case PlayerEvent.BAGANG:
                this.bagang(data);
                break;
            case PlayerEvent.ANGANG:
                this.angang(data);
                break;
            case PlayerEvent.DIANGANG:
                this.diangang(data);
                break;
            case PlayerEvent.TING:
                this.ting(player);
                this.head.setTing(true);
                break;
            case PlayerEvent.BEIHU:
                this.beihu(data);
                break;
            case PlayerEvent.HU:
                this.hu(player, this.isZiMo(player.huTypeList));
                break;
            case PlayerEvent.KAIPAI:
                this.updateShouPai(player);
                break;
            ...
            case PlayerEvent.READY:
                ...
                break;
            case PlayerEvent.REMOVE_ROB_FAILED:
                this.resetBaiPai(player);
                break;
            case PlayerEvent.UPDATE_OWNER:
                this.updateOwner(data);
                break;
            case PlayerEvent.DAPAI_CDING://显示头像打牌特效
                this.showChupai(player.userId, player.dapaiCD);
                break;
            case PlayerEvent.CLEAR_DAPAI_CDING://取消头像特效
                this.showChupai(-1);
                break;
            case PlayerEvent.UPDATE_BANKER: //刷新房主
                this.head.setZJ(player.isbanker);
                break;
            case PlayerEvent.DA_BAO: // 打宝
                this.speakDaBao();
                break;
            case PlayerEvent.SHOW_CLICK://显示可操作的牌
                this.showHideClickCard(data[1], data[2], data[3]);
                break;
            case PlayerEvent.SHOW_DAPAI_TING:
                this.showDaPaiTing(data[1], data[2], data[3]);
                break;
            case PlayerEvent.CLEA_SELECT_CARD:
                this.crearSelectCard();
                break;
            case PlayerEvent.SET_COIN://设置金币
                this.setHeadCoin(player.coin);
                break;
            case PlayerEvent.TAKE_OVER_ZSQ:
                var lastCardNode = this.chuPai[this.chuPai.length - 1];
                this.setZsq(lastCardNode, this.viewIdx);
                break;
            case PlayerEvent.AUTO_CHU_PAI:
                this.autoChupai(data[0], data[1]);
                break;
            case PlayerEvent.IS_ON_LINE:
                this.head.setLX(!data[1]);
                break;
            case PlayerEvent.PLAY_MID2SHOU_PAI:
                this.play_mid2dapai_ani(data[1]);
                break;
            case PlayerEvent.CHANGE_PAI_UI:
    ...
    removeChupai: function (player) {
        ...
        //     let jlmj_pai = this.chuPai[i].getComponent('jlmj_pai');
