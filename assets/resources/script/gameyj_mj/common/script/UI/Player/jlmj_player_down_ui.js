...
const pai_analysts = require('jlmj_pai_analysts');
...
cc.Class({
    extends: require('./jlmj_player_base_ui'),

    properties: {
        touch_node: { default: null, type: cc.Node, tooltip: '触摸节点' },
    ...
    onLoad: function () {
        ...
        this.initPai();
        cc.log('jlmj_player_down_ui onLoad');
    ...
    getBaipaiShouPaiDis: function (state) {
        if (state == PlayerState.DAPAI) {
            ...
            if (this.baipai_ui_list.length == 1) {
                ...
                } else if (last_baipai_ui.pais.length == 4) {
                    return DeskData.Instance().getIs2D() ? 82 : 100;
                }
            }
            return 100;
    ...
    updateShouPai: function (player) {
        ...
        if (player.hasMoPai()) {//说明手牌中是包含
            ...
            if (player.state != PlayerState.HUPAI) {
                cc.find("Canvas/game_menu_list").getComponent("jlmj_game_menu_list").setMenus(player);
            }
        } else {
            this.modepai.node.active = false;
        }
    ...
    biaojiPai: function (selectId) {
        ...
        for (var i in userList) {
            var com = userList[i].getComponent('jlmj_player_down_ui');
    ...
    sendTingPai: function (playCardId, tingType) {
        ...
        cc.gateNet.Instance().sendMsg(cc.netCmd.jilinmajiang.cmd_p17_req_tingpai_out_card, msg, "cmd_p17_req_tingpai_out_card");
    ...
    sendOutCard: function (cardID) {
        ...
        this.quxiaoBiaoji();
        var msg = new cc.pb.jilinmajiang.p17_req_game_send_out_card();
        msg.setCardid(cardID);
        cc.gateNet.Instance().sendMsg(cc.netCmd.jilinmajiang.cmd_p17_req_game_send_out_card, msg, "p17_req_game_send_out_card");
        cc.dd.NetWaitUtil.net_wait_start();
    },
