...
var DeskData = cc.Class({
    ...
    setCurrRound: function (currValue) {
        if (currValue && this.gameStatus != GameState.NoStart && DeskData.Instance().isFriend()) {
    ...
    setDeskRule: function (msg, gamestatus) {
        if (!msg)
            return;
        this.beishu = msg.createcfg.guangguotype;//逛锅-
        this.isDianPaoSanJia = msg.createcfg.isdianpaosanjia; //是否点炮包三家
        this.isUseYaoJiu = msg.createcfg.isuseyaojiu; // 是否有幺九蛋-
        this.isXiaoJiFeiDan = msg.createcfg.isxiaojifeidan; // 是否小鸡飞蛋-
        this.isKuaiBao = msg.createcfg.iskuaibao; // 是否快宝
        this.isKuaiGuo = msg.createcfg.iskuaiguo; // 是否快锅
        this.isXiaoJiWanNeng = msg.createcfg.isxiaojiwanneng; // 是否小鸡万能宝-
        this.isYaoJiuSanSe = msg.createcfg.isyaojiusanse; // 是否幺九蛋顶三色-
        this.isUnCheat = msg.createcfg.isuncheat;	//是否防作弊-gps
        this.payType = msg.createcfg.paytype; //房费支付模式

        this.userCountLimit = msg.createcfg.usercountlimit;
        if (gamestatus) {
            // 游戏所处状态: 0.未开始 1.已开始
            this.gameStatus = gamestatus;
        }
        this.setTotalRound(msg.createcfg.boardscout);//总圈数--
    ...
    dabao: function (msg) {
        ...
        playerMgr.Instance().getPlayerList().forEach(function (player) {
            if(player){
        ...
        cc.log("【数据】" + "用户:" + msg.userid + " 打宝" + this.unBaopai ? pai3d_value.desc[this.unBaopai] : "");
    ...
    isFriend: function () {
        var currSceneName = cc.dd.SceneManager.getCurrScene().name;
        var g_id = RoomMgr.Instance().gameId==cc.dd.Define.GameType.JLMJ_FRIEND;
        var c_name = currSceneName==cc.dd.Define.GameId[cc.dd.Define.GameType.JLMJ_FRIEND];
        return c_name && g_id;
    },
    isReplay: function () {
        var currSceneName = cc.dd.SceneManager.getCurrScene().name;
        return currSceneName=='jlmj_replay_game'&&RoomMgr.Instance().gameId==cc.dd.Define.GameType.JLMJ_FRIEND;
    },
    isJBC: function () {
        var currSceneName = cc.dd.SceneManager.getCurrScene().name;
        return currSceneName==cc.dd.Define.GameId[cc.dd.Define.GameType.JLMJ_GOLD]&&RoomMgr.Instance().gameId==cc.dd.Define.GameType.JLMJ_GOLD;
    },

    isMatch: function () {
        var g_id = RoomMgr.Instance().gameId==cc.dd.Define.GameType.JLMJ_MATCH;
        var c_name = cc.dd.SceneManager.getCurrScene().name==cc.dd.Define.GameId[cc.dd.Define.GameType.JLMJ_MATCH];
        return g_id && c_name;
    },

    isInMaJiang: function () {
        let scenename = cc.dd.SceneManager.getCurrScene().name;
        if(scenename == cc.dd.Define.GameId[cc.dd.Define.GameType.JLMJ_GOLD] || scenename == cc.dd.Define.GameId[cc.dd.Define.GameType.JLMJ_FRIEND] || scenename == 'jlmj_replay_game' || cc.dd.Define.GameId[cc.dd.Define.GameType.JLMJ_MATCH]){
            return true
        }else{
            return false;
        }
