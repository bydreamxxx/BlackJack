...
cc.Class({
    ...
    onSelectRule: function (target, data) {
        ...
        switch (data) {
            case "ff1":
                this.m_objRuleIndex.ff = 1;
                group = this.p_ff;
                break;
            case "ff2":
                this.m_objRuleIndex.ff = 2;
                group = this.p_ff;
                break;
            case "qs1":
            ...
            case "rs1":
                ...
                var conf = cost_room_cards_config.getItemList(function (item) {
                    return item.id == cc.dd.Define.GameType.JLMJ_FRIEND && item.player_num == 4;
                }.bind(this));
            ...
            case "rs2":
                ...
                var conf = cost_room_cards_config.getItemList(function (item) {
                    return item.id == cc.dd.Define.GameType.JLMJ_FRIEND && item.player_num == 3;
                }.bind(this));
            ...
            case "rs3":
                ...
                var conf = cost_room_cards_config.getItemList(function (item) {
                    return item.id == cc.dd.Define.GameType.JLMJ_FRIEND && item.player_num == 2;
                }.bind(this));
                ...
                break;
            case "fd1":
                this.m_objRuleIndex.fd = 16;
                group = this.p_fd;
                break;
            case "fd2":
                this.m_objRuleIndex.fd = 32;
                group = this.p_fd;
                break;
            case "fd3":
                this.m_objRuleIndex.fd = 64;
                group = this.p_fd;
                break;
            case "gd1":
            ...
            case "wf1":
                ...
                if (!this.m_objRuleIndex.wf[1]) {
                    this.setEnableToggle(this.p_wf[3], false);
                    this.m_objRuleIndex.wf[4] = this.m_objRuleIndex.wf[1];
                } else {
                    this.setEnableToggle(this.p_wf[3], true);
                    //this.m_objRuleIndex.wf[4] = this.isCheckedToggle( this.p_wf[3] );
                }
                group = this.p_wf;
                break;
            case "wf2":
    ...
    sendCreateRoom: function (data, isProxy) {
        ...
        cc.sys.localStorage.setItem(cc.dd.user.id + '_jlmj_create', JSON.stringify(data));
     ...
     */
    setGD: function (gd) {
        this.initToggleColor(this.p_gd);
        switch (this.m_objRuleIndex.gd) {
            case 0:
                this.setCheckedToggle(this.p_gd[0], true);
                break;
            case 1:
                this.setCheckedToggle(this.p_gd[1], true);
                break;
            case 2:
                this.setCheckedToggle(this.p_gd[2], true);
                break;
            case 3:
                this.setCheckedToggle(this.p_gd[3], true);
                break;
        }
    },
    ...
    setWF: function () {
        ...
        this.setCheckedToggle(this.p_wf[0], this.m_objRuleIndex.wf[1]);
        this.setCheckedToggle(this.p_wf[1], this.m_objRuleIndex.wf[2]);
        this.setCheckedToggle(this.p_wf[2], this.m_objRuleIndex.wf[3]);
    ...
    initData: function () {
        ...
        this.m_objRuleIndex = {
            ...
            gd: 1,
            // 玩法
            wf: [true, true, false, false, true, true],
            // 防作弊
            fzb: true,
            // 是否快锅
            kg: true,
        ...
        let result = cc.sys.localStorage.getItem(cc.dd.user.id + '_jlmj_create');
    ...
    getRules(data, isProxy){
        ...
        var conf = cost_room_cards_config.getItemList(function (item) {
            return item.id == cc.dd.Define.GameType.JLMJ_FRIEND && item.player_num == this.m_objRuleIndex.rs;
        }.bind(this));

        qsOrJs = conf[this.m_objRuleIndex.qs].circle_num

        var gameReq = new cc.pb.room_mgr.msg_create_game_req();
        var pbData = new cc.pb.room_mgr.p17_req_createdesk();
        ...
        pbData.setGuangguotype(data.gd);
        pbData.setIsdianpaosanjia(data.wf[0]);
        pbData.setIsuseyaojiu(data.wf[1]);
        pbData.setIsxiaojiwanneng(data.wf[2]);
        pbData.setIsxiaojifeidan(data.wf[3]);
        pbData.setIsyaojiusanse(data.wf[4]);
        pbData.setIskuaibao(data.wf[5]);
        pbData.setIsuncheat(data.fzb);
        pbData.setIskuaiguo(data.kg);
        pbData.setGps(data.fzb);
        pbData.setIsyuyin(data.yy);
        pbData.setReservedList([String(data.fd)]);

        commonGame.setGameType(20);
        if (isProxy) {
    ...
    setRoomRule(rule){
        ...
        let _rule = rule.rule.mjJilinRule;
        if(!_rule.reservedList || _rule.reservedList.length == 0){
            switch(_rule.guangguotype){
                case 0:
                    _rule.fengding = 16;
                    break;
                case 1:
            ...
            _rule.fengding = parseInt(_rule.reservedList[0]);
        }

        let qsOrJs = 0;
        var conf = cost_room_cards_config.getItemList(function (item) {
            return item.id == cc.dd.Define.GameType.JLMJ_FRIEND && item.player_num == _rule.usercountlimit;
        }.bind(this));
        ...
        this.m_objRuleIndex = {
            ...
            fd: _rule.fengding,
            // 锅底
            gd: _rule.guangguotype,
            // 玩法
            wf: [_rule.isdianpaosanjia, _rule.isuseyaojiu, _rule.isxiaojiwanneng, _rule.isxiaojifeidan, _rule.isyaojiusanse, _rule.iskuaibao],
            // 防作弊
            fzb: _rule.gps,
            // 是否快锅
            kg: _rule.iskuaiguo,
