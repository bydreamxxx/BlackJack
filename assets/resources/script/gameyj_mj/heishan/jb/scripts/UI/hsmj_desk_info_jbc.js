var baseDeskInfo = require( "hsmj_desk_info" );
...
var baseDeskInfoJB = cc.Class({
    ...
    onLoad:function () {
        cc.log("hsmj_desk_info_jbc onLoad");
        this._super();
        this.initDeskDataUI();
        this.initDeskData();
    },
    ...
    initDeskData: function() {
        var deskDataJBC= hsmj_desk_data_jbc.getInstance();
    ...
    start: function () {
        if( hsmj_desk_data_jbc.getInstance().getIsReconnect() ) {
    ...
    onDestroy: function() {
        ...
        hsmj_desk_data_jbc.getInstance().destroy();
        this._super();
        //RoomMgr.Destroy();
    },
    ...
    on_room_replace:function (msg) {
        if( msg.retCode !== 0 ) {
            ...
            this.playMatchAnim();
            hsmj_desk_data_jbc.getInstance().setIsMatching(true);
            this.tuo_guan(false);
            cc.find("Canvas/desk_node/jlmj_zhishiqi").active = false;
        }
    },
    ...
    on_room_enter:function () {
        cc.log("hsmj jbc desk on_room_enter--->"+(DeskData.Instance().isGameStart));
    ...
    recover_desk:function () {
        cc.log("hsmj jbc desk recover_desk------执行0");
        this.on_room_enter();
        //this.recoverDesk();
    },
        ...
        if( msg.retCode !== 0 ) {
            ...
            switch (msg.retCode) {
                case 1:
                    ...
                    this.popupEnterHall( str, hsmj_util.enterRoomList.bind(hsmj_util) );
                    break;
                case 2:
    ...
    tuo_guan: function (isShow) {
        if(isShow){
            ...
            if(this.menu_list){
                this.menu_list.getComponent("hsmj_game_menu_list").closeMenuAndOptions();
            }
    ...
    clean_tuo_guan:function () {
        if(DeskData.Instance().isoffline) {
            const req = new cc.pb.heishanmj.heishan_req_update_deposit();
            req.setIsdeposit(false);
            cc.gateNet.Instance().sendMsg(cc.netCmd.heishanmj.cmd_heishan_req_update_deposit, req, "heishan_req_update_deposit", true);

            if(UserPlayer.Instance().state != PlayerState.HUPAI && UserPlayer.Instance().state != PlayerState.TINGPAI){
                cc.find("Canvas/game_menu_list").getComponent("hsmj_game_menu_list").setMenus(UserPlayer.Instance());
            }
        }
    },
    ...
    onEventMessage: function (event, data) {
        ...
        switch (event) {
            ...
            case DeskEvent.RECOVER_DESK:
                cc.log("hsmj jbc recover_desk------ 开始0");
                this.recover_desk();
                break;
