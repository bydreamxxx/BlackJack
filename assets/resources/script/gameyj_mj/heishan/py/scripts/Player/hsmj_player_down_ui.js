...
cc.Class({
    ...
    onLoad: function () {
        cc.log('hsmj_player_down_ui onLoad');
    ...
    updateShouPai: function (player) {
        ...
        if (player.hasMoPai()) {//说明手牌中是包含
            ...
            if (player.state != PlayerState.HUPAI && player.state != PlayerState.TINGPAI) {
                cc.find("Canvas/game_menu_list").getComponent("hsmj_game_menu_list").setMenus(player);
            }
        } else {
            this.modepai.node.active = false;
        }
    ...
    touchStart: function (event) {
        ...
        if (pai_touched) {
            ...
            if (this.touchCardMode == TouchCardMode.DA_TING_PAI) {
                this.closeJiaoInfo();
                cc.log("hsmj touchStart openJiaoInfo");
    ...
    touchEnd: function (event) {
        ...
        if (!this.pai_touched) {
            ...
            if (this.touchCardMode == TouchCardMode.TING_PAI) {
                this.closeJiaoInfo();
                cc.log("hsmj touchEnd openJiaoInfo");
                this.openJiaoInfo(this.pai_touched.cardId);
            }
        }
            ...
            if (this.yidong_pai.node.y > -200 * this._node_scale_y) {
                ...
                if (UserPlayer.isTempBaoTing) {
                    ...
                    if (UserPlayer.hasMoPai()) {
                        // if(this.touchCardMode == TouchCardMode.DA_TING_PAI){
                        //     cc.log("hsmj touchEnd sendOutCard");
                        //     UserPlayer.setJiaoInfo(this.yidong_pai.cardId);
                        // }
    ...
    biaojiPai: function (selectId) {
        ...
        for (var i in userList) {
            var com = userList[i].getComponent('hsmj_player_down_ui');
    ...
    sendTingPai: function (playCardId, tingType) {
        ...
        cc.gateNet.Instance().sendMsg(cc.netCmd.heishanmj.cmd_heishan_req_tingpai_out_card, msg, "cmd_heishan_req_tingpai_out_card");
    ...
    sendOutCard: function (cardID) {
        ...
        this.quxiaoBiaoji();
        var msg = new cc.pb.heishanmj.heishan_req_game_send_out_card();
        msg.setCardid(cardID);
        cc.gateNet.Instance().sendMsg(cc.netCmd.heishanmj.cmd_heishan_req_game_send_out_card, msg, "heishan_req_game_send_out_card");
        cc.dd.NetWaitUtil.net_wait_start();
    },
    ...
    ting: function (player) {
        ...
        this.closeJiaoInfo();
        var desk_info = cc.find('Canvas/desk_info').getComponent('hsmj_desk_info');
