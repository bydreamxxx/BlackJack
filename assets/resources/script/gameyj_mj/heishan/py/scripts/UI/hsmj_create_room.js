...
cc.Class({
    ...
    properties: {
        ...
        p_gf: { default: [], type: cc.Toggle, tooltip: '杠分', },
        ...
        switch (data) {
            case "rs1":
                ...
                var conf = cost_room_cards_config.getItemList(function (item) {
                    return item.id == cc.dd.Define.GameType.HSMJ_FRIEND && item.player_num == 4;
                }.bind(this));
            ...
            case "rs2":
                ...
                var conf = cost_room_cards_config.getItemList(function (item) {
                    return item.id == cc.dd.Define.GameType.HSMJ_FRIEND && item.player_num == 3;
                }.bind(this));
            ...
            case "rs3":
                ...
                var conf = cost_room_cards_config.getItemList(function (item) {
                    return item.id == cc.dd.Define.GameType.HSMJ_FRIEND && item.player_num == 2;
                }.bind(this));
            ...
            case "qs3":
                ...
                break;
            case "gf1":
                this.m_objRuleIndex.gf = 0;
                group = this.p_gf;
                break;
            case "gf2":
                this.m_objRuleIndex.gf = 1;
                group = this.p_gf;
                break;
            case "fd1":
    ...
    onCreate: function (event, custom) {
        /************************游戏统计 start************************/
        cc.dd.Utils.sendClientAction(cc.dd.clientAction.HSMJ_FRIEND, cc.dd.clientAction.T_HALL.CREATE_ROOM);
        /************************游戏统计   end************************/
    ...
    sendCreateRoom: function (data, isProxy) {
        ...
        cc.sys.localStorage.setItem(cc.dd.user.id + '_hsmj_create', JSON.stringify(data));
     ...
     */
    setGF: function () {
        this.initToggleColor(this.p_gf);
        switch (this.m_objRuleIndex.gf) {
            case 0:
                this.setCheckedToggle(this.p_gf[0], true);
                break;
            case 1:
                this.setCheckedToggle(this.p_gf[1], true);
                break;
            case 2:
                this.setCheckedToggle(this.p_gf[2], true);
                break;
        }
    },
    ...
    initData: function () {
        ...
        this.m_objRuleIndex = {
            ...
            fd: 0,
            // 玩法
            wf: [false, true, true, true, true, true, true, true, true],
            // 防作弊
            fzb: true,
            //俱乐部限定
            club: false,
        ...
        let result = cc.sys.localStorage.getItem(cc.dd.user.id + '_hsmj_create');
        if(cc.dd._.isString(result) && result != ""){
            ...
            if(data){
                ...
                this.m_objRuleIndex.fd = data.fd - 1;
            }
        }
    },
    ...
    getRules(data, isProxy){
        ...
        var conf = cost_room_cards_config.getItemList(function (item) {
            return item.id == cc.dd.Define.GameType.HSMJ_FRIEND && item.player_num == this.m_objRuleIndex.rs;
        }.bind(this));

        qsOrJs = conf[this.m_objRuleIndex.qs].circle_num

        var gameReq = new cc.pb.room_mgr.msg_create_game_req();
        var pbData = new cc.pb.room_mgr.heishan_req_createdesk();
        ...
        pbData.setIsjihujiafan(data.wf[0]);
        pbData.setIshaidilao(data.wf[1]);
        pbData.setIsguoquanhu(data.wf[2]);
        pbData.setIshavednxb(data.wf[3]);
        pbData.setIsqingyise(data.wf[4]);
        pbData.setIsdandiaojiahu(data.wf[5]);
        pbData.setIsdumenjiafan(data.wf[6]);
        pbData.setIsqidui(data.wf[7]);
        pbData.setIsdanhu19jiahu(data.wf[8]);

        pbData.setGangfentype(data.gf);
        pbData.setFengdingtype(data.fd);

        pbData.setGps(data.fzb);
        pbData.setIsyuyin(data.yy);

        commonGame.setGameType(72);
        if (isProxy) {
    ...
    setRoomRule(rule){
        ...
        let _rule = rule.rule.mjHeishanRule;

        let qsOrJs = 0;
        var conf = cost_room_cards_config.getItemList(function (item) {
            return item.id == cc.dd.Define.GameType.HSMJ_FRIEND && item.player_num == _rule.usercountlimit;
        }.bind(this));
        ...
        this.m_objRuleIndex = {
            ...
            // 封顶
            fd: _rule.fengdingtype - 1,
            //杠分
            gf: _rule.gangfentype,
            // 玩法
            wf: [_rule.isjihujiafan, _rule.ishaidilao, _rule.isguoquanhu, _rule.ishavednxb, _rule.isqingyise, _rule.isdandiaojiahu, _rule.isdumenjiafan, _rule.isqidui, _rule.isdanhu19jiahu],
            // 防作弊
            fzb: _rule.gps,
            //俱乐部限定
            club: _rule.gps,
