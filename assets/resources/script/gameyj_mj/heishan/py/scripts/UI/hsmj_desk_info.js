...
var UserPlayer = require("hsmj_userPlayer_data");
...
var baseDeskInfo = cc.Class({
    ...
    onLoad: function() {
        cc.log("hsmj_desk_info onLoad");
        this.logo = cc.find("Canvas/desk_node/c-logo-heishan").getComponent(cc.Sprite);
    ...
    initLocalData: function() {
        ...
        //桌布
        var json = cc.sys.localStorage.getItem('hsmj_zhuobu_set_' + cc.dd.user.id);
        if (json) {
            var sprite = null;
            ...
            if (sprite) {
                ...
                cc.sys.localStorage.setItem('hsmj_zhuobu_set_' + cc.dd.user.id, this.zhuomianImg[0]._name);
            }
        }
        else {
            this.deskImage.spriteFrame = this.zhuomianImg[0];
    ...
    initGuiZeInfo: function() {
        ...
        let gangfen = [Text.TEXT_PY_RULE_95, Text.TEXT_PY_RULE_96];
        let fengding = [Text.TEXT_PY_RULE_54, Text.TEXT_PY_RULE_97, Text.TEXT_PY_RULE_98, Text.TEXT_PY_RULE_99, Text.TEXT_PY_RULE_100]

        this.gz_arr_box = [];
        this.gz_arr_box.push(cc.dd.Text.TEXT_PY_RULE_94.format([RoomMgr.Instance()._Rule.usercountlimit]));
        this.gz_arr_box.push(gangfen[RoomMgr.Instance()._Rule.gangfentype]);
        this.gz_arr_box.push(fengding[RoomMgr.Instance()._Rule.fengdingtype]);
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isjihujiafan ? Text.TEXT_PY_RULE_101 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.ishaidilao ? Text.TEXT_PY_RULE_64 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isguoquanhu ? Text.TEXT_PY_RULE_102 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.ishavednxb ? Text.TEXT_PY_RULE_103 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isqingyise ? Text.TEXT_PY_RULE_29 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isdandiaojiahu ? Text.TEXT_PY_RULE_104 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isdumenjiafan ? Text.TEXT_PY_RULE_105 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isqidui ? Text.TEXT_PY_RULE_27 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isdanhu19jiahu ? Text.TEXT_PY_RULE_106 : '');
        ...
        this.gz_arr_info.push({ str: Text.TEXT_PY_RULE_94.format([RoomMgr.Instance()._Rule.usercountlimit]), nodetype: 0 });
        this.gz_arr_info.push({ str: juquan_txt.format([RoomMgr.Instance()._Rule.boardscout]), nodetype: 0 });

        this.gz_arr_info.push({ str: gangfen[RoomMgr.Instance()._Rule.gangfentype], nodetype: 1 });
        this.gz_arr_info.push({ str: fengding[RoomMgr.Instance()._Rule.fengdingtype], nodetype: 1 });

        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.isjihujiafan ? Text.TEXT_PY_RULE_101 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.ishaidilao ? Text.TEXT_PY_RULE_64 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.isguoquanhu ? Text.TEXT_PY_RULE_102 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.ishavednxb ? Text.TEXT_PY_RULE_103 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.isqingyise ? Text.TEXT_PY_RULE_29 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.isdandiaojiahu ? Text.TEXT_PY_RULE_104 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.isdumenjiafan ? Text.TEXT_PY_RULE_105 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.isqidui ? Text.TEXT_PY_RULE_27 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.isdanhu19jiahu ? Text.TEXT_PY_RULE_106 : '', nodetype: 1 });
    ...
    initZhiNan: function() {
        if (this.zhinan.active && DeskData.Instance().zhiNan_site != null) {
            ...
            this.zhinan.getComponent("hsmj_zhinan_ui").initsetZhiNan(site);
    ...
    recoverDesk: function() {
        cc.log("hsmj 朋 友 recover_desk------ 接收1");
        cc.log("------恢复桌子------py");
        this.updateDesk();
        DeskData.Instance().enterSceneRecoverDesk(this.sendReadyOK.bind(this));
        if (this.fenzhang) {
    ...
    sendReadyOK: function() {
        ...
        var msg = new cc.pb.heishanmj.heishan_req_reloading_ok();
        cc.gateNet.Instance().sendMsg(cc.netCmd.heishanmj.cmd_heishan_req_reloading_ok, msg, "heishan_req_reloading_ok");
        this.initGuiZeInfo();
    },
    ...
    jiesuan: function(data) {
        ...
        this.jiesuan_TimeID = this.scheduleOnce(function() {
            ...
            if (cc.director.getScene().name == 'hsmj_py_game' ||
                cc.director.getScene().name == 'hsmj_jbc_game') {
                cc.dd.UIMgr.destroyUI(jlmj_prefab.JLMJ_JIAOPAI_INFO);
    ...
    fen_zhang_ani: function() {
        ...
        var player_down_ui = cc.find("Canvas/desk_node/jlmj_player_down_ui").getComponent('hsmj_player_down_ui');
    ...
    leave_tips: function(data) {
        ...
        if (DeskData.Instance().isFriend()) {//朋友场
            ...
            } else if (hsmj_desk_data_jbc.getInstance().getIsMatching() || DeskData.Instance().player_read_gamne) {//取消匹配状态
                txt = Text.TEXT_LEAVE_ROOM_3;
                callfunc = this.jbc_matching_leave_room_req;
            } else if (hsmj_desk_data_jbc.getInstance().getIsStart()) {//游戏中退出游戏
    ...
    py_dissolve_room_req: function() {
        ...
        var msg = new cc.pb.heishanmj.heishan_req_sponsor_dissolve_room();
        msg.setSponsorid(dd.user.id);
        cc.gateNet.Instance().sendMsg(cc.netCmd.heishanmj.cmd_heishan_req_sponsor_dissolve_room, msg, "heishan_req_sponsor_dissolve_room");
    },
    ...
    on_room_ready: function(msg) {
        if(hsmj_desk_data_jbc.getInstance().getIsStart()){
        ...
        if (msg.retCode !== 0) {
            ...
            switch (msg.retCode) {
                case 1:
                    ...
                    this.popupEnterHall(str, hsmj_util.enterHall.bind(hsmj_util));
                    break;
                case 2:
    ...
    py_ready: function() {
        ...
        var msg = new cc.pb.heishanmj.heishan_req_ready();
        cc.gateNet.Instance().sendMsg(cc.netCmd.heishanmj.cmd_heishan_req_ready, msg, "heishan_req_ready");
    },
    ...
    update_player_info: function() {
        ...
        this.zhinan.getComponent("hsmj_zhinan_ui").initDirection();
        var play_list = cc.find('Canvas/player_list').getComponent('hsmj_player_list');
    ...
    getTextByWanFa: function() {
        ...
        this.gz_arr_box = [];
        this.gz_arr_box.push(cc.dd.Text.TEXT_PY_RULE_94.format([cur_rule.usercountlimit]));
        this.gz_arr_box.push(gangfen[cur_rule.gangfentype]);
        this.gz_arr_box.push(fengding[cur_rule.fengdingtype]);
        this.gz_arr_box.push(cur_rule.isjihujiafan ? Text.TEXT_PY_RULE_101 : '');
        this.gz_arr_box.push(cur_rule.ishaidilao ? Text.TEXT_PY_RULE_64 : '');
        this.gz_arr_box.push(cur_rule.isguoquanhu ? Text.TEXT_PY_RULE_102 : '');
        this.gz_arr_box.push(cur_rule.ishavednxb ? Text.TEXT_PY_RULE_103 : '');
        this.gz_arr_box.push(cur_rule.isqingyise ? Text.TEXT_PY_RULE_29 : '');
        this.gz_arr_box.push(cur_rule.isdandiaojiahu ? Text.TEXT_PY_RULE_104 : '');
        this.gz_arr_box.push(cur_rule.isdumenjiafan ? Text.TEXT_PY_RULE_105 : '');
        this.gz_arr_box.push(cur_rule.isqidui ? Text.TEXT_PY_RULE_27 : '');
        this.gz_arr_box.push(cur_rule.isdanhu19jiahu ? Text.TEXT_PY_RULE_106 : '');
    ...
    onInvaite: function(event, custom) {
        ...
        jlmj_audio_mgr.com_btn_click();
        var title = Text.TEXT_PY_RULE_94.format(playerMgr.Instance().playerList.length);
    ...
    on_show_result_view: function() {
        ...
        cc.dd.UIMgr.openUI(jlmj_prefab.MJ_ZHANJITONGJI, function(ui) {
            var fj_arr = [];
            var juquan_txt = RoomMgr.Instance()._Rule.usercountlimit != 4 ? Text.TEXT_PY_RULE_10 : Text.TEXT_PY_RULE_11;
            fj_arr.push(Text.TEXT_PY_RULE_94.format([RoomMgr.Instance()._Rule.usercountlimit]));
    ...
    onEventMessage: function(event, data) {
        ...
        switch (event) {
            ...
            case HallCommonEvent.HALL_NO_RECONNECT_GAME:
                cc.log("hsmj  响应------> HallCommonEvent.HALL_NO_RECONNECT_GAME");
            ...
            case DeskEvent.RECOVER_DESK://恢复桌子
                cc.log("hsmj 朋 友 recover_desk------ 开始0");
                this.recoverDesk();
                break;
    ...
    _change2D() {
        ...
        cc.find("Canvas/desk_node/jlmj_player_left_ui").getComponent('hsmj_player_left_ui').resetConfig();
        cc.find("Canvas/desk_node/jlmj_player_down_ui").getComponent('hsmj_player_down_ui').resetConfig();
        cc.find("Canvas/desk_node/jlmj_player_right_ui").getComponent('hsmj_player_right_ui').resetConfig();
        cc.find("Canvas/desk_node/jlmj_player_up_ui").getComponent('hsmj_player_up_ui').resetConfig();
    },
