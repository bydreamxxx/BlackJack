...
var cc_handler = cc.Class({
    ...
    ctor: function () {
        cc.log("hsmj_net_handler_hsmj 父类");
    },
    ...
    on_heishan_ack_game_opening: function( msg ) {
        ...
        if(DeskData.Instance().isFriend()){
            cc.find('Canvas/desk_info').getComponent('hsmj_desk_info').cleanJieSuan()
        }else if(DeskData.Instance().isJBC() || DeskData.Instance().isMatch()){
            cc.find('Canvas/desk_info').getComponent('hsmj_desk_info_jbc').cleanJieSuan()
        }
    },

    on_heishan_ack_game_deal_cards: function( msg ) {
    ...
    on_heishan_ack_game_overturn: function( msg ) {
        if (!this.headerHandle(msg)) return;
        ...
        if (!UserPlayer.hasMoPai()) {
            //吃,碰,点杠
            if (menu_list) {
                menu_list.getComponent("hsmj_game_menu_list").setMenus(UserPlayer);
            }
    ...
    on_heishan_ack_game_send_out_card: function( msg ) {
        ...
        if (!this.headerHandle(msg)) {
            ...
            if(msg.userid == cc.dd.user.id && msg.card && this.realCardID && msg.card.id != this.realCardID[msg.userid]){
                ...
                if(player.shoupai.indexOf(msg.card.id) == -1) {
                    ...
                    //     for(let i = UserPlayer.waitDapai.length - 1; i >=0; i--){
    ...
    on_heishan_ack_update_deposit: function( msg ) {
        if (!this.headerHandle(msg)) return;
    ...
    on_heishan_ack_chi: function( msg ) {
        if (!this.headerHandle(msg)) return;
        //被吃的牌，移除
    ...
    on_heishan_ack_game_act_peng: function( msg ) {
        if (!this.headerHandle(msg)) return;
        //被碰的牌，移除
        var playerOut = playerMgr.Instance().getPlayer(msg.useridout);
        if (playerOut) {
    ...
    on_heishan_ack_game_act_gang: function( msg ) {
        if (!this.headerHandle(msg)) return;
        var playerOut = playerMgr.Instance().getPlayer(msg.useridout);
        if (msg.isrob) {
    ...
    on_heishan_ack_game_act_bugang: function( msg ) {
        if (!this.headerHandle(msg)) return;
        //玩家补杠
    ...
    on_heishan_ack_game_act_guo: function( msg ) {
        if (!this.headerHandle(msg)) return;
        UserPlayer.guo();
        DeskED.notifyEvent(DeskEvent.CLOSE_MENU, []);
    ...
    on_heishan_ack_game_ting: function( msg ) {
        if (!this.headerHandle(msg)) return;
        //听牌
        var player = playerMgr.Instance().getPlayer(msg.userid);
        player.ting();
    ...
    on_heishan_ack_game_dabao: function( msg ) {
        if (!this.headerHandle(msg)) return;
    ...
    on_heishan_ack_game_changbao: function( msg ) {
        if (!this.headerHandle(msg)) return;
    ...
    on_heishan_ack_reconnect: function( msg ) {
        ...
        if(msg.deskrules.desktype != 72){   //金币场
    ...
    comData: function (msg) {
        ...
        if (isClear) {
            ...
            msg.deskgameinfo.remaincards = RoomMgr.Instance()._Rule.ishavednxb ? 136 : 120;
            msg.deskgameinfo.banker = 0;
        }
    },

    on_heishan_ack_remain_majiang: function( msg ) {
        if (!this.headerHandle(msg)) return;
    ...
    on_heishan_ack_change_majiang: function( msg ) {
        if (!this.headerHandle(msg)) return;
     ...
     * @param msg
     */
    on_heishan_game_ack_act_hu: function( msg ) {
        if ( !this.headerHandle( msg ) ) {
    ...
    on_heishan_ack_operator: function( msg ) {
        if (!this.headerHandle(msg)) return;
    ...
    on_heishan_ack_reloading_ok: function( msg ) {
        if (!this.headerHandle(msg)) return;

        DeskData.Instance().unlockScene();
    },

    on_heishan_ack_ready: function( msg ) {
        if (!this.headerHandle(msg)) return;
        ...
        if (player) {
            ...
            if(msg.header.userid == cc.dd.user.id){
                ...
                if(DeskData.Instance().isFriend()){
                    cc.find('Canvas/desk_info').getComponent('hsmj_desk_info').cleanJieSuan();
                    cc.find('Canvas/desk_info').getComponent('hsmj_desk_info').setLastJieSuanActive();
                }else if(DeskData.Instance().isJBC() || DeskData.Instance().isMatch()){
                    cc.find('Canvas/desk_info').getComponent('hsmj_desk_info_jbc').cleanJieSuan()
                }
            }
    ...
    on_heishan_ack_sponsor_dissolve_room: function( msg ) {
        if (!this.headerHandle(msg)) return;
        DeskData.Instance().sponsorDissolveRoom(msg);
    },

    on_heishan_ack_response_dissolve_room: function( msg ) {
        if (!this.headerHandle(msg)) return;
        DeskData.Instance().responseDissolveRoom(msg);
    },

    on_heishan_ack_dissolve_room: function( msg ) {
        if (!this.headerHandle(msg)) return;
    ...
    on_heishan_ack_exit_room: function( msg ) {
        if (!this.headerHandle(msg)) return;
        DeskData.Instance().exitRoom(msg);
    },

    on_heishan_ack_finally_result: function( msg ) {
        cc.log('【数据】战绩消息接收 开始');
        if (!this.headerHandle(msg)) return;
        cc.dd.UIMgr.destroyUI(jlmj_prefab.JLMJ_JIESAN);
    ...
    on_heishan_ack_fen_zhang: function( msg ) {
        if (!this.headerHandle(msg)) return;
        cc.log('【数据】分张消息接收');
        DeskData.Instance().fenzhang();
    },

    on_heishan_ack_rob_remove_card: function( msg ) {
        if (!this.headerHandle(msg)) return;
    ...
    on_heishan_game_ack_act_huangzhuangpais: function( msg ) {
        if (!this.headerHandle(msg)) return;
     ...
     * @param msg
     */
    on_heishan_ack_bao: function (msg) {
    ...
    stopFaPaiAni(){
        ...
        if(!cc.dd._.isNull(desk_node)){
            ...
            desk_node.getComponent('hsmj_scene').jlmjPlayerDownSortShouPai();
        }
    }
});
