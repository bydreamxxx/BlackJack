...
cc.Class({
    ...
    properties: {
        player_ui_arr: { default: [], type: [require('namj_player_base_ui')], tooltip:"玩家ui 顺序为下右上左", override: true},
    },
    initUI: function () {
    ...
    playerExit: function (player) {
        ...
        } else if( RoomMgr.Instance().gameId == cc.dd.Define.GameType.NAMJ_FRIEND ) {
            if( player.userId === cc.dd.user.id ) {
                // 返回俱乐部
                namj_util.enterHall();
            }
    ...
    on_player_signal_state: function(msg) {
        ...
        if(player){
            if(isWeak && !this.player_ui_arr[player.viewIdx].head.offline.active) {
                this.player_ui_arr[player.viewIdx].head.setWeak(isWeak);
            }
        }
    },
