...
var PlayerState = require("namj_player_data").PlayerState;
...
cc.Class({
    extends: require('namj_player_base_ui'),
    ...
    onLoad: function () {
        cc.log('namj_player_down_ui onLoad');
    ...
    updateShouPai: function (player) {
        ...
        if (!DeskData.Instance().isHu && !DeskData.Instance().isFenZhang &&
        ...
        if (player.hasMoPai()) {//说明手牌中是包含
            ...
            if (player.state != PlayerState.HUPAI && player.state != PlayerState.TINGPAI) {
                cc.find("Canvas/game_menu_list").getComponent("namj_game_menu_list").setMenus(player);
            }
        } else {
            this.modepai.node.active = false;
        }
            ...
            if (this.yidong_pai.node.y > -200 * this._node_scale_y) {
                ...
                if (UserPlayer.isTempBaoTing) {
                    ...
                    if (UserPlayer.hasMoPai()) {
                        if (this.touchCardMode == TouchCardMode.DA_TING_PAI) {
                            cc.log("namj touchEnd sendOutCard");
                            UserPlayer.setJiaoInfo(this.yidong_pai.cardId);
                        }
    ...
    biaojiPai: function (selectId) {
        ...
        for (var i in userList) {
            var com = userList[i].getComponent('namj_player_down_ui');
    ...
    sendTingPai: function (playCardId, tingType) {
        ...
        var msg = new cc.pb.nonganmajiang.na_req_tingpai_out_card();
        msg.setCardid(playCardId);
        msg.setTingtype(tingType);

        cc.gateNet.Instance().sendMsg(cc.netCmd.nonganmajiang.cmd_na_req_tingpai_out_card, msg, "cmd_na_req_tingpai_out_card");
    ...
    sendOutCard: function (cardID) {
        ...
        this.quxiaoBiaoji();
        var msg = new cc.pb.nonganmajiang.na_req_game_send_out_card();
        msg.setCardid(cardID);
        cc.gateNet.Instance().sendMsg(cc.netCmd.nonganmajiang.cmd_na_req_game_send_out_card, msg, "na_req_game_send_out_card");
        cc.dd.NetWaitUtil.net_wait_start();
    },
    ...
    ting: function (player) {
        ...
        this.closeJiaoInfo();
        var desk_info = cc.find('Canvas/desk_info').getComponent('namj_desk_info');
    ...
    sendGuPai: function (playCardId) {
        ...
        var msg = new cc.pb.nonganmajiang.na_req_game_act_guout();
        msg.setCardid(playCardId);

        cc.gateNet.Instance().sendMsg(cc.netCmd.nonganmajiang.cmd_na_req_game_act_guout, msg, "cmd_na_req_game_act_guout");
    },
    ...
    gu: function (data) {
        ...
        // this.tcp_ani.addEventListener(dragonBones.EventObject.LOOP_COMPLETE, this.tcpAniCallback, this);
        this.playSpine(this.cpgtgh_ani, ['gu', 'guXS']);
        DeskED.notifyEvent(DeskEvent.UPDATE_BAO_PAI, []);
    },
