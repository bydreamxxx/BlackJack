...
var hupai_type = require("namj_game_menu").hupai_type;

var menu_type = require("namj_game_menu").menu_type;
...
cc.Class({
    ...
    addMenu: function (type) {
        ...
        var menu = menu_node.getComponent("namj_game_menu");
        if(!menu){
            cc.error("namj_game_menu预制没有挂在namj_game_menu组件");
            return;
        }
        menu.type = type;
    ...
    setMenus: function(player){
        ...
        if(player.canhu == hupai_type.CAN_HU_TRUE_DUIBAO || player.canhu == hupai_type.CAN_HU_TRUE_MOBAO){
            ...
            if(player.cangu && !player.canhu){
                this.onShow_list.push(this.getMenu(this.menu_list,menu_type.GU));
            }
        }
