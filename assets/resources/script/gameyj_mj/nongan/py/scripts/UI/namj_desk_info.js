...
var namj_util = require("namj_util");
var namj_desk_data_jbc = require("namj_desk_data_jbc");
var namj_net_handler_namj = require("namj_net_handler_namj");
...
var playerED = require("namj_player_data").PlayerED;
var PlayerEvent = require("namj_player_data").PlayerEvent;
...
var UserPlayer = require("namj_userPlayer_data");
...
var baseDeskInfo = cc.Class({
    ...
    initDeskDataUI:function () {
        ...
        cc.find("Canvas/desk_node/com_game_head_right").active = false;
        cc.find("Canvas/desk_node/com_game_head_left").active = false;
        cc.find("Canvas/desk_node/com_game_head_up").active = false;
    ...
    initLocalData:function() {
        ...
        //桌布
        var json = cc.sys.localStorage.getItem('namj_zhuobu_set_' + cc.dd.user.id);
        if (json) {
            var sprite = null;
            ...
            if (sprite) {
                ...
                cc.sys.localStorage.setItem('namj_zhuobu_set_' + cc.dd.user.id, this.zhuomianImg[0]._name);
            }
        }
        else {
            this.deskImage.spriteFrame = this.zhuomianImg[0];
    ...
    initGuiZeInfo:function () {
        ...
        this.gz_arr_box = [];
        this.gz_arr_box.push(cc.dd.Text.TEXT_PY_RULE_25.format([RoomMgr.Instance()._Rule.usercountlimit]));
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isqiduidaigu?          Text.TEXT_PY_RULE_21:'');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.istongbaofanfan?       Text.TEXT_PY_RULE_16:'');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.ispiaohudandiaofanfan? Text.TEXT_PY_RULE_22:'');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isqiduiyaojiu?         Text.TEXT_PY_RULE_23:'');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isdianpaosanjia?       Text.TEXT_PY_RULE_24:'');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isuseyaojiu?           Text.TEXT_PY_RULE_5:'');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.ismenqingfanbei?           Text.TEXT_PY_RULE_109:'');
        ...
        this.gz_arr_info.push({str:Text.TEXT_PY_RULE_25.format([RoomMgr.Instance()._Rule.usercountlimit]),nodetype:0});
        this.gz_arr_info.push({str:juquan_txt.format([RoomMgr.Instance()._Rule.boardscout]),nodetype:0});
        this.gz_arr_info.push({str:RoomMgr.Instance()._Rule.isqiduidaigu?          Text.TEXT_PY_RULE_21:'',nodetype:1});
        this.gz_arr_info.push({str:RoomMgr.Instance()._Rule.istongbaofanfan?       Text.TEXT_PY_RULE_16:'',nodetype:1});
        this.gz_arr_info.push({str:RoomMgr.Instance()._Rule.ispiaohudandiaofanfan? Text.TEXT_PY_RULE_22:'',nodetype:1});
        this.gz_arr_info.push({str:RoomMgr.Instance()._Rule.isqiduiyaojiu?         Text.TEXT_PY_RULE_23:'',nodetype:1});
        this.gz_arr_info.push({str:RoomMgr.Instance()._Rule.isdianpaosanjia?       Text.TEXT_PY_RULE_24:'',nodetype:1});
        this.gz_arr_info.push({str:RoomMgr.Instance()._Rule.isuseyaojiu?           Text.TEXT_PY_RULE_5:'',nodetype:1});
        this.gz_arr_info.push({str:RoomMgr.Instance()._Rule.ismenqingfanbei?           Text.TEXT_PY_RULE_109:'',nodetype:1});
    ...
    initZhiNan:function () {
        if(this.zhinan.active && DeskData.Instance().zhiNan_site != null){
            ...
            this.zhinan.getComponent("namj_zhinan_ui").initsetZhiNan(site);
    ...
    change_room:function () {
        ...
        var play_list = cc.find('Canvas/player_list').getComponent('namj_player_list');
        if(play_list) {
    ...
    recoverDesk: function () {
        cc.log("namj 朋 友 recover_desk------ 接收1");
        cc.log("------恢复桌子------py");
        this.updateDesk();
        DeskData.Instance().enterSceneRecoverDesk(this.sendReadyOK.bind(this));
        if (this.fenzhang) {
    ...
    sendReadyOK: function () {
        ...
        var msg = new cc.pb.nonganmajiang.na_req_reloading_ok();
        cc.gateNet.Instance().sendMsg(cc.netCmd.nonganmajiang.cmd_na_req_reloading_ok,msg,"na_req_reloading_ok");
        this.initGuiZeInfo();
    },
    ...
    jiesuan:function (data) {
        ...
        this.jiesuan_TimeID = setTimeout(function () {
            ...
            if (cc.director.getScene().name != 'namj_py_game' ||
                cc.director.getScene().name != 'namj_jbc_game') {
                cc.dd.UIMgr.destroyUI(jlmj_prefab.JLMJ_JIAOPAI_INFO);
    ...
    leave_tips: function (data) {
        ...
        if(DeskData.Instance().isFriend()){//朋友场
            ...
            }else if(namj_desk_data_jbc.getInstance().getIsMatching() || DeskData.Instance().player_read_gamne){//取消匹配状态
                txt = Text.TEXT_LEAVE_ROOM_3;
                callfunc = this.jbc_matching_leave_room_req;
            }else if( namj_desk_data_jbc.getInstance().getIsStart()){//游戏中退出游戏
    ...
    py_dissolve_room_req:function() {
        ...
        var msg = new cc.pb.nonganmajiang.na_req_sponsor_dissolve_room();
        msg.setSponsorid(dd.user.id);
        cc.gateNet.Instance().sendMsg(cc.netCmd.nonganmajiang.cmd_na_req_sponsor_dissolve_room,msg,"na_req_sponsor_dissolve_room");
    },
        ...
        if( msg.retCode !== 0 ) {
            ...
            switch (msg.retCode) {
                case 1:
                    ...
                    this.popupEnterHall( str, namj_util.enterHall.bind(namj_util) );
                    break;
                case 2:
    ...
    py_ready:function () {
        ...
        var msg = new cc.pb.nonganmajiang.na_req_ready();
        cc.gateNet.Instance().sendMsg(cc.netCmd.nonganmajiang.cmd_na_req_ready,msg,"na_req_ready");
    },
    ...
    update_player_info:function () {
        ...
        for(var i in player_list){
            ...
            if (player_list[i].viewIdx == 1) {
                ...
                cc.find("Canvas/desk_node/com_game_head_right").active = true;
            }
            if (player_list[i].viewIdx == 2) {
                ...
                cc.find("Canvas/desk_node/com_game_head_up").active = true;
            }
            if (player_list[i].viewIdx == 3) {
                ...
                cc.find("Canvas/desk_node/com_game_head_left").active = true;
            }
        }
        this.zhinan.getComponent("namj_zhinan_ui").initDirection();
        var play_list = cc.find('Canvas/player_list').getComponent('namj_player_list');
    ...
    getTextByWanFa: function() {
        ...
        this.gz_arr_box = [];
        this.gz_arr_box.push(cc.dd.Text.TEXT_PY_RULE_25.format([cur_rule.usercountlimit]));
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isqiduidaigu?          Text.TEXT_PY_RULE_21:'');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.istongbaofanfan?       Text.TEXT_PY_RULE_16:'');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.ispiaohudandiaofanfan? Text.TEXT_PY_RULE_22:'');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isqiduiyaojiu?         Text.TEXT_PY_RULE_23:'');
    ...
    onInvaite: function (event, custom) {
        ...
        jlmj_audio_mgr.com_btn_click();
        var title = Text.TEXT_PY_RULE_25.format(playerMgr.Instance().playerList.length);
    ...
    onTingPaiTouch:function () {
        var UserPlayer = require('namj_userPlayer_data').Instance();
    ...
    on_show_result_view: function () {
        ...
        cc.dd.UIMgr.openUI(jlmj_prefab.MJ_ZHANJITONGJI, function(ui) {
            var fj_arr = [];
            var juquan_txt = RoomMgr.Instance()._Rule.usercountlimit==2?Text.TEXT_PY_RULE_10:Text.TEXT_PY_RULE_11;
            fj_arr.push(Text.TEXT_PY_RULE_25.format([RoomMgr.Instance()._Rule.usercountlimit]));
    ...
    onEventMessage: function (event, data) {
        ...
        switch (event) {
            ...
            case HallCommonEvent.HALL_NO_RECONNECT_GAME:
                cc.log("namj  响应------> HallCommonEvent.HALL_NO_RECONNECT_GAME");
            ...
            case DeskEvent.RECOVER_DESK://恢复桌子
                cc.log("namj 朋 友 recover_desk------ 开始0");
                this.recoverDesk();
                break;
    ...
    _change2D(){
        ...
        cc.find("Canvas/desk_node/jlmj_player_left_ui").getComponent('namj_player_left_ui').resetConfig();
        cc.find("Canvas/desk_node/jlmj_player_down_ui").getComponent('namj_player_down_ui').resetConfig();
        cc.find("Canvas/desk_node/jlmj_player_right_ui").getComponent('namj_player_right_ui').resetConfig();
        cc.find("Canvas/desk_node/jlmj_player_up_ui").getComponent('namj_player_up_ui').resetConfig();
    },
