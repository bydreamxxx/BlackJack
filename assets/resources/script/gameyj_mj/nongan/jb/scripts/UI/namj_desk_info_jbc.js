var baseDeskInfo = require( "namj_desk_info" );
...
var baseDeskInfoJB = cc.Class({
    ...
    onLoad:function () {
        cc.log("namj_desk_info_jbc onLoad");
        this._super();
        this.initDeskDataUI();
        this.initDeskData();
    },
    ...
    initDeskData: function() {
        var deskDataJBC= namj_desk_data_jbc.getInstance();
    ...
    start: function () {
        ...
        if( namj_desk_data_jbc.getInstance().getIsReconnect() ) {
    ...
    onDestroy: function() {
        ...
        namj_desk_data_jbc.getInstance().destroy();
        this._super();
        RoomMgr.Destroy();
    },
        ...
        if(play_list) {
            for(var i=0; i< play_list.player_ui_arr.length; ++i){
                ...
                if(headinfo && headinfo.head.player && headinfo.head.player.userId != cc.dd.user.id){
                    ...
                    headinfo.head.ready.active = true;
                }
            }
        }
    },
        ...
        if( msg.retCode !== 0 ) {
            ...
            if(play_list) {
                for(var i=0; i< play_list.player_ui_arr.length; ++i){
                    var headinfo = play_list.player_ui_arr[i];
                    if(headinfo && headinfo.head.player && headinfo.head.player.userId == cc.dd.user.id){
                        headinfo.head.ready.active = false;
                    }
                }
            }
            playerMgr.Instance().clear();
            this.playMatchAnim();
            namj_desk_data_jbc.getInstance().setIsMatching(true);
            this.tuo_guan(false);
            cc.find("Canvas/desk_node/jlmj_zhishiqi").active = false;
        }
    },
    ...
    on_room_enter:function () {
        cc.log("namj jbc desk on_room_enter--->"+(DeskData.Instance().isGameStart));
    ...
    recover_desk:function () {
        cc.log("namj jbc desk recover_desk------执行0");
        this.on_room_enter();
        // this.recoverDesk();
    },
        ...
        if( msg.retCode !== 0 ) {
            ...
            switch (msg.retCode) {
                case 1:
                    ...
                    this.popupEnterHall( str, namj_util.enterRoomList.bind(namj_util) );
                    break;
                case 2:
    ...
    tuo_guan: function (isShow) {
        if(isShow){
            ...
            if(this.menu_list){
                this.menu_list.getComponent("namj_game_menu_list").closeMenuAndOptions();
            }
    ...
    game_opening:function () {
        namj_desk_data_jbc.getInstance().setIsStart( true );
        namj_desk_data_jbc.getInstance().setIsMatching( false );
    ...
    onEventMessage: function (event, data) {
        ...
        switch (event) {
            ...
            case DeskEvent.RECOVER_DESK:
                cc.log("namj jbc recover_desk------ 开始0");
                this.recover_desk();
                break;
