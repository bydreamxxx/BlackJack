...
cc.Class({
    ...
    onLoad: function () {
        cc.log('jzmj_player_down_ui onLoad');
    ...
    initModepai: function () {
        ...
        jlmj_user_shoupai.setTouchAble(true);
        jlmj_user_shoupai.zhanli(this.pai3dCfg.shoupai_zhanli_cfg['frame_' + 12], this.pai3dCfg.shoupai_zhanli_cfg['value_' + 12], null, this.pai3dCfg.shoupai_zhanli_cfg['hunpai_' + 12]);
    ...
    updateShouPai: function (player) {
        ...
        for (let i = 0; i < player.baipai_data_list.length; i++) {
            if (player.baipai_data_list[i].type == BaipaiType.ANGANG) {
                hasAn = true;
                break;
            }
        }
        ...
        if (!DeskData.isFenZhang && (player.shoupai.length == 4 || player.shoupai.length == 5) &&
            player.state != PlayerState.TINGPAI && player.state != PlayerState.HUPAI && !hasAn) {
        ...
        if (player.hasMoPai()) {//说明手牌中是包含
            ...
            if (player.state != PlayerState.HUPAI && player.state != PlayerState.TINGPAI) {
                cc.find("Canvas/game_menu_list").getComponent("jzmj_game_menu_list").setMenus(player);
            }
        } else {
            this.modepai.node.active = false;
        }
    ...
    updateSelectedPai: function (player) {
        ...
        for (var i = 0; i < this.shouPai.length; ++i) {
            if (shoupai_visible_cfg[shouPaiLen] && shoupai_visible_cfg[shouPaiLen].indexOf(i) != -1) {
                ...
                this.shouPai[i].setHunPai(DeskData.Instance().isHunPai(player.shoupai[count]), true);
                count++;
            } else {
                this.shouPai[i].node.active = false;
            }
    ...
    setShouPaiAppearance: function (idx, state, cfg_view_idx) {
        ...
        switch (state) {
            case PlayerState.TINGPAI:
                this.shouPai[idx].zhanli(this.pai3dCfg.shoupai_zhanli_cfg['frame_' + cfg_view_idx], this.pai3dCfg.shoupai_zhanli_cfg['value_' + cfg_view_idx], null, this.pai3dCfg.shoupai_zhanli_cfg['hunpai_' + cfg_view_idx]);
                this.shouPai[idx].setTouchAble(false);
                break;
            case PlayerState.HUPAI:
                this.shouPai[idx].kaipai(this.pai3dCfg.shoupai_kaipai_cfg['frame_' + cfg_view_idx], this.pai3dCfg.shoupai_kaipai_cfg['value_' + cfg_view_idx], this.pai3dCfg.shoupai_kaipai_cfg, null, this.pai3dCfg.shoupai_kaipai_cfg['hunpai_' + cfg_view_idx]);
                this.shouPai[idx].setTouchAble(false);
    ...
    setMoPaiAppearance: function (idx, state, cfg_view_idx) {
        ...
        switch (state) {
            case PlayerState.HUPAI:
                this.modepai.kaipai(this.pai3dCfg.shoupai_kaipai_cfg['frame_' + cfg_view_idx], this.pai3dCfg.shoupai_kaipai_cfg['value_' + cfg_view_idx], this.pai3dCfg.shoupai_kaipai_cfg, null, this.pai3dCfg.shoupai_kaipai_cfg['hunpai_' + cfg_view_idx]);
                this.modepai.setTouchAble(false);
                this.modepai.mask.active = false;
                break;
            default:
                this.modepai.zhanli(this.pai3dCfg.shoupai_zhanli_cfg['frame_' + cfg_view_idx], this.pai3dCfg.shoupai_zhanli_cfg['value_' + cfg_view_idx], null, this.pai3dCfg.shoupai_zhanli_cfg['hunpai_' + cfg_view_idx]);
    ...
    touchStart: function (event) {
        ...
        if (pai_touched) {
            ...
            if (this.touchCardMode == TouchCardMode.DA_TING_PAI) {
                this.closeJiaoInfo();
                cc.log("jzmj touchStart openJiaoInfo");
    ...
    touchEnd: function (event) {
        ...
        if (!this.pai_touched) {
            ...
            if (this.touchCardMode == TouchCardMode.TING_PAI) {
                this.closeJiaoInfo();
                cc.log("jzmj touchEnd openJiaoInfo");
                this.openJiaoInfo(this.pai_touched.cardId);
            }
        }
            ...
            if (this.yidong_pai.node.y > -200 * this._node_scale_y) {
                ...
                if (UserPlayer.isTempBaoTing) {
                    ...
                    if (UserPlayer.hasMoPai()) {
                        // if(this.touchCardMode == TouchCardMode.DA_TING_PAI){
                        //     cc.log("jzmj touchEnd sendOutCard");
                        //     UserPlayer.setJiaoInfo(this.yidong_pai.cardId);
                        // }
    ...
    biaojiPai: function (selectId) {
        ...
        for (var i in userList) {
            var com = userList[i].getComponent('jzmj_player_down_ui');
    ...
    sendTingPai: function (playCardId, tingType) {
        ...
        var msg = new cc.pb.jinzhoumj.jinzhou_req_tingpai_out_card();
        msg.setCardid(playCardId);
        msg.setTingtype(tingType);

        cc.gateNet.Instance().sendMsg(cc.netCmd.jinzhoumj.cmd_jinzhou_req_tingpai_out_card, msg, "cmd_jinzhou_req_tingpai_out_card");
    ...
    sendOutCard: function (cardID) {
        ...
        this.quxiaoBiaoji();
        var msg = new cc.pb.jinzhoumj.jinzhou_req_game_send_out_card();
        msg.setCardid(cardID);
        cc.gateNet.Instance().sendMsg(cc.netCmd.jinzhoumj.cmd_jinzhou_req_game_send_out_card, msg, "jinzhou_req_game_send_out_card");
        cc.dd.NetWaitUtil.net_wait_start();
    },
    ...
    zhanli: function () {
        this.shouPai.forEach(function (jlmj_pai, idx) {
            if (jlmj_pai) {
                jlmj_pai.zhanli(this.pai3dCfg.shoupai_zhanli_cfg['frame_' + idx], this.pai3dCfg.shoupai_zhanli_cfg['value_' + idx], null, this.pai3dCfg.shoupai_zhanli_cfg['hunpai_' + idx]);
                jlmj_pai.setTouchAble(true);
            }
        }, this);
        var _idx = 13;
        this.modepai.zhanli(this.pai3dCfg.shoupai_zhanli_cfg['frame_' + _idx], this.pai3dCfg.shoupai_zhanli_cfg['value_' + _idx], null, this.pai3dCfg.shoupai_zhanli_cfg['hunpai_' + _idx]);
    ...
    dapai: function (userplayer, chupai_idx_in_shoupai) {
        ...
        jlmj_pai.kaipai(this.getDaPaiCfg()['frame_' + cur_idx], this.getDaPaiCfg()['value_' + cur_idx], this.pai3dCfg.dapai_cfg, null, this.getDaPaiCfg()['hunpai_' + cur_idx]);
    ...
    ting: function (player) {
        ...
        this.closeJiaoInfo();
        var desk_info = cc.find('Canvas/desk_info').getComponent('jzmj_desk_info');
