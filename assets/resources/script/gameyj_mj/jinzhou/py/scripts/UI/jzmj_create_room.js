...
cc.Class({
        ...
        switch (data) {
            case "rs1":
                ...
                var conf = cost_room_cards_config.getItemList(function (item) {
                    return item.id == cc.dd.Define.GameType.JZMJ_FRIEND && item.player_num == 4;
                }.bind(this));
            ...
            case "rs2":
                ...
                var conf = cost_room_cards_config.getItemList(function (item) {
                    return item.id == cc.dd.Define.GameType.JZMJ_FRIEND && item.player_num == 3;
                }.bind(this));
            ...
            case "rs3":
                ...
                var conf = cost_room_cards_config.getItemList(function (item) {
                    return item.id == cc.dd.Define.GameType.JZMJ_FRIEND && item.player_num == 2;
                }.bind(this));
    ...
    onCreate: function (event, custom) {
        /************************游戏统计 start************************/
        cc.dd.Utils.sendClientAction(cc.dd.clientAction.JZMJ_FRIEND, cc.dd.clientAction.T_HALL.CREATE_ROOM);
        /************************游戏统计   end************************/
    ...
    sendCreateRoom: function (data, isProxy) {
        ...
        cc.sys.localStorage.setItem(cc.dd.user.id + '_jzmj_create', JSON.stringify(data));
    ...
    initData: function () {
        ...
        this.m_objRuleIndex = {
            ...
            fd: 1,
            // 玩法
            wf: [true, true, true, true, false, false, false],
            // 防作弊
            fzb: true,
            //俱乐部限定
            club: false,
        ...
        let result = cc.sys.localStorage.getItem(cc.dd.user.id + '_jzmj_create');
    ...
    getRules(data, isProxy){
        ...
        var conf = cost_room_cards_config.getItemList(function (item) {
            return item.id == cc.dd.Define.GameType.JZMJ_FRIEND && item.player_num == this.m_objRuleIndex.rs;
        }.bind(this));

        qsOrJs = conf[this.m_objRuleIndex.qs].circle_num

        var gameReq = new cc.pb.room_mgr.msg_create_game_req();
        var pbData = new cc.pb.room_mgr.jinzhou_req_createdesk_new();
        ...
        pbData.setIsdaihun(data.wf[0]);
        pbData.setIsqionghujiafan(data.wf[1]);
        pbData.setIsqidui(data.wf[2]);
        pbData.setIsqingyise(data.wf[3]);
        pbData.setIs3qing(data.wf[4]);
        pbData.setIs4qing(data.wf[5]);
        pbData.setIsjiehu(data.wf[6]);

        pbData.setGps(data.fzb);
        pbData.setIsyuyin(data.yy);

        commonGame.setGameType(71);
        if (isProxy) {
        ...
        rule.setMjJinzhouRuleNew(pbData);
    ...
    setRoomRule(rule){
        ...
        let _rule = rule.rule.mjJinzhouRuleNew;

        let qsOrJs = 0;
        var conf = cost_room_cards_config.getItemList(function (item) {
            return item.id == cc.dd.Define.GameType.JZMJ_FRIEND && item.player_num == _rule.usercountlimit;
        }.bind(this));
        ...
        this.m_objRuleIndex = {
            ...
            // 玩法
            wf: [_rule.isdaihun, _rule.isqionghujiafan, _rule.isqidui, _rule.isqingyise, _rule.is3qing, _rule.is4qing, _rule.isjiehu],
            // 防作弊
            fzb: _rule.gps,
            //俱乐部限定
            club: _rule.gps,
