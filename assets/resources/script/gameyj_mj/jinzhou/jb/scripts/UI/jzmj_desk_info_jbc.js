var baseDeskInfo = require( "jzmj_desk_info" );
...
var baseDeskInfoJB = cc.Class({
    ...
    onLoad:function () {
        cc.log("jzmj_desk_info_jbc onLoad");
        this._super();
        this.initDeskDataUI();
        this.initDeskData();
    },
    ...
    initDeskData: function() {
        var deskDataJBC= jzmj_desk_data_jbc.getInstance();
    ...
    start: function () {
        if( jzmj_desk_data_jbc.getInstance().getIsReconnect() ) {
    ...
    onDestroy: function() {
        ...
        jzmj_desk_data_jbc.getInstance().destroy();
        this._super();
        //RoomMgr.Destroy();
    },
    ...
    on_room_replace:function (msg) {
        if( msg.retCode !== 0 ) {
            ...
            this.playMatchAnim();
            jzmj_desk_data_jbc.getInstance().setIsMatching(true);
            this.tuo_guan(false);
            cc.find("Canvas/desk_node/jlmj_zhishiqi").active = false;
        }
    },
    ...
    on_room_enter:function () {
        cc.log("jzmj jbc desk on_room_enter--->"+(DeskData.Instance().isGameStart));
    ...
    recover_desk:function () {
        cc.log("jzmj jbc desk recover_desk------执行0");
        this.on_room_enter();
        //this.recoverDesk();
    },
        ...
        if( msg.retCode !== 0 ) {
            ...
            switch (msg.retCode) {
                case 1:
                    ...
                    this.popupEnterHall( str, jzmj_util.enterRoomList.bind(jzmj_util) );
                    break;
                case 2:
    ...
    tuo_guan: function (isShow) {
        if(isShow){
            ...
            if(this.menu_list){
                this.menu_list.getComponent("jzmj_game_menu_list").closeMenuAndOptions();
            }
    ...
    onEventMessage: function (event, data) {
        ...
        switch (event) {
            ...
            case DeskEvent.RECOVER_DESK:
                cc.log("jzmj jbc recover_desk------ 开始0");
                this.recover_desk();
                break;
