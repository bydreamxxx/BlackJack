...
var shmj_Control_Combination_ui = require( "shmj_Control_Combination_ui" );
...
var menu_cfg = [
    ...
    { txtFrame:11,   bgFrame:2,    desc:"过" },
    { txtFrame:12,   bgFrame:1,    desc:"碰听" },
    { txtFrame:13,   bgFrame:1,    desc:"杠听" },
    { txtFrame:14,   bgFrame:1,    desc:"对倒算夹" },
    { txtFrame:15,   bgFrame:1,    desc:"亮掌宝" },
    { txtFrame:16,   bgFrame:1,    desc:"闷听" },
];
...
var sh_GameMenu = cc.Class({
     ...
     */
    guo: function () {
        var msg = new cc.pb.suihuamj.suihua_req_game_act_guo();
        msg.setUserid(dd.user.id);
        msg.setGuotype(this.getGuoType());
        cc.gateNet.Instance().sendMsg(cc.netCmd.suihuamj.cmd_suihua_req_game_act_guo,msg,"suihua_req_game_act_guo");
    },
    /**
     * 过牌
     */
    ...
    chi: function () {
        ...
        if(UserPlayer.chi_options.length == 1){
            //只有一个吃，直接吃
            var msg = new cc.pb.suihuamj.suihua_req_chi();
            ...
            cc.gateNet.Instance().sendMsg(cc.netCmd.suihuamj.cmd_suihua_req_chi,msg,"suihua_req_chi");
    ...
    chiting: function () {
        ...
        for(let i = 0; i < UserPlayer.chi_jiaoInfo_list.length; i++){
            if(!cc.dd._.isUndefined(UserPlayer.chi_jiaoInfo_list[i]) && !cc.dd._.isUndefined(UserPlayer.chi_options[i])){
                list[i] = UserPlayer.chi_options[i];
                list[i].chiidx = i;
            }
        }

        for(let i = 0; i < UserPlayer.chi_tuidaoInfo_list.length; i++){
            if(!cc.dd._.isUndefined(UserPlayer.chi_tuidaoInfo_list[i]) && !cc.dd._.isUndefined(UserPlayer.chi_options[i])){
                list[i] = UserPlayer.chi_options[i];
                list[i].chiidx = i;
            }
        }

        UserPlayer.chi_options = list;
        let realLength = 0;
        let realIdx = -1;
        ...
        if(realLength == 1){
            UserPlayer.chitingPaiId = UserPlayer.chi_options[realIdx];
            UserPlayer.chitingPaiId.idx = 0;
            UserPlayer.jiaoInfo_list = UserPlayer.chi_jiaoInfo_list[realIdx];
            UserPlayer.tuidaoInfo_list = UserPlayer.chi_tuidaoInfo_list[realIdx];
    ...
    gangting: function () {
        ...
        // var msg = new cc.pb.songyuanmajiang.sy_req_gangting();
        // var cards = [];
        ...
        // cc.gateNet.Instance().sendMsg(cc.netCmd.songyuanmajiang.cmd_sy_req_gangting, msg, "cmd_sy_req_gangting");
    },
    ...
    xiaosaTingCallBack:function (fun) {
        ...
        let func = ()=>{
            ...
            /*if(UserPlayer.isTempXiaosa == false) {
                ...
                menuState.canxiaosa = false;
                menuState.fun = fun?fun:null;
                cc.find("Canvas/game_menu_list").getComponent("shmj_game_menu_list").setPlayerMenus(menuState);
            }*/
        ...
        if(UserPlayer.isTempChiTing || UserPlayer.isTempPengTing){
            if(UserPlayer.isDuiDaoSuanJia()){
                var menu_list = cc.find("Canvas/game_menu_list");
                if (menu_list) {
                    menu_list.getComponent("shmj_game_menu_list").showTuidao(cc.dd._.isArray(UserPlayer.jiaoInfo_list) && UserPlayer.jiaoInfo_list.length != 0, func);
                }
            }else{
                func();
            }
    ...
    chiTingCallback:function (chiList) {
        if(UserPlayer.isTempChiTing){
            ...
            UserPlayer.jiaoInfo_list = UserPlayer.chi_jiaoInfo_list[chiList.chiidx];
            UserPlayer.tuidaoInfo_list = UserPlayer.chi_tuidaoInfo_list[chiList.chiidx];
     ...
     */
    peng: function () {
        var msg = new cc.pb.suihuamj.suihua_req_game_act_peng();
        msg.setUserid(dd.user.id);

        var pengCard = new cc.pb.jilinmajiang.CardInfo();
        pengCard.setId(DeskData.Instance().last_chupai_id);
        cc.gateNet.Instance().sendMsg(cc.netCmd.suihuamj.cmd_suihua_req_game_act_peng,msg,"suihua_req_game_act_peng");
        cc.dd.NetWaitUtil.net_wait_start();
    },
    ...
    onSeclectedGang: function ( selectedList ) {
        ...
        shmj_Control_Combination_ui.Instance().onCloseCombinationUi();
    },
    ...
    gang: function () {
        ...
        if( gangOption.cardList.length == 1 ) {
            ...
            if( isGangTing ) {
                ...
                shmj_Control_Combination_ui.Instance().getAllCombinationList( gangOptionList1, gangOptionList,true);

            } else {
                shmj_Control_Combination_ui.Instance().getAllCombinationList( gangOption.cardList, gangOptionList,false);
            }
    ...
    ting: function () {
        ...
        if(UserPlayer.isDuiDaoSuanJia()){
            var menu_list = cc.find("Canvas/game_menu_list");
            if (menu_list) {
                menu_list.getComponent("shmj_game_menu_list").showTuidao(UserPlayer.jiaoInfo_list.length != 0, func);
            }
        }else{
            func();
        }
     ...
     */
    hupai: function () {
        var msg = new cc.pb.suihuamj.suihua_req_game_act_hu();
        msg.setUserid(dd.user.id);
        cc.gateNet.Instance().sendMsg(cc.netCmd.suihuamj.cmd_suihua_req_game_act_hu,msg,"suihua_req_game_act_hu");
    },
    ...
    okBtnCallBack:function (data) {
        ...
        if(data.type == menu_type.TING){//听牌
            ...
            msg.setTingtype( 1 );
            cc.gateNet.Instance().sendMsg(cc.netCmd.suihuamj.cmd_suihua_req_tingpai_out_card,msg,"suihua_req_tingpai_out_card");
        }else{//杠牌
    ...
    sendGangPai:function (gang_option) {
        var msg = new cc.pb.suihuamj.suihua_req_game_act_gang();
        ...
        msg.setUserid(dd.user.id);
        msg.setChoosecardsList(cards);
        cc.gateNet.Instance().sendMsg(cc.netCmd.suihuamj.cmd_suihua_req_game_act_gang,msg,"suihua_req_game_act_gang");
        cc.dd.NetWaitUtil.net_wait_start();
    },
    ...
    duidaosuanjia:function () {
        let list = []
        let temp_id = [];
        var jiaoInfo = UserPlayer.getTuiDaoSuanJia();
        if (!jiaoInfo) {
            return;
        }

        for(let i = 0; i < jiaoInfo.length; i++){
            if(temp_id.indexOf(jiaoInfo[i].id) == -1){
                temp_id.push(jiaoInfo[i].id);
                list.push([jiaoInfo[i].id]);
            }
        }

        DeskED.notifyEvent(DeskEvent.OPEN_BAO_OPTION,[list, (ddsjID)=>{
            if(cc.dd._.isArray(ddsjID)){
                UserPlayer.canhu = 0;

                DeskED.notifyEvent(DeskEvent.CLOSE_MENU, []);
                UserPlayer.setTuidDaoSuanJia(ddsjID[0]);

                UserPlayer.isTempBaoTing = true;
                let arr = [];
                let list = UserPlayer.jiaoInfo_list;
                for(var i=0; i<list.length; ++i ){
                    arr.push(list[i].out_id);
                }
    ...
    liangzhangbao:function(){
        let list = UserPlayer.getPair();
        if(list.length == 0){
            return;
        }

        DeskED.notifyEvent(DeskEvent.OPEN_BAO_OPTION,[list, (liangID)=>{
            if(cc.dd._.isArray(liangID)){
                ...
                DeskED.notifyEvent(DeskEvent.CLOSE_MENU, []);
                PlayerED.notifyEvent(PlayerEvent.CLEA_SELECT_CARD,[UserPlayer]); //清除选择数组
                var msg = new cc.pb.suihuamj.suihua_req_liangzhang();
                msg.setUserid( cc.dd.user.id );
                msg.setCardsList( card );
                cc.gateNet.Instance().sendMsg(cc.netCmd.suihuamj.cmd_suihua_req_liangzhang,msg,"suihua_req_liangzhang", true);
            }
        }]);
    }
});
