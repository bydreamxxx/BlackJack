...
var UserPlayer = require("shmj_userPlayer_data");
...
var baseDeskInfo = cc.Class({
    ...
    properties: {
        ...
        spf_yu_an: cc.SpriteFrame,
        spf_yu_an_2d: cc.SpriteFrame,
        prefab_fenzhang:cc.Prefab,//分张预制体
    },

    onLoad :function() {
        cc.log("shmj_desk_info onLoad");
        this.logo = cc.find("Canvas/desk_node/c-logo-suihua").getComponent(cc.Sprite);
    ...
    initLocalData:function() {
        ...
        //桌布
        var json = cc.sys.localStorage.getItem('shmj_zhuobu_set_' + cc.dd.user.id);
        if (json) {
            var sprite = null;
            ...
            if (sprite) {
                ...
                cc.sys.localStorage.setItem('shmj_zhuobu_set_' + cc.dd.user.id, this.zhuomianImg[0]._name);
            }
        }
        else {
            this.deskImage.spriteFrame = this.zhuomianImg[0];
    ...
    initGuiZeInfo:function () {
        ...
        var cc_txt = dd.Text.TEXT_PY_RULE_73;
        cur_rule.roomId = RoomMgr.Instance().roomId;

        let fengdingTxt = [Text.TEXT_PY_RULE_54, Text.TEXT_PY_RULE_39.format('64'), Text.TEXT_PY_RULE_39.format('128'), Text.TEXT_PY_RULE_39.format('256')]

        let _text1 = cc_txt.format([RoomMgr.Instance()._Rule.usercountlimit]);
        let _text2 = fengdingTxt[RoomMgr.Instance()._Rule.fengdingtype];
        // let _text3 = RoomMgr.Instance()._Rule.isbujiabuhu?       Text.TEXT_PY_RULE_52:'';
        let _text4 = RoomMgr.Instance()._Rule.isduidaosuanjia?      Text.TEXT_PY_RULE_74:'';
        let _text5 = RoomMgr.Instance()._Rule.is3or7jia?     Text.TEXT_PY_RULE_75:'';
        // let _text6 = RoomMgr.Instance()._Rule.ishongzhongfei?       Text.TEXT_PY_RULE_76:'';
        let _text7 = RoomMgr.Instance()._Rule.isloubao?     Text.TEXT_PY_RULE_77:'';
        let _text8 = RoomMgr.Instance()._Rule.isshanggunbao?   Text.TEXT_PY_RULE_83:'';
        // let _text9 = RoomMgr.Instance()._Rule.isanbao?   Text.TEXT_PY_RULE_85: Text.TEXT_PY_RULE_86;
        let _text10 = RoomMgr.Instance()._Rule.iskaizhapai?   Text.TEXT_PY_RULE_78:'';
        let _text11 = RoomMgr.Instance()._Rule.isgangjiafan?   Text.TEXT_PY_RULE_79:'';
        let _text12 = RoomMgr.Instance()._Rule.isduan19?   Text.TEXT_PY_RULE_84:'';
        let _text13 = RoomMgr.Instance()._Rule.isliangzhangbao? Text.TEXT_PY_RULE_80:'';
        let _text14 = RoomMgr.Instance()._Rule.isqidui? Text.TEXT_PY_RULE_27:'';
        let _text15 = RoomMgr.Instance()._Rule.isguadafeng? Text.TEXT_PY_RULE_81:'';
        let _text16 = RoomMgr.Instance()._Rule.ismoyu? Text.TEXT_PY_RULE_82:'';
        let _text17 = RoomMgr.Instance()._Rule.ismenting? Text.TEXT_PY_RULE_88:'';
        let _text18 = RoomMgr.Instance()._Rule.isqingyise? Text.TEXT_PY_RULE_29:'';
        let _text19 = RoomMgr.Instance()._Rule.ishaoqi? Text.TEXT_PY_RULE_169:'';
        let _text20 = RoomMgr.Instance()._Rule.isqiduifeng? Text.TEXT_PY_RULE_170:'';
        let _text21 = RoomMgr.Instance()._Rule.meting? Text.TEXT_PY_RULE_171:'';
        ...
        this.gz_arr_info.push({str:_text16,nodetype:1});
    ...
    initZhiNan:function () {
        if(this.zhinan.active && DeskData.Instance().zhiNan_site != null){
            ...
            this.zhinan.getComponent("shmj_zhinan_ui").initsetZhiNan(site);
    ...
    clear: function (data) {
        ...
        let menu_list = cc.find("Canvas/game_menu_list");
        if(menu_list){
            menu_list.getComponent("shmj_game_menu_list").closeBaoOptions();
        }
    ...
    recoverDesk: function () {
        cc.log("shmj 朋 友 recover_desk------ 接收1");
        cc.log("------恢复桌子------py");
        this.updateDesk();
        DeskData.Instance().enterSceneRecoverDesk(this.sendReadyOK.bind(this));
        if (this.fenzhang) {
    ...
    sendReadyOK: function () {
        ...
        var msg = new cc.pb.suihuamj.suihua_req_reloading_ok();
        cc.gateNet.Instance().sendMsg(cc.netCmd.suihuamj.cmd_suihua_req_reloading_ok,msg,"suihua_req_reloading_ok");
        this.initGuiZeInfo();
    },
    ...
    jiesuan:function (data) {
        ...
        this.jiesuan_TimeID = setTimeout(function () {
            ...
            if (cc.director.getScene().name != 'shmj_py_game' ||
                cc.director.getScene().name != 'shmj_jbc_game') {
                cc.dd.UIMgr.destroyUI(jlmj_prefab.JLMJ_JIAOPAI_INFO);
    ...
    leave_tips: function (data) {
        ...
        if(DeskData.Instance().isFriend()){//朋友场
            ...
            }else if(shmj_desk_data_jbc.getInstance().getIsMatching() || DeskData.Instance().player_read_gamne){//取消匹配状态
                txt = Text.TEXT_LEAVE_ROOM_3;
                callfunc = this.jbc_matching_leave_room_req;
            }else if( shmj_desk_data_jbc.getInstance().getIsStart()){//游戏中退出游戏
    ...
    py_dissolve_room_req:function() {
        ...
        var msg = new cc.pb.suihuamj.suihua_req_sponsor_dissolve_room();
        msg.setSponsorid(dd.user.id);
        cc.gateNet.Instance().sendMsg(cc.netCmd.suihuamj.cmd_suihua_req_sponsor_dissolve_room,msg,"suihua_req_sponsor_dissolve_room");
    },
        ...
        if( msg.retCode !== 0 ) {
            ...
            switch (msg.retCode) {
                case 1:
                    ...
                    this.popupEnterHall( str, shmj_util.enterHall.bind(shmj_util) );
                    break;
                case 2:
    ...
    py_ready:function () {
        ...
        var msg = new cc.pb.suihuamj.suihua_req_ready();
        cc.gateNet.Instance().sendMsg(cc.netCmd.suihuamj.cmd_suihua_req_ready,msg,"suihua_req_ready");
    },
    ...
    update_bao_pai: function () {
        ...
        if(baoPaiValue == -2){
            cc.find("Canvas/desk_node/baopai").active = false;
            this.baopai.node.active = false;
            if(RoomMgr.Instance()._Rule.ismoyu && cc.dd.mj_game_start){
                this.baopai = cc.find("Canvas/desk_node/baopai").getComponent("jlmj_pai");
                this.baopai.node.active = true;
                this.baopai.value.node.active = false;
                let use2D =DeskData.Instance().getIs2D();
                if(use2D){
                    this.baopai.frame.spriteFrame = this.spf_yu_an_2d;
                }else{
                    this.baopai.frame.spriteFrame = this.spf_yu_an;
                }
            }
            ...
            if(RoomMgr.Instance()._Rule.ismoyu){
                if(use2D){
                    this.baopai.frame.spriteFrame = this.spf_yu_an_2d;
                }else{
                    this.baopai.frame.spriteFrame = this.spf_yu_an;
    ...
    getTextByWanFa: function() {
        ...
        this.gz_arr_box = [];
        this.gz_arr_box.push(cc_txt.format([RoomMgr.Instance()._Rule.usercountlimit]));
        RoomMgr.Instance()._Rule.fengdingtype == 0 ? this.gz_arr_box.push(Text.TEXT_PY_RULE_54) : this.gz_arr_box.push(Text.TEXT_PY_RULE_55.format([RoomMgr.Instance()._Rule.fengdingtype+7]));
        // RoomMgr.Instance()._Rule.isbujiabuhu?       this.gz_arr_box.push(Text.TEXT_PY_RULE_52):'';
        RoomMgr.Instance()._Rule.isduidaosuanjia?      this.gz_arr_box.push(Text.TEXT_PY_RULE_74):'';
        RoomMgr.Instance()._Rule.is3or7jia?     this.gz_arr_box.push(Text.TEXT_PY_RULE_75):'';
        // RoomMgr.Instance()._Rule.ishongzhongfei?       this.gz_arr_box.push(Text.TEXT_PY_RULE_76):'';
        RoomMgr.Instance()._Rule.isloubao?     this.gz_arr_box.push(Text.TEXT_PY_RULE_77):'';
        RoomMgr.Instance()._Rule.isshanggunbao?   this.gz_arr_box.push(Text.TEXT_PY_RULE_83):'';
        // RoomMgr.Instance()._Rule.isanbao?   this.gz_arr_box.push(Text.TEXT_PY_RULE_85):this.gz_arr_box.push(Text.TEXT_PY_RULE_86);
        RoomMgr.Instance()._Rule.iskaizhapai?    this.gz_arr_box.push(Text.TEXT_PY_RULE_78):'';
        RoomMgr.Instance()._Rule.isgangjiafan?   this.gz_arr_box.push(Text.TEXT_PY_RULE_79):'';
        RoomMgr.Instance()._Rule.isduan19?    this.gz_arr_box.push(Text.TEXT_PY_RULE_84):'';
        RoomMgr.Instance()._Rule.isliangzhangbao? this.gz_arr_box.push(Text.TEXT_PY_RULE_80):'';
        RoomMgr.Instance()._Rule.isqidui? this.gz_arr_box.push(Text.TEXT_PY_RULE_27):'';
        RoomMgr.Instance()._Rule.isguadafeng? this.gz_arr_box.push(Text.TEXT_PY_RULE_81):'';
        RoomMgr.Instance()._Rule.ismoyu? this.gz_arr_box.push(Text.TEXT_PY_RULE_82):'';
        RoomMgr.Instance()._Rule.ismenting? this.gz_arr_box.push(Text.TEXT_PY_RULE_88):'';
    ...
    onInvaite: function (event, custom) {
        ...
        jlmj_audio_mgr.com_btn_click();
        var cc_txt = dd.Text.TEXT_PY_RULE_73;
        var title = cc_txt.format(playerMgr.Instance().playerList.length);
        title += " "+ "房间号:" + (RoomMgr.Instance().roomId || 888888);
        var content = '共'+RoomMgr.Instance()._Rule.boardscout+''+(RoomMgr.Instance()._Rule.usercountlimit==4?'圈':'局');
        ...
        if (cc.sys.isNative) {
            ...
            let info = {
                ...
                jushutitle: RoomMgr.Instance()._Rule.usercountlimit==4 ? '圈数' : '局数',//局\圈标题，默认局数
    ...
    on_show_result_view: function () {
        ...
        cc.dd.UIMgr.openUI(jlmj_prefab.MJ_ZHANJITONGJI, function(ui) {
            var fj_arr = [];
            var juquan_txt = RoomMgr.Instance()._Rule.usercountlimit!=4?Text.TEXT_PY_RULE_10:Text.TEXT_PY_RULE_11;
            fj_arr.push(Text.TEXT_PY_RULE_73.format([RoomMgr.Instance()._Rule.usercountlimit]));
    ...
    onEventMessage: function (event, data) {
        ...
        switch (event) {
            ...
            case HallCommonEvent.HALL_NO_RECONNECT_GAME:
                cc.log("shmj  响应------> HallCommonEvent.HALL_NO_RECONNECT_GAME");
            ...
            case DeskEvent.RECOVER_DESK://恢复桌子
                cc.log("shmj 朋 友 recover_desk------ 开始0");
                this.recoverDesk();
                break;
    ...
    _change2D(){
        ...
        cc.find("Canvas/desk_node/jlmj_player_left_ui").getComponent('shmj_player_left_ui').resetConfig();
        cc.find("Canvas/desk_node/jlmj_player_down_ui").getComponent('shmj_player_down_ui').resetConfig();
        cc.find("Canvas/desk_node/jlmj_player_right_ui").getComponent('shmj_player_right_ui').resetConfig();
        cc.find("Canvas/desk_node/jlmj_player_up_ui").getComponent('shmj_player_up_ui').resetConfig();
    },
