...
cc.Class({
    extends: require('shmj_player_base_ui'),
    ...
    onLoad: function () {
        cc.log('shmj_player_down_ui onLoad');
    ...
    initModepai: function () {
        ...
        jlmj_user_shoupai.setTouchAble(true);
        jlmj_user_shoupai.zhanli(this.pai3dCfg.shoupai_zhanli_cfg['frame_' + 12], this.pai3dCfg.shoupai_zhanli_cfg['value_' + 12], this.pai3dCfg.shoupai_zhanli_cfg['liangzhang_' + 12]);
    ...
    updateShouPai: function (player) {
        ...
        if (!DeskData.Instance().isHu && !DeskData.isFenZhang &&
        ...
        if (player.hasMoPai()) {//说明手牌中是包含
            ...
            if (player.state != PlayerState.HUPAI && player.state != PlayerState.TINGPAI) {
                cc.find("Canvas/game_menu_list").getComponent("shmj_game_menu_list").setMenus(player);
            }
        } else {
            this.modepai.node.active = false;
        }
    ...
    setShouPaiAppearance: function (idx, state, cfg_view_idx) {
        ...
        switch (state) {
            case PlayerState.TINGPAI:
                this.shouPai[idx].zhanli(this.pai3dCfg.shoupai_zhanli_cfg['frame_' + cfg_view_idx], this.pai3dCfg.shoupai_zhanli_cfg['value_' + cfg_view_idx], this.pai3dCfg.shoupai_zhanli_cfg['liangzhang_' + cfg_view_idx], null, this.pai3dCfg.shoupai_zhanli_cfg['tuidao_' + cfg_view_idx]);
                this.shouPai[idx].setTouchAble(false);
                break;
            case PlayerState.HUPAI:
                this.shouPai[idx].kaipai(this.pai3dCfg.shoupai_kaipai_cfg['frame_' + cfg_view_idx], this.pai3dCfg.shoupai_kaipai_cfg['value_' + cfg_view_idx], this.pai3dCfg.shoupai_kaipai_cfg, this.pai3dCfg.shoupai_kaipai_cfg['liangzhang_' + cfg_view_idx], null, this.pai3dCfg.shoupai_kaipai_cfg['tuidao_' + cfg_view_idx]);
                this.shouPai[idx].setTouchAble(false);
    ...
    setMoPaiAppearance: function (idx, state, cfg_view_idx) {
        ...
        switch (state) {
            case PlayerState.HUPAI:
                this.modepai.kaipai(this.pai3dCfg.shoupai_kaipai_cfg['frame_' + cfg_view_idx], this.pai3dCfg.shoupai_kaipai_cfg['value_' + cfg_view_idx], this.pai3dCfg.shoupai_kaipai_cfg, this.pai3dCfg.shoupai_kaipai_cfg['liangzhang_' + cfg_view_idx]);
                this.modepai.setTouchAble(false);
                this.modepai.mask.active = false;
                break;
            default:
                this.modepai.zhanli(this.pai3dCfg.shoupai_zhanli_cfg['frame_' + cfg_view_idx], this.pai3dCfg.shoupai_zhanli_cfg['value_' + cfg_view_idx], this.pai3dCfg.shoupai_zhanli_cfg['liangzhang_' + cfg_view_idx]);
    ...
    touchStart: function (event) {
        if (DeskData.Instance().isHu || (DeskData.Instance().isFriend() && UserPlayer.canhu)) {
            return;
        }
        ...
        if (pai_touched) {
            cc.log("shmj touchpai ", pai_touched.cardId);
            ...
            if (this.touchCardMode == TouchCardMode.DA_TING_PAI) {
                this.closeJiaoInfo();
                cc.log("shmj touchStart openJiaoInfo");
    ...
    touchEnd: function (event) {
        ...
        cc.log("shmj touchpai end", this.pai_touched.cardId);
        this.pai_touched = null;
        this.isCanMove = null;
        ...
        if (this.yidong_pai.node.active) {
            if (this.yidong_pai.node.y > -200 * this._node_scale_y) {
                ...
                if (this.touchCardMode == 3) {
                    ...
                    playerED.notifyEvent(PlayerEvent.SHOW_CLICK, [UserPlayer, true, arr, 3]);
                }
            }
        } else {
            var pai_touched = this.getTouchPai(event);
            ...
            if (UserPlayer.isTempBaoTing) {
                ...
                cc.log("能否出牌: ", UserPlayer.hasMoPai(), " ", UserPlayer.waitTing);
                if (UserPlayer.hasMoPai()) {
    ...
    touchCancel: function (event) {
        if (this.pai_touched) {
            cc.log("shmj touchpai cancel ", this.pai_touched.cardId);
            this.pai_touched.node.active = true;
        }
        this.pai_touched = null;
        this.isCanMove = null;
    ...
    biaojiPai: function (selectId) {
        ...
        for (var i in userList) {
            var com = userList[i].getComponent('shmj_player_down_ui');
    ...
    sendTingPai: function (playCardId, tingType) {
        ...
        let _duidaosuanjiaID = UserPlayer.ddsjID;
        let _liangzhangbaoID = UserPlayer.liangzhangID

        let hasDuiDaoSuanJian = cc.dd._.isNumber(_duidaosuanjiaID);
        let hasLiangZhangBao = cc.dd._.isNumber(_liangzhangbaoID);

        if (hasDuiDaoSuanJian) {
            if (!UserPlayer.curjiaoPaiInfo_list) {
                UserPlayer.setJiaoInfo(playCardId);
            }
            UserPlayer.curjiaoPaiInfo_list = UserPlayer.curjiaoPaiInfo_list.filter((jiaoinfo) => {
                return jiaoinfo.id === _duidaosuanjiaID;
            });
        }

        let _tp = 1;
        if (hasDuiDaoSuanJian && !hasLiangZhangBao) {
            _tp = 4;
        } else if (!hasDuiDaoSuanJian && hasLiangZhangBao) {
            _tp = 5;
        } else if (hasDuiDaoSuanJian && hasLiangZhangBao) {
            _tp = 6;
        }
        ...
        if (tingType == 2) {
            ...
            UserPlayer.chitingPaiId = null;
            var msg = new cc.pb.suihuamj.suihua_req_chiting();
            msg.setChicard(chi_card);
            msg.setChoosecardsList(card_option);
            msg.setOutcard(out_card);
            msg.setTingtype(_tp);
            msg.setDdsjcardid(hasDuiDaoSuanJian ? _duidaosuanjiaID : -1);
            msg.setLzbcardid(hasLiangZhangBao ? _liangzhangbaoID : -1);
            cc.gateNet.Instance().sendMsg(cc.netCmd.suihuamj.cmd_suihua_req_chiting, msg, "cmd_suihua_req_chiting");
        } else if (tingType == 3) {
            var msg = new cc.pb.suihuamj.suihua_req_pengting();
            ...
            cc.gateNet.Instance().sendMsg(cc.netCmd.suihuamj.cmd_suihua_req_pengting, msg, "cmd_suihua_req_pengting");
        } else {
            var msg = new cc.pb.suihuamj.suihua_req_tingpai_out_card();
            msg.setCardid(playCardId);
            msg.setTingtype(_tp);
            msg.setDdsjcardid(hasDuiDaoSuanJian ? _duidaosuanjiaID : -1);
            msg.setLzbcardid(hasLiangZhangBao ? _liangzhangbaoID : -1);
            cc.gateNet.Instance().sendMsg(cc.netCmd.suihuamj.cmd_suihua_req_tingpai_out_card, msg, "cmd_suihua_req_tingpai_out_card");
        }
    ...
    sendOutCard: function (cardID) {
        ...
        this.quxiaoBiaoji();
        var msg = new cc.pb.suihuamj.suihua_req_game_send_out_card();
        msg.setCardid(cardID);
        cc.gateNet.Instance().sendMsg(cc.netCmd.suihuamj.cmd_suihua_req_game_send_out_card, msg, "suihua_req_game_send_out_card");
        cc.dd.NetWaitUtil.net_wait_start();
    },
    ...
    zhanli: function () {
        this.shouPai.forEach(function (jlmj_pai, idx) {
            if (jlmj_pai) {
                jlmj_pai.zhanli(this.pai3dCfg.shoupai_zhanli_cfg['frame_' + idx], this.pai3dCfg.shoupai_zhanli_cfg['value_' + idx], this.pai3dCfg.shoupai_zhanli_cfg['liangzhang_' + idx]);
                jlmj_pai.setTouchAble(true);
            }
        }, this);
        var _idx = 13;
        this.modepai.zhanli(this.pai3dCfg.shoupai_zhanli_cfg['frame_' + _idx], this.pai3dCfg.shoupai_zhanli_cfg['value_' + _idx], this.pai3dCfg.shoupai_zhanli_cfg['liangzhang_' + _idx]);
    ...
    dapai: function (userplayer, chupai_idx_in_shoupai) {
        ...
        jlmj_pai.kaipai(this.getDaPaiCfg()['frame_' + cur_idx], this.getDaPaiCfg()['value_' + cur_idx], this.pai3dCfg.dapai_cfg, this.getDaPaiCfg()['liangzhang_' + cur_idx]);
    ...
    openPaitouch: function (canCradList) {
        if (this.touchCardMode == 3 && UserPlayer.isTempBaoTing && canCradList.length == 1) {
            var tingType = 1;
            // if (UserPlayer.isTempGang) {
            //     tingType = 2;
            // }
            ...
            UserPlayer.setJiaoInfo(canCradList[0]);
    ...
    ting: function (player) {
        ...
        this.closeJiaoInfo();
        var desk_info = cc.find('Canvas/desk_info').getComponent('shmj_desk_info');
    ...
    onEventMessage: function (event, data) {
        ...
        switch (event) {
            case PlayerEvent.SHOW_LIANG_ZHANG_BAO:
                this.shouPai.forEach(function (jlmj_pai, idx) {
                    if (jlmj_pai) {
