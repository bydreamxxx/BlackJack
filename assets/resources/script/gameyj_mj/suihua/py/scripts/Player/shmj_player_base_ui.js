...
var HuTypeDesc = require('jlmj_define').SHHuTypeDesc;
...
cc.Class({
    ...
    properties: {
        head: { default: null, type: require("shmj_playerHead"), tooltip: '头像', override: true },              //头像
    },
    ...
    play_chupai_ani: function (player, chupai_idx_in_shoupai, last_chupai_idx, cardID) {
        ...
        this.zhanshi_pai.kaipai(config['frame_' + last_chupai_idx], config['value_' + last_chupai_idx], this.pai3dCfg.dapai_cfg, config['liangzhang_' + last_chupai_idx]);
    ...
    hu: function (player, isZiMo) {
        ...
        huInfoList.push({ type: HuType.GANG_PAO_HU, ani: ['loubao01'], audio: 'loubao' });//漏宝
        huInfoList.push({ type: HuType.DUI_BAO, ani: ['baozhongbao'], audio: 'duibao' });     //对宝
        ...
        huInfoList.push({ type: HuType.DANDIAO_PIAOHU, ani: ['zimo', 'zimoXS'], audio: 'zimo' });//开牌炸
        ...
        zimoHuList.push({ type: HuType.GANG_PAO_HU, ani: ['loubao01'], audio: 'loubao' });//漏宝
    ...
    hupaiAni: function (maxHu, isZimo) {
        switch (maxHu.type) {
            ...
            case HuType.XIAO_SA:
                ...
                break;
            case HuType.DUI_BAO:
                this.playSpine(this.baozhongbao_ani, maxHu.ani, this.onHuAniFinished.bind(this));
                break;
            default:
    ...
    onEventMessage: function (event, data) {
        ...
        switch (event) {
            ...
            case PlayerEvent.CHGANG:
                ...
                break;
            case PlayerEvent.LIANGZHANG:
                this.langzhang(data);
                break;
            case PlayerEvent.TUIDAO:
                this.tuidao(data[1]);
                break;
            default:
                break;
    ...
    langzhang: function (data) {
        ...
        this.resetBaiPai(player);
        this.updateShouPai(player);
        mj_audio.playAudioBySex("liangzhang", player.sex);
    ...
    ting: function (player) {
        ...
        if ((arrBaiPaiData.length == 0 || (arrBaiPaiData.length == 1 && arrBaiPaiData[0].type == BaipaiType.LZB)) && RoomMgr.Instance()._Rule.meting) {
            // this.ani2_ani.node.active = true;
