...
var sh_PlayerData = cc.Class({
    ...
    setGameData: function (playerMsg) {
        ...
        for(let i = 0; i < playerMsg.playercard.composecardList.length; i++){
            if(playerMsg.playercard.composecardList[i].type == 10){
                playerMsg.playercard.composecardList[i].cardsList.forEach((card)=>{
                    this.pushData(this.tuidaoList, card.id);
                });
                break;
            }
        }
    ...
    liangzhangbao(pengcardList){
        ...
        this.resetBaiPaiIndex();
        PlayerED.notifyEvent(PlayerEvent.LIANGZHANG,[this,baipai_data]);
    },
    ...
    tuidao(tuidaoList){
        if(cc.dd._.isArray(tuidaoList) && tuidaoList.length > 0){
            this.tuidaoList = [];
            tuidaoList.forEach((card)=>{
                this.pushData(this.tuidaoList, card.id);
            });
            PlayerED.notifyEvent(PlayerEvent.TUIDAO,[this,this.tuidaoList]);
        }
    },
    ...
    kaipai: function(holdCardList, mopai, hupaiId, duibao) {
        ...
        for(var i in holdCardList){
            ...
            if(holdCardList[i].type == 1000){
                this.pushData(this.tuidaoList, holdCardList[i].id);
            }
        }
        this.paixu();
