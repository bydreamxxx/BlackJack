...
cc.Class({
    ...
    shezhiBtnCallBack:function () {
        ...
        cc.dd.UIMgr.openUI(jlmj_prefab.JLMJ_SHEZHI,function (ui) {
            ...
            ui.getComponent("jlmj_shezhi").initZhuobuMenu('shmj_zhuobu_set_');
        });
    ...
    tuoguanBtnCallBack:function () {
        jlmj_audio_mgr.com_btn_click();
        if(!DeskData.Instance().isFriend() && shmj_desk_data_jbc.getInstance().getIsStart()) {
    ...
    JBCCheck(){
        if(DeskData.Instance().isJBC()){
            return !shmj_desk_data_jbc.getInstance().getIsStart();
        }else{
            return false;
        }
    },
