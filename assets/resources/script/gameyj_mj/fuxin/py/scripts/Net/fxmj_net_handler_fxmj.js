...
var fx_handler = cc.Class({

    ctor: function () {
        cc.log("fxmj_net_handler_fxmj 父类");
    },
     ...
     * 开始游戏 消息
     */
    on_fx_ack_game_opening: function( msg ) {
        ...
        if(DeskData.Instance().isFriend()){
            cc.find('Canvas/desk_info').getComponent('fxmj_desk_info').cleanJieSuan()
        }else if(DeskData.Instance().isJBC() || DeskData.Instance().isMatch()){
            cc.find('Canvas/desk_info').getComponent('fxmj_desk_info_jbc').cleanJieSuan()
        }
    },

    on_fx_ack_game_deal_cards: function( msg ) {
     ...
     * @param msg
     */
    on_fx_ack_roomInit: function( msg ) {
        if (!this.headerHandle(msg)) return;
        this.waitTime = 0;
        ...
        if(play_list){
            var pl = play_list.getComponent('fxmj_player_list');
            pl.closeGPSWarn();
        }

    },

    on_fx_ack_game_overturn: function( msg ) {
        if (!this.headerHandle(msg)) return;
        ...
        if (!UserPlayer.hasMoPai()) {
            //吃,碰,点杠
            if (menu_list) {
                menu_list.getComponent("fxmj_game_menu_list").setMenus(UserPlayer);
            }
    ...
    on_fx_ack_game_send_out_card: function( msg ) {
    ...
    on_fx_ack_update_deposit: function( msg ) {
        if (!this.headerHandle(msg)) return;
    ...
    on_fx_ack_chi: function( msg ) {
        ...
        if(!cc.replay_gamedata_scrolling){
            ...
            var play_list = cc.find('Canvas/player_list');
            if(play_list){
                play_list.getComponent('fxmj_player_list').playerStopChuPaiAni();
            }
            cc.log('停止出牌动画-吃牌');
        }
    ...
    on_fx_ack_game_act_peng: function( msg ) {
        if (!this.headerHandle(msg)) return;
        //被碰的牌，移除
        var playerOut = playerMgr.Instance().getPlayer(msg.useridout);
        if (playerOut) {
    ...
    on_fx_ack_game_act_gang: function( msg ) {
        if (!this.headerHandle(msg)) return;
        var playerOut = playerMgr.Instance().getPlayer(msg.useridout);
        if (msg.isrob) {
    ...
    on_fx_ack_game_act_bugang: function( msg ) {
        if (!this.headerHandle(msg)) return;
        //玩家补杠
    ...
    on_fx_ack_game_act_guo: function( msg ) {
        if (!this.headerHandle(msg)) return;
        UserPlayer.guo();
        DeskED.notifyEvent(DeskEvent.CLOSE_MENU, []);
    ...
    on_fx_ack_game_ting: function( msg ) {
        if (!this.headerHandle(msg)) return;
        //听牌
        var player = playerMgr.Instance().getPlayer(msg.userid);
        player.ting(msg.isxiaosa);
        DeskED.notifyEvent(DeskEvent.CLOSE_MENU, []);
    ...
    on_fx_ack_chiting: function( msg ) {
        if (!this.headerHandle(msg)) return;
        //被吃的牌，移除
        var playerOut = playerMgr.Instance().getPlayer(msg.useridout);
        if (playerOut) {
    ...
    on_fx_ack_pengting: function( msg ) {
        if (!this.headerHandle(msg)) return;
        //被碰的牌，移除
        var playerOut = playerMgr.Instance().getPlayer(msg.useridout);
        if (playerOut) {
    ...
    on_fx_ack_gangting: function( msg ) {
        if (!this.headerHandle(msg)) return;
        msg.gangtype = 16;
        var playerOut = playerMgr.Instance().getPlayer(msg.useridout);
        if (msg.isrob) {
    ...
    on_fx_ack_game_act_guout: function( msg ) {
        if (!this.headerHandle(msg)) return;
    ...
    on_fx_ack_game_dabao: function( msg ) {
        if (!this.headerHandle(msg)) return;
    ...
    on_fx_ack_reconnect: function( msg ) {
        ...
        if(msg.deskrules.desktype != 59){   //金币场
    ...
    on_fx_ack_remain_majiang: function( msg ) {
        if (!this.headerHandle(msg)) return;
    ...
    on_fx_ack_change_majiang: function( msg ) {
        if (!this.headerHandle(msg)) return;
     ...
     * @param msg
     */
    on_fx_game_ack_act_hu: function( msg ) {
        if ( !this.headerHandle( msg ) ) {
        ...
        for(let i = 0; i < msg.hutypeList.length; i++){
            if(msg.hutypeList[i] == HuType.QI_DUI || msg.hutypeList[i] == HuType.GANG_HUA_HU || msg.hutypeList[i] == HuType.GANG_PAO_HU){
     ...
     * @param msg
     */
    on_fx_ack_send_current_result: function( msg ) {
    ...
    on_fx_ack_operator: function( msg ) {
        if (!this.headerHandle(msg)) return;
    ...
    on_fx_ack_reloading_ok: function( msg ) {
        if (!this.headerHandle(msg)) return;

        DeskData.Instance().unlockScene();
    },

    on_fx_ack_ready: function( msg ) {
        if (!this.headerHandle(msg)) return;
        ...
        if (player) {
            ...
            if(msg.header.userid == cc.dd.user.id){
                ...
                if(DeskData.Instance().isFriend()){
                    cc.find('Canvas/desk_info').getComponent('fxmj_desk_info').cleanJieSuan();
                    cc.find('Canvas/desk_info').getComponent('fxmj_desk_info').setLastJieSuanActive();
                }else if(DeskData.Instance().isJBC() || DeskData.Instance().isMatch()){
                    cc.find('Canvas/desk_info').getComponent('fxmj_desk_info_jbc').cleanJieSuan()
                }
            }
    ...
    on_fx_ack_sponsor_dissolve_room: function( msg ) {
        if (!this.headerHandle(msg)) return;
        DeskData.Instance().sponsorDissolveRoom(msg);
    },

    on_fx_ack_response_dissolve_room: function( msg ) {
        if (!this.headerHandle(msg)) return;
        DeskData.Instance().responseDissolveRoom(msg);
    },

    on_fx_ack_dissolve_room: function( msg ) {
        if (!this.headerHandle(msg)) return;
    ...
    on_fx_ack_exit_room: function( msg ) {
        if (!this.headerHandle(msg)) return;
        DeskData.Instance().exitRoom(msg);
    },

    on_fx_ack_finally_result: function( msg ) {
        cc.log('【数据】战绩消息接收 开始');
        if (!this.headerHandle(msg)) return;
        cc.dd.UIMgr.destroyUI(jlmj_prefab.JLMJ_JIESAN);
    ...
    on_fx_ack_fen_zhang: function( msg ) {
        if (!this.headerHandle(msg)) return;
        cc.log('【数据】分张消息接收');
        DeskData.Instance().fenzhang();
    },

    on_fx_ack_rob_remove_card: function( msg ) {
        if (!this.headerHandle(msg)) return;
    ...
    on_fx_game_ack_act_huangzhuangpais: function( msg ) {
        if (!this.headerHandle(msg)) return;
     ...
     * @param msg
     */
    /*on_fx_ack_bao: function (msg) {
        DeskData.Instance().setBaoPai( msg.card );
        if(!cc.replay_gamedata_scrolling){
            cc.find("Canvas/desk_info").getComponent('fxmj_desk_info').update_bao_pai();
        }
    },*/
    ...
    stopFaPaiAni(){
        ...
        if(!cc.dd._.isNull(desk_node)){
            ...
            desk_node.getComponent('fxmj_scene').jlmjPlayerDownSortShouPai();
        }
    }
});

module.exports = new fx_handler();