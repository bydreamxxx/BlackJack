...
var fxmj_net_handler_fxmj = require("fxmj_net_handler_fxmj");
...
var UserPlayer = require("fxmj_userPlayer_data");
...
var baseDeskInfo = cc.Class({
    ...
    initLocalData: function () {
        ...
        //桌布
        var json = cc.sys.localStorage.getItem('fxmj_zhuobu_set_' + cc.dd.user.id);
        if (json) {
            var sprite = null;
            ...
            if (sprite) {
                ...
                cc.sys.localStorage.setItem('fxmj_zhuobu_set_' + cc.dd.user.id, this.zhuomianImg[0]._name);
            }
        }
        else {
            this.deskImage.spriteFrame = this.zhuomianImg[0];
    ...
    initGuiZeInfo: function () {
        ...
        var fengding_txt = fengding[RoomMgr.Instance()._Rule.fengdingindex - 1];
        var jifentype = gf_txt[RoomMgr.Instance()._Rule.jifentypeindex - 1];
        cur_rule.roomId = RoomMgr.Instance().roomId;


        this.gz_arr_box = [];
        this.gz_arr_box.push(cc.dd.Text.TEXT_PY_RULE_33.format([RoomMgr.Instance()._Rule.usercountlimit]));
        this.gz_arr_box.push(cc.dd.Text.TEXT_PY_RULE_39.format([fengding_txt]));
        this.gz_arr_box.push(jifentype);
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isjiehu ? Text.TEXT_PY_RULE_26 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isqidui ? Text.TEXT_PY_RULE_27 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.issijia ? Text.TEXT_PY_RULE_42 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isgunjsj ? Text.TEXT_PY_RULE_28 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isqinyise ? Text.TEXT_PY_RULE_29 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isshoubayijiafan ? Text.TEXT_PY_RULE_30 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.ischiting ? Text.TEXT_PY_RULE_31 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isshuipao ? Text.TEXT_PY_RULE_110 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.genzysindex ? gzf_txt[RoomMgr.Instance()._Rule.genzysindex] : '');
        this.gz_arr_box.push(zf_txt[RoomMgr.Instance()._Rule.zha5or10]);
        ...
        this.gz_arr_info.push({ str: Text.TEXT_PY_RULE_33.format([RoomMgr.Instance()._Rule.usercountlimit]), nodetype: 0 });
        this.gz_arr_info.push({ str: juquan_txt.format([RoomMgr.Instance()._Rule.boardscout]), nodetype: 0 });

        this.gz_arr_info.push({ str: Text.TEXT_PY_RULE_39.format([fengding_txt]), nodetype: 1 });
        this.gz_arr_info.push({ str: jifentype, nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.isjiehu ? Text.TEXT_PY_RULE_26 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.isqidui ? Text.TEXT_PY_RULE_27 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.issijia ? Text.TEXT_PY_RULE_42 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.isgunjsj ? Text.TEXT_PY_RULE_28 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.isqinyise ? Text.TEXT_PY_RULE_29 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.isshoubayijiafan ? Text.TEXT_PY_RULE_30 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.ischiting ? Text.TEXT_PY_RULE_31 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.isshuipao ? Text.TEXT_PY_RULE_110 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.genzysindex ? gzf_txt[RoomMgr.Instance()._Rule.genzysindex] : '', nodetype: 1 });
        this.gz_arr_info.push({ str: zf_txt[RoomMgr.Instance()._Rule.zha5or10], nodetype: 1 });
    ...
    initZhiNan: function () {
        if (this.zhinan.active && DeskData.Instance().zhiNan_site != null) {
            ...
            this.zhinan.getComponent("fxmj_zhinan_ui").initsetZhiNan(site);
    ...
    change_room: function () {
        ...
        var play_list = cc.find('Canvas/player_list').getComponent('fxmj_player_list');
        if (play_list) {
    ...
    recoverDesk: function () {
        cc.log("fxmj 朋 友 recover_desk------ 接收1");
        cc.log("------恢复桌子------py");
        this.updateDesk();
        DeskData.Instance().enterSceneRecoverDesk(this.sendReadyOK.bind(this));
        if (this.fenzhang) {
    ...
    sendReadyOK: function () {
        ...
        var msg = new cc.pb.fuxinmajiang.fx_req_reloading_ok();
        cc.gateNet.Instance().sendMsg(cc.netCmd.fuxinmajiang.cmd_fx_req_reloading_ok, msg, "fx_req_reloading_ok");
        this.initGuiZeInfo();
    },
    ...
    jiesuan: function (data) {
        ...
        this.jiesuan_TimeID = setTimeout(function () {
            ...
            if (cc.director.getScene().name != 'fxmj_py_game' ||
                cc.director.getScene().name != 'fxmj_jbc_game') {
                cc.dd.UIMgr.destroyUI(jlmj_prefab.JLMJ_JIAOPAI_INFO);
    ...
    leave_tips: function (data) {
        ...
        if (DeskData.Instance().isFriend()) {//朋友场
            ...
            } else if (fxmj_desk_data_jbc.getInstance().getIsMatching() || DeskData.Instance().player_read_gamne) {//取消匹配状态
                txt = Text.TEXT_LEAVE_ROOM_3;
                callfunc = this.jbc_matching_leave_room_req;
            } else if (fxmj_desk_data_jbc.getInstance().getIsStart()) {//游戏中退出游戏
    ...
    py_dissolve_room_req: function () {
        ...
        var msg = new cc.pb.fuxinmajiang.fx_req_sponsor_dissolve_room();
        msg.setSponsorid(dd.user.id);
        cc.gateNet.Instance().sendMsg(cc.netCmd.fuxinmajiang.cmd_fx_req_sponsor_dissolve_room, msg, "fx_req_sponsor_dissolve_room");
    },
        ...
        if (msg.retCode !== 0) {
            ...
            switch (msg.retCode) {
                case 1:
                    ...
                    this.popupEnterHall(str, fxmj_util.enterHall.bind(fxmj_util));
                    break;
                case 2:
    ...
    py_ready: function () {
        ...
        var msg = new cc.pb.fuxinmajiang.fx_req_ready();
        cc.gateNet.Instance().sendMsg(cc.netCmd.fuxinmajiang.cmd_fx_req_ready, msg, "fx_req_ready");
    },
    ...
    update_player_info: function () {
        ...
        this.zhinan.getComponent("fxmj_zhinan_ui").initDirection();
        var play_list = cc.find('Canvas/player_list');
        //play_list.playerUpdateUI();
        if (play_list) {
            var fxmj_player = play_list.getComponent('fxmj_player_list');
            fxmj_player.refreshGPSWarn();
        }
    },
    ...
    getTextByWanFa: function () {
        ...
        this.gz_arr_box = [];
        this.gz_arr_box.push(cc.dd.Text.TEXT_PY_RULE_33.format([cur_rule.usercountlimit]));
        RoomMgr.Instance()._Rule.isjiehu ? this.gz_arr_box.push(Text.TEXT_PY_RULE_26) : null;
        RoomMgr.Instance()._Rule.isqidui ? this.gz_arr_box.push(Text.TEXT_PY_RULE_27) : null;
        RoomMgr.Instance()._Rule.issijia ? this.gz_arr_box.push(Text.TEXT_PY_RULE_42) : null;
        RoomMgr.Instance()._Rule.isgunjsj ? this.gz_arr_box.push(Text.TEXT_PY_RULE_28) : null;
        RoomMgr.Instance()._Rule.isqinyise ? this.gz_arr_box.push(Text.TEXT_PY_RULE_29) : null;
        RoomMgr.Instance()._Rule.isshoubayijiafan ? this.gz_arr_box.push(Text.TEXT_PY_RULE_30) : null;
        RoomMgr.Instance()._Rule.ischiting ? this.gz_arr_box.push(Text.TEXT_PY_RULE_31) : null;
        RoomMgr.Instance()._Rule.isshuipao ? this.gz_arr_box.push(Text.TEXT_PY_RULE_110) : null;
        RoomMgr.Instance()._Rule.genzysindex ? this.gz_arr_box.push(Text.TEXT_PY_RULE_43) : null;
    ...
    onInvaite: function (event, custom) {
        ...
        jlmj_audio_mgr.com_btn_click();
        var title = Text.TEXT_PY_RULE_33.format(playerMgr.Instance().playerList.length);
    ...
    on_show_result_view: function () {
        ...
        cc.dd.UIMgr.openUI(jlmj_prefab.MJ_ZHANJITONGJI, function (ui) {
            var fj_arr = [];
            var juquan_txt = RoomMgr.Instance()._Rule.usercountlimit == 2 ? Text.TEXT_PY_RULE_10 : Text.TEXT_PY_RULE_11;
            fj_arr.push(Text.TEXT_PY_RULE_33.format([RoomMgr.Instance()._Rule.usercountlimit]));
    ...
    onEventMessage: function (event, data) {
        ...
        switch (event) {
            ...
            case HallCommonEvent.HALL_NO_RECONNECT_GAME:
                cc.log("fxmj  响应------> HallCommonEvent.HALL_NO_RECONNECT_GAME");
            ...
            case DeskEvent.RECOVER_DESK://恢复桌子
                cc.log("fxmj 朋 友 recover_desk------ 开始0");
                this.recoverDesk();
                break;
    ...
    _change2D() {
        ...
        cc.find("Canvas/desk_node/jlmj_player_left_ui").getComponent('fxmj_player_left_ui').resetConfig();
        cc.find("Canvas/desk_node/jlmj_player_down_ui").getComponent('fxmj_player_down_ui').resetConfig();
        cc.find("Canvas/desk_node/jlmj_player_right_ui").getComponent('fxmj_player_right_ui').resetConfig();
        cc.find("Canvas/desk_node/jlmj_player_up_ui").getComponent('fxmj_player_up_ui').resetConfig();
    },
