...
cc.Class({
    extends: require('fxmj_player_base_ui'),
    ...
    updateShouPai: function (player) {
        ...
        if (!DeskData.Instance().isHu && !DeskData.isFenZhang &&
            ...
            player.state != PlayerState.TINGPAI && !RoomMgr.Instance()._Rule.isshoubayijiafan) {
    ...
    touchStart: function (event) {
        ...
        if (pai_touched) {
            ...
            if (this.touchCardMode == TouchCardMode.DA_TING_PAI) {
                this.closeJiaoInfo();
                cc.log("fxmj touchStart openJiaoInfo");
    ...
    biaojiPai: function (selectId) {
        ...
        for (var i in userList) {
            var com = userList[i].getComponent('fxmj_player_down_ui');
    ...
    sendTingPai: function (playCardId, tingType) {
        ...
        if (tingType == 3) {
            ...
            var msg = new cc.pb.fuxinmajiang.fx_req_chiting();
            ...
            cc.gateNet.Instance().sendMsg(cc.netCmd.fuxinmajiang.cmd_fx_req_chiting, msg, "cmd_fx_req_chiting");
            cc.dd.NetWaitUtil.net_wait_start();
        } else if (tingType == 4) {
            var msg = new cc.pb.fuxinmajiang.fx_req_pengting();

            msg.setOutcard(out_card);
            msg.setIsxiaosa(UserPlayer.isTempXiaosa);

            cc.gateNet.Instance().sendMsg(cc.netCmd.fuxinmajiang.cmd_fx_req_pengting, msg, "cmd_fx_req_pengting");
            cc.dd.NetWaitUtil.net_wait_start();
    ...
    ting: function (player) {
        ...
        this.closeJiaoInfo();
        var desk_info = cc.find('Canvas/desk_info').getComponent('fxmj_desk_info');
