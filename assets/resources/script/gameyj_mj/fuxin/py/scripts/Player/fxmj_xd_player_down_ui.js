...
cc.Class({
    ...
    onLoad: function () {
        cc.log('fxmj_player_down_ui onLoad');

        let desk_node = cc.find("Canvas/desk_node");
        this._node_scale = desk_node.scaleX;
        let canvas = cc.find("Canvas");
        this._offsetX = canvas.width / 2;
        this._offsetY = canvas.height / 2;

        this._super();              //特效
        this.zhanshi_pai = cc.find("Canvas/zhanshi_pais/jlmj_zhanshi_pai_down").getComponent('jlmj_pai');     //展示牌
        this.node_hu_1 = cc.find("Canvas/play_anis/jlmj_hu_ani_1_d");
        this.node_hu_2 = cc.find("Canvas/play_anis/jlmj_hu_ani_2_d");
        this.node_hu_3 = cc.find("Canvas/play_anis/jlmj_hu_ani_3_d");
        this.tcp_ani = cc.find("Canvas/play_anis/jlmj_tcp_ani_d");
        this.jbdga_ani = cc.find("Canvas/play_anis/jlmj_jbdga_ani_d");
        this.touch_node = cc.find("Canvas/player_down_ui/user_touch_node");
        //cc.find("Canvas/jlmj_player_down_ui/com_game_head").setLocalZOrder(UIZorder.MJ_LAYER_TOP);
        this.chupai_node = cc.find("Canvas/chupai_down_node");
    ...
    updateShouPai: function (player) {
        ...
        if (!DeskData.Instance().isHu && !DeskData.isFenZhang &&
            (player.shoupai.length == 4 || player.shoupai.length == 5) && player.state != PlayerState.TINGPAI) {
        ...
        for (var i = 0; i < this.shouPai.length; ++i) {
            if (shoupai_visible_cfg[shouPaiLen] && shoupai_visible_cfg[shouPaiLen].indexOf(i) != -1) {
                ...
                if (this.touchCardMode != TouchCardMode.DA_TING_PAI && this.touchCardMode != TouchCardMode.DA_GU_PAI) {
        ...
        if (player.hasMoPai()) {//说明手牌中是包含
            ...
            if (player.state != PlayerState.HUPAI && player.state != PlayerState.TINGPAI) {
                cc.find("Canvas/game_menu_list").getComponent("fxmj_game_menu_list").setMenus(player);
            }
        } else {
            this.modepai.node.active = false;
        }
    ...
    touchMove: function (event) {
        ...
        if (pai_touched) {//找到选择的牌
            ...
            if (this.yidong_pai.node.active == false) {
                if (!this.yidong_pai.cloned || this.yidong_pai.cardId != this.pai_touched.cardId) {
                    ...
                    this.yidong_pai.node.scaleX = this.yidong_pai.node.scaleX * this._node_scale;
                    this.yidong_pai.node.scaleY = this.yidong_pai.node.scaleY * this._node_scale;
            ...
            if (pai_touched && (this.touchCardMode == TouchCardMode.DA_TING_PAI || this.touchCardMode == TouchCardMode.DA_GU_PAI)) {
                this.closeJiaoInfo();
                this.openJiaoInfo(this.pai_touched.cardId);
            }
        }
        ...
        if (this.pai_touched &&                          //没有选择牌
            ...
            this.yidong_pai.node.x = event.touch.getLocationX() - this._offsetX;
            this.yidong_pai.node.y = event.touch.getLocationY() - this._offsetY;
            if (this.yidong_pai.node.y > -260 * this._node_scale) {
    ...
    touchEnd: function (event) {
        ...
        if (this.yidong_pai.node.active) {
            if (this.yidong_pai.node.y > -200 * this._node_scale) {
                //出牌
                // this.resetSelected();
                ...
                } else if (UserPlayer.isTempBaoGu) {
                    this.sendGuPai(this.yidong_pai.cardId);
                } else {
                    if (UserPlayer.hasMoPai()) {
                        if (this.touchCardMode == TouchCardMode.DA_TING_PAI) {
                            cc.log("fxmj touchEnd sendOutCard");
                            UserPlayer.setJiaoInfo(this.yidong_pai.cardId);
                        }
            ...
            if (UserPlayer.isTempBaoTing) {
                ...
                UserPlayer.setJiaoInfo(pai_touched.cardId);
                this.sendGuPai(pai_touched.cardId);
            } else {
                if (UserPlayer.hasMoPai()) {
                    if (this.touchCardMode == TouchCardMode.DA_TING_PAI) {
                        cc.log("fxmj touchEnd sendOutCard");
    ...
    biaojiPai: function (selectId) {
        ...
        for (var i in userList) {
            var com = userList[i].getComponent('fxmj_xd_player_down_ui');
    ...
    sendTingPai: function (playCardId, tingType) {
        ...
        var msg = new cc.pb.fuxinmajiang.fx_req_tingpai_out_card();
        msg.setCardid(playCardId);
        msg.setTingtype(tingType);

        cc.gateNet.Instance().sendMsg(cc.netCmd.fuxinmajiang.cmd_fx_req_tingpai_out_card, msg, "cmd_fx_req_tingpai_out_card");
    },
    ...
    sendOutCard: function (cardID) {
        ...
        this.quxiaoBiaoji();
        var msg = new cc.pb.fuxinmajiang.fx_req_game_send_out_card();
        msg.setCardid(cardID);
        cc.gateNet.Instance().sendMsg(cc.netCmd.fuxinmajiang.cmd_fx_req_game_send_out_card, msg, "fx_req_game_send_out_card");
    },
    ...
    play_chupai_ani: function (userplayer, chupai_idx_in_shoupai, last_chupai_idx, cardID) {
        ...
        // var seq = cc.sequence(cc.moveTo(0.03,cc.v2(0,-142)),cc.delayTime(0.03),cc.callFunc(shou2midEnd.bind(this)));
        var seq = cc.sequence(
            cc.callFunc(() => {
                mj_audio.playAduio('dapai');
            }),
    ...
    showDaPaiGu: function (isShow, canCradList, type) {
        this.setShoupaiTingbiaoji(false);
        this.touchCardMode = type || 5;
        isShow = isShow || false;
        if (isShow && canCradList) {//可以操作canCardList中牌
    ...
    ting: function (player) {
        ...
        this.closeJiaoInfo();
        var desk_info = cc.find('Canvas/desk_info').getComponent('fxmj_xd_desk_info');
     ...
     * @param playCardId 出牌
     */
    sendGuPai: function (playCardId) {
        if (playCardId < 0) {
            cc.error("估后出的牌为空")
            return;
        }
        var msg = new cc.pb.fuxinmajiang.fx_req_game_act_guout();
        msg.setCardid(playCardId);

        cc.gateNet.Instance().sendMsg(cc.netCmd.fuxinmajiang.cmd_fx_req_game_act_guout, msg, "cmd_fx_req_game_act_guout");
    },
    ...
    gu: function (data) {
        ...
        mj_audio.playAudioBySex("ting", player.sex);
        this.tcp_ani.node.active = true;
        this.tcp_ani.playAnimation('gu', 1);
        this.tcp_ani.addEventListener(dragonBones.EventObject.LOOP_COMPLETE, this.tcpAniCallback, this);
        DeskED.notifyEvent(DeskEvent.UPDATE_BAO_PAI, []);
    },
