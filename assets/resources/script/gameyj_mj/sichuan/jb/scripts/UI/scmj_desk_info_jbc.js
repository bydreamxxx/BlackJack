var baseDeskInfo = require( "scmj_desk_info" );
...
var baseDeskInfoJB = cc.Class({
    ...
    onLoad:function () {
        cc.log("scmj_desk_info_jbc onLoad");
        this._super();
        this.initDeskDataUI();
        this.initDeskData();
    },
    ...
    initDeskDataUI:function () {
        ...
        this.goOnBtn = cc.find("Canvas/desk_node/jixuBtn");
        if(this.goOnBtn){
    ...
    initDeskData: function() {
        var deskDataJBC= scmj_desk_data_jbc.getInstance();
    ...
    start: function () {
        if( scmj_desk_data_jbc.getInstance().getIsReconnect() ) {
            DeskData.Instance().enterSceneRecoverDesk(this.sendReadyOK.bind(this));
        }
    },

    onDestroy: function() {
        scmj_desk_data_jbc.getInstance().destroy();
        this._super();
        //RoomMgr.Destroy();
    },
    ...
    on_room_replace:function (msg) {
        if( msg.retCode !== 0 ) {
            ...
            this.playMatchAnim();
            scmj_desk_data_jbc.getInstance().setIsMatching(true);
            this.tuo_guan(false);
            cc.find("Canvas/desk_node/jlmj_zhishiqi").active = false;
        }
    },
    ...
    on_room_enter:function () {
        cc.log("scmj jbc desk on_room_enter--->"+(DeskData.Instance().isGameStart));
    ...
    recover_desk:function () {
        cc.log("scmj jbc desk recover_desk------执行0");
        this.on_room_enter();
        //this.recoverDesk();
    },
        ...
        if( msg.retCode !== 0 ) {
            ...
            switch (msg.retCode) {
                case 1:
                    ...
                    this.popupEnterHall( str, scmj_util.enterRoomList.bind(scmj_util) );
                    break;
                case 2:
    ...
    tuo_guan: function (isShow) {
        if(isShow){
            ...
            if(this.menu_list){
                this.menu_list.getComponent("scmj_game_menu_list").closeMenuAndOptions();
            }
    ...
    clean_tuo_guan:function () {
        if(DeskData.Instance().isoffline) {
            const req = new cc.pb.xuezhanmj.xzmj_req_update_deposit();
            req.setIsdeposit(false);
            cc.gateNet.Instance().sendMsg(cc.netCmd.xuezhanmj.cmd_xzmj_req_update_deposit, req, "xzmj_req_update_deposit", true);
        }
    },
    ...
    onEventMessage: function (event, data) {
        ...
        switch (event) {
            ...
            case DeskEvent.RECOVER_DESK:
                cc.log("scmj jbc recover_desk------ 开始0");
                this.recover_desk();
                break;
            case DeskEvent.JBC_READY://
                this.jbc_ready();
                break;
            case DeskEvent.PO_CHAN:
                this.pochan();
                break;
    ...
    onClickGoOn(){
        ...
        DeskData.Instance().pauseGameOut = true;
    ...
    pochan(){
        if(DeskData.Instance().isJBC()){
            let _action = this.da_pai_prompt.getActionByTag(10086);
            ...
            for(let i = 7; i >= 0; i--){
                ...
                actionList.push(cc.callFunc(()=>{
                    ...
                    this.da_pai_prompt_label.string = "您已破产~无法继续牌局了，将在"+idx+"秒后离开....";
                }));
            }
            actionList.push(cc.delayTime(1));
            actionList.push(cc.callFunc(()=>{
                this.leave_tips({status:7});
            }));

            this.da_pai_prompt.active = true;
            this.da_pai_prompt_label.string = "您已破产~无法继续牌局了，将在8秒后离开....";

            let action = cc.sequence(actionList);
            action.setTag(10086);
            this.da_pai_prompt.stopAllActions();
            this.da_pai_prompt.runAction(action)
        }
    },
