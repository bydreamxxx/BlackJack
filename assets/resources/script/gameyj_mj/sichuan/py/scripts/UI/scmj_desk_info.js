...
var scmj_util = require("scmj_util");
...
var baseDeskInfo = cc.Class({
    ...
    properties: {
        ...
        xz_kuaileba: cc.SpriteFrame,
        xl_kuaileba: cc.SpriteFrame,
    },

    onLoad: function () {
        cc.log("检查场景名是否为空: " + cc.dd.SceneManager.getCurrScene().name);
        cc.log("scmj_desk_info onLoad");
        ...
        this.logo = cc.find("Canvas/desk_node/c-logo-fuxin").getComponent(cc.Sprite);
        if (cc._isKuaiLeBaTianDaKeng) {
            this.logo.spriteFrame = RoomMgr.Instance().gameId == cc.dd.Define.GameType.XLMJ_GOLD || RoomMgr.Instance().gameId == cc.dd.Define.GameType.XLMJ_FRIEND ? this.xl_kuaileba : this.xz_kuaileba;
        } else {
            this.logo.spriteFrame = RoomMgr.Instance().gameId == cc.dd.Define.GameType.XLMJ_GOLD || RoomMgr.Instance().gameId == cc.dd.Define.GameType.XLMJ_FRIEND ? this.xl : this.xz;
        }
        ...
        cc.find("Canvas/desk_node/pai").active = false;
        cc.find("Canvas/desk_node/pai_2d").active = false;
    ...
    initDeskDataUI: function () {
        ...
        // cc.find("Canvas/desk_node/jlmj_player_down_ui/tingButton").setLocalZOrder(UIZorder.MJ_LAYER_UI);
    ...
    initLocalData: function () {
        ...
        //桌布
        var json = cc.sys.localStorage.getItem('scmj_zhuobu_set_' + cc.dd.user.id);
        if (json) {
            var sprite = null;
            ...
            if (sprite) {
                ...
                cc.sys.localStorage.setItem('scmj_zhuobu_set_' + cc.dd.user.id, this.zhuomianImg[0]._name);
            }
        }
        else {
            this.deskImage.spriteFrame = this.zhuomianImg[0];
    ...
    initGuiZeInfo: function () {
        ...
        let _text = !RoomMgr.Instance()._Rule.isxueliu ? Text.TEXT_PY_RULE_69 : Text.TEXT_PY_RULE_70;
        let _text1 = _text.format([RoomMgr.Instance()._Rule.usercountlimit]);
        let _text15 = '';
        if (!RoomMgr.Instance()._Rule.isxueliu) {
            if (RoomMgr.Instance()._Rule.usercountlimit == 3) {
                _text15 = RoomMgr.Instance()._Rule.issanfang ? Text.TEXT_PY_RULE_143 : Text.TEXT_PY_RULE_144
            } else if (RoomMgr.Instance()._Rule.usercountlimit == 2) {
                _text15 = RoomMgr.Instance()._Rule.issanfang ? Text.TEXT_PY_RULE_145 : Text.TEXT_PY_RULE_146
            }
        }
        let _text2 = Text.TEXT_PY_RULE_56.format([RoomMgr.Instance()._Rule.difen]);
        let _text3 = RoomMgr.Instance()._Rule.fengding == 0 ? Text.TEXT_PY_RULE_54 : Text.TEXT_PY_RULE_55.format([RoomMgr.Instance()._Rule.fengding]);
        let _text4 = RoomMgr.Instance()._Rule.zimotype == 0 ? Text.TEXT_PY_RULE_57 : Text.TEXT_PY_RULE_58;
        let _text5 = RoomMgr.Instance()._Rule.isshishiyu ? Text.TEXT_PY_RULE_59 : Text.TEXT_PY_RULE_60;
        let _text16 = RoomMgr.Instance()._Rule.isdiangangzimo ? Text.TEXT_PY_RULE_147 : Text.TEXT_PY_RULE_148;
        let _text6 = RoomMgr.Instance()._Rule.ishuan3zhang ? Text.TEXT_PY_RULE_61 : '';
        let _text17 = RoomMgr.Instance()._Rule.huan4zhang && !RoomMgr.Instance()._Rule.isxueliu ? Text.TEXT_PY_RULE_151 : '';
        let _text7 = RoomMgr.Instance()._Rule.ishujiaozhuanyi ? Text.TEXT_PY_RULE_62 : '';
        let _text18 = RoomMgr.Instance()._Rule.hujiaozhuanyizhuangen && !RoomMgr.Instance()._Rule.isxueliu ? Text.TEXT_PY_RULE_149 : '';
        let _text8 = RoomMgr.Instance()._Rule.isjingoudiao ? Text.TEXT_PY_RULE_63 : '';
        let _text9 = RoomMgr.Instance()._Rule.ishaidilao ? Text.TEXT_PY_RULE_64 : '';
        let _text10 = RoomMgr.Instance()._Rule.ishaidipao ? Text.TEXT_PY_RULE_65 : '';
        let _text11 = RoomMgr.Instance()._Rule.istiandihu ? Text.TEXT_PY_RULE_66 : '';
        let _text12 = RoomMgr.Instance()._Rule.ismenqingzz ? Text.TEXT_PY_RULE_67 : '';
        let _text13 = RoomMgr.Instance()._Rule.isxueliu ? Text.TEXT_PY_RULE_68 : '';
        let _text14 = RoomMgr.Instance()._Rule.is2fanqibu && !RoomMgr.Instance()._Rule.isxueliu ? Text.TEXT_PY_RULE_111 : '';
        let _text19 = RoomMgr.Instance()._Rule.duiduihu3fan && !RoomMgr.Instance()._Rule.isxueliu ? Text.TEXT_PY_RULE_150 : '';
        let _text20 = RoomMgr.Instance()._Rule.jiaxinwu && !RoomMgr.Instance()._Rule.isxueliu ? Text.TEXT_PY_RULE_152 : '';
        let _text21 = RoomMgr.Instance()._Rule.yitiaolong && !RoomMgr.Instance()._Rule.isxueliu ? Text.TEXT_PY_RULE_115 : '';

        this.gz_arr_box = [];
        this.gz_arr_box.push(_text1);
        this.gz_arr_box.push(_text15);
        this.gz_arr_box.push(_text2);
        this.gz_arr_box.push(_text3);
        this.gz_arr_box.push(_text4);
        this.gz_arr_box.push(_text5);
        this.gz_arr_box.push(!RoomMgr.Instance()._Rule.isxueliu ? _text16 : '');
        this.gz_arr_box.push(_text6);
        this.gz_arr_box.push(_text17);
        this.gz_arr_box.push(_text7);
        this.gz_arr_box.push(_text18);
        this.gz_arr_box.push(_text8);
        this.gz_arr_box.push(_text9);
        this.gz_arr_box.push(_text10);
        this.gz_arr_box.push(_text11);
        this.gz_arr_box.push(_text12);
        this.gz_arr_box.push(_text13);
        this.gz_arr_box.push(_text14);
        this.gz_arr_box.push(_text19);
        this.gz_arr_box.push(_text20);
        this.gz_arr_box.push(_text21);
        ...
        this.gz_arr_info = [];
        this.gz_arr_info.push({ str: _text1, nodetype: 0 });
        this.gz_arr_info.push({ str: Text.TEXT_PY_RULE_10.format([RoomMgr.Instance()._Rule.boardscout]), nodetype: 0 });
        this.gz_arr_info.push({ str: _text15, nodetype: 1 });

        this.gz_arr_info.push({ str: _text2, nodetype: 1 });
        this.gz_arr_info.push({ str: _text3, nodetype: 1 });
        this.gz_arr_info.push({ str: _text4, nodetype: 1 });
        this.gz_arr_info.push({ str: _text5, nodetype: 1 });
        this.gz_arr_info.push({ str: !RoomMgr.Instance()._Rule.isxueliu ? _text16 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: _text6, nodetype: 1 });
        this.gz_arr_info.push({ str: _text17, nodetype: 1 });
        this.gz_arr_info.push({ str: _text7, nodetype: 1 });
        this.gz_arr_info.push({ str: _text18, nodetype: 1 });
        this.gz_arr_info.push({ str: _text8, nodetype: 1 });
        this.gz_arr_info.push({ str: _text9, nodetype: 1 });
        this.gz_arr_info.push({ str: _text10, nodetype: 1 });
        this.gz_arr_info.push({ str: _text11, nodetype: 1 });
        this.gz_arr_info.push({ str: _text12, nodetype: 1 });
        this.gz_arr_info.push({ str: _text13, nodetype: 1 });
        this.gz_arr_info.push({ str: _text14, nodetype: 1 });
        this.gz_arr_info.push({ str: _text19, nodetype: 1 });
        this.gz_arr_info.push({ str: _text20, nodetype: 1 });
        this.gz_arr_info.push({ str: _text21, nodetype: 1 });
    ...
    initZhiNan: function () {
        if (this.zhinan.active && DeskData.Instance().zhiNan_site != null) {
            ...
            this.zhinan.getComponent("scmj_zhinan_ui").initsetZhiNan(site);
    ...
    onDestroy: function () {
        ...
        if (this.da_pai_prompt) {
            this.da_pai_prompt.stopAllActions();
        }
    ...
    recoverDesk: function () {
        cc.log("scmj 朋 友 recover_desk------ 接收1");
        cc.log("------恢复桌子------py");
        this.updateDesk();
        DeskData.Instance().enterSceneRecoverDesk(this.sendReadyOK.bind(this));
        if (this.fenzhang) {
    ...
    sendReadyOK: function () {
        ...
        var msg = new cc.pb.xuezhanmj.xzmj_req_reloading_ok();
        cc.gateNet.Instance().sendMsg(cc.netCmd.xuezhanmj.cmd_xzmj_req_reloading_ok, msg, "xzmj_req_reloading_ok");
        this.initGuiZeInfo();
    },
    ...
    jiesuan: function (data) {
        ...
        let waitTime = cc._needShowDrop ? 4000 : 1000;
        ...
        cc.find("Canvas/desk_node/jlmj_player_down_ui").getComponent('scmj_player_down_ui').quxiaoBiaoji();
        ...
        this.jiesuan_TimeID = setTimeout(function () {
            ...
            if (cc.director.getScene().name != 'scmj_py_game' ||
                cc.director.getScene().name != 'scmj_jbc_game') {
                cc.dd.UIMgr.destroyUI(jlmj_prefab.SCMJ_JIAOPAI_INFO);
    ...
    leave_tips: function (data) {
        ...
        if (DeskData.Instance().isFriend()) {//朋友场
            ...
            if (data.status == 7 || DeskData.Instance().canLeave === true) {
                cc.log("正常离开");
                this.jbc_matching_leave_room_req();
            } else if (scmj_desk_data_jbc.getInstance().getIsMatching() || DeskData.Instance().player_read_gamne) {//取消匹配状态
                cc.log("取消匹配");
                txt = Text.TEXT_LEAVE_ROOM_3;
                callfunc = this.jbc_matching_leave_room_req;
            } else if (scmj_desk_data_jbc.getInstance().getIsStart()) {//游戏中退出游戏
    ...
    py_dissolve_room_req: function () {
        ...
        var msg = new cc.pb.xuezhanmj.xzmj_req_sponsor_dissolve_room();
        msg.setSponsorid(dd.user.id);
        cc.gateNet.Instance().sendMsg(cc.netCmd.xuezhanmj.cmd_xzmj_req_sponsor_dissolve_room, msg, "xzmj_req_sponsor_dissolve_room");
    },
    ...
    on_player_HuangZhuang_ani_begin: function () {
        ...
        // this.huangzhuang.setAnimation(0, "liuju",false);
        // this.huangzhuang.scheduleOnce(function () {
        //     this.huangzhuang.clearTracks();
        //     this.huangzhuang.node.active = false;
        //     this.on_player_HuangZhuang_ani_end();
        //     cc.log("liuju call back ---------");
        // }.bind(this), 1);
        ...
        this.playSpine(this.huangzhuang, ['liuju']);
    },
    ...
    on_room_ready: function (msg) {
        if (scmj_desk_data_jbc.getInstance().getIsStart()) {
        ...
        if (msg.retCode !== 0) {
            ...
            switch (msg.retCode) {
                case 1:
                    ...
                    this.popupEnterHall(str, scmj_util.enterHall.bind(scmj_util));
                    break;
                case 2:
    ...
    py_ready: function () {
        ...
        var msg = new cc.pb.xuezhanmj.xzmj_req_ready();
        cc.gateNet.Instance().sendMsg(cc.netCmd.xuezhanmj.cmd_xzmj_req_ready, msg, "xzmj_req_ready");
    },
    ...
    update_player_info: function () {
        ...
        this.zhinan.getComponent("scmj_zhinan_ui").initDirection();
        var play_list = cc.find('Canvas/player_list');
        //play_list.playerUpdateUI();
        if (play_list) {
            var scmj_player = play_list.getComponent('scmj_player_list');
            scmj_player.refreshGPSWarn();
        }
    },
    ...
    update_remain_card: function (cardNum) {
        ...
        if (DeskData.Instance().isGameStart) {
            cc.find("Canvas/desk_node/pai/pai_num").getComponent(cc.Label).string = cardNum;
            cc.find("Canvas/desk_node/pai_2d/pai_num_2d").getComponent(cc.Label).string = cardNum;


            cc.find("Canvas/desk_node/pai").active = !this.use2D;
            cc.find("Canvas/desk_node/pai_2d").active = this.use2D;
        } else {
            cc.find("Canvas/desk_node/pai").active = false;
            cc.find("Canvas/desk_node/pai_2d").active = false;
        }
    ...
    on_show_da_pai_prompt: function (data) {
        let action = this.da_pai_prompt.getActionByTag(10086);
        ...
        if (data[1] != null && this.da_pai_prompt) {
            ...
            var text_arr = [Text.TEXT_MJ_DESK_INFO_0, Text.TEXT_MJ_DESK_INFO_7, Text.TEXT_MJ_DESK_INFO_8, Text.TEXT_MJ_DESK_INFO_9];
    ...
    getTextByWanFa: function () {
        ...
        let _text = cur_rule.isxueliu ? Text.TEXT_PY_RULE_70 : Text.TEXT_PY_RULE_69;
        let _text1 = _text.format([cur_rule.usercountlimit]);
        let _text2 = Text.TEXT_PY_RULE_56.format([cur_rule.difen]);
        let _text3 = cur_rule.fengding == 0 ? Text.TEXT_PY_RULE_54 : Text.TEXT_PY_RULE_55.format([cur_rule.fengding]);
        let _text4 = cur_rule.zimotype == 0 ? Text.TEXT_PY_RULE_57 : Text.TEXT_PY_RULE_58;
        let _text5 = cur_rule.isshishiyu ? Text.TEXT_PY_RULE_59 : Text.TEXT_PY_RULE_60;

        this.gz_arr_box = [];
        this.gz_arr_box.push(_text1);
        let _text15 = '';
        if (!cur_rule.isxueliu) {
            if (cur_rule.usercountlimit == 3) {
                _text15 = cur_rule.issanfang ? Text.TEXT_PY_RULE_143 : Text.TEXT_PY_RULE_144
                this.gz_arr_box.push(_text15);
            } else if (cur_rule.usercountlimit == 2) {
                _text15 = cur_rule.issanfang ? Text.TEXT_PY_RULE_145 : Text.TEXT_PY_RULE_146
                this.gz_arr_box.push(_text15);
            }
        }
        this.gz_arr_box.push(_text2);
        ...
        this.gz_arr_box.push(_text4);
        this.gz_arr_box.push(_text5);
        let _text16 = cur_rule.isdiangangzimo ? Text.TEXT_PY_RULE_147 : Text.TEXT_PY_RULE_148;
        !cur_rule.isxueliu ? this.gz_arr_box.push(_text16) : null;
        cur_rule.ishuan3zhang ? this.gz_arr_box.push(Text.TEXT_PY_RULE_61) : null;
        cur_rule.huan4zhang && !cur_rule.isxueliu ? this.gz_arr_box.push(Text.TEXT_PY_RULE_151) : null;
        cur_rule.ishujiaozhuanyi ? this.gz_arr_box.push(Text.TEXT_PY_RULE_62) : null;
        cur_rule.hujiaozhuanyizhuangen && !cur_rule.isxueliu ? this.gz_arr_box.push(Text.TEXT_PY_RULE_149) : null;
        cur_rule.isjingoudiao ? this.gz_arr_box.push(Text.TEXT_PY_RULE_63) : null;
        cur_rule.ishaidilao ? this.gz_arr_box.push(Text.TEXT_PY_RULE_64) : null;
        cur_rule.ishaidipao ? this.gz_arr_box.push(Text.TEXT_PY_RULE_65) : null;
        cur_rule.istiandihu ? this.gz_arr_box.push(Text.TEXT_PY_RULE_66) : null;
        cur_rule.ismenqingzz ? this.gz_arr_box.push(Text.TEXT_PY_RULE_67) : null;
        cur_rule.isxueliu ? this.gz_arr_box.push(Text.TEXT_PY_RULE_68) : null;
        cur_rule.is2fanqibu && !cur_rule.isxueliu ? this.gz_arr_box.push(Text.TEXT_PY_RULE_111) : null;
        cur_rule.duiduihu3fan && !cur_rule.isxueliu ? this.gz_arr_box.push(Text.TEXT_PY_RULE_150) : null;
        cur_rule.jiaxinwu && !cur_rule.isxueliu ? this.gz_arr_box.push(Text.TEXT_PY_RULE_152) : null;
        cur_rule.yitiaolong && !cur_rule.isxueliu ? this.gz_arr_box.push(Text.TEXT_PY_RULE_115) : null;
    ...
    onInvaite: function (event, custom) {
        ...
        jlmj_audio_mgr.com_btn_click();
        let _text = RoomMgr.Instance()._Rule.isxueliu ? Text.TEXT_PY_RULE_70 : Text.TEXT_PY_RULE_69;
        let length = 0;
        playerMgr.Instance().playerList.forEach((player) => {
            if (player) {
                length++;
            }
        })
        var title = _text.format(length);
        title += " " + "房间号:" + (RoomMgr.Instance().roomId || 888888);
        var content = '共' + RoomMgr.Instance()._Rule.boardscout + '' + '局';
    ...
    onTingPaiTouch: function () {
        ...
        var ui_jiaoinfo = cc.dd.UIMgr.getUI(jlmj_prefab.SCMJ_JIAOPAI_INFO);
        ...
        if (jiaoInfo != null) {
            cc.dd.UIMgr.openUI(jlmj_prefab.SCMJ_JIAOPAI_INFO, function (ui) {
                var play_list = cc.find('Canvas/player_list').getComponent('scmj_player_list');
                this.mj_jiao_info = ui.getComponent('scmj_jiao_info');
    ...
    on_show_result_view: function () {
        ...
        cc.dd.UIMgr.openUI(jlmj_prefab.MJ_ZHANJITONGJI, function (ui) {
            ...
            let title = !RoomMgr.Instance()._Rule.isxueliu ? Text.TEXT_PY_RULE_69 : Text.TEXT_PY_RULE_70;
            fj_arr.push(title.format([RoomMgr.Instance()._Rule.usercountlimit]));
    ...
    update_hall_data: function (msg) {
        var jiesuan_ui = cc.dd.UIMgr.getUI(jlmj_prefab.SCMJ_JIESUAN);
    ...
    onEventMessage: function (event, data) {
        ...
        switch (event) {
            ...
            case DeskEvent.FAPAI://发牌
                this.fapai();
                break;
            case DeskEvent.DUIJU_START://对局开始动画
            ...
            case HallCommonEvent.HALL_NO_RECONNECT_GAME:
                cc.log("scmj  响应------> HallCommonEvent.HALL_NO_RECONNECT_GAME");
            ...
            case DeskEvent.RECOVER_DESK://恢复桌子
                cc.log("scmj 朋 友 recover_desk------ 开始0");
                this.recoverDesk();
                break;
            ...
            case DeskEvent.TIMEUP:
                if (!this._jiesuan && !DeskData.Instance().noCDAudio) {
    ...
    _change2D() {
        ...
        cc.find("Canvas/desk_node/jlmj_player_left_ui").getComponent('scmj_player_left_ui').resetConfig();
        cc.find("Canvas/desk_node/jlmj_player_down_ui").getComponent('scmj_player_down_ui').resetConfig();
        cc.find("Canvas/desk_node/jlmj_player_right_ui").getComponent('scmj_player_right_ui').resetConfig();
        cc.find("Canvas/desk_node/jlmj_player_up_ui").getComponent('scmj_player_up_ui').resetConfig();
    },
