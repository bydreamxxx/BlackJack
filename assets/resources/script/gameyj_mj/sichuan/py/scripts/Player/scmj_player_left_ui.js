...
cc.Class({
    ...
    onLoad: function () {
        cc.log('scmj_player_left_ui onLoad');
        this.zhanshi_pai = cc.find("Canvas/desk_node/zhanshi_pais/jlmj_zhanshi_pai_left").getComponent('jlmj_pai');     //展示牌
        this.node_hu_5 = cc.find("Canvas/desk_node/play_anis/jlmj_xiayu_ani_l");
        this.node_hu_6 = cc.find("Canvas/desk_node/play_anis/jlmj_guafeng_ani_l");
        // cc.find("Canvas/desk_node/jlmj_player_left_ui/mj_playerHead").zIndex = UIZorder.MJ_LAYER_TOP;
        // cc.find("Canvas/desk_node/jlmj_player_left_ui/chat_node").setLocalZOrder(UIZorder.MJ_LAYER_TOP);
        // cc.find("Canvas/desk_node/jlmj_player_left_ui/biaoqing_node").setLocalZOrder(UIZorder.MJ_LAYER_TOP);
        this.chupai_node = cc.find("Canvas/desk_node/chupai_left_node");

        this.score = cc.find("Canvas/desk_node/score_l").getComponent(cc.Label);
        this.score.node.zIndex = UIZorder.MJ_LAYER_TOP;

        this.huNode = cc.find("Canvas/desk_node/hu_l");
        ...
        this.hutype_ani = cc.find("Canvas/desk_node/play_anis/hutype_l").getComponent(cc.Sprite);
    ...
    move3Zhang(data) {
        ...
        //         let mosPos = use2D ? cc.v2(-364, 78 - 23 * j) : cc.v2(-364 - j * 5, 78 - 23 * j);
        //
    ...
    quickHuan3Zhang(choosepaisList, huanpaisList) {
        ...
        for (let i = 0; i < choosePais.length; i++) {
            ...
            if (shoupai) {
                ...
                let mosPos = use2D ? cc.v2(-364, 78 - 23 * i) : cc.v2(-364 - i * 5, 78 - 23 * i);
                shoupai.node.runAction(cc.sequence(
                    cc.moveTo(0.1, mosPos),
                    cc.delayTime(0.3),
                    cc.callFunc(() => {
