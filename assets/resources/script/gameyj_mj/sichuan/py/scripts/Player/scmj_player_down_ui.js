...
cc.Class({
    ...
    onLoad: function () {
        cc.log('scmj_player_down_ui onLoad');
        ...
        this.node_hu_5 = cc.find("Canvas/desk_node/play_anis/jlmj_xiayu_ani_d");
        this.node_hu_6 = cc.find("Canvas/desk_node/play_anis/jlmj_guafeng_ani_d");

        this.touch_node = cc.find("Canvas/desk_node/jlmj_player_down_ui/user_touch_node");
        // cc.find("Canvas/desk_node/jlmj_player_down_ui/chat_node").setLocalZOrder(UIZorder.MJ_LAYER_TOP);
        // cc.find("Canvas/desk_node/jlmj_player_down_ui/biaoqing_node").setLocalZOrder(UIZorder.MJ_LAYER_TOP);
        this.chupai_node = cc.find("Canvas/desk_node/chupai_down_node");

        this.score = cc.find("Canvas/desk_node/score_d").getComponent(cc.Label);
        this.score.node.zIndex = UIZorder.MJ_LAYER_TOP;

        this.huNode = cc.find("Canvas/desk_node/hu_d");
        ...
        this.hutype_ani = cc.find("Canvas/desk_node/play_anis/hutype_d").getComponent(cc.Sprite);
    ...
    updateShouPai: function (player) {
        ...
        for (var i = 0; i < this.shouPai.length; ++i) {
            ...
            if (shoupai_visible_cfg[shouPaiLen] && shoupai_visible_cfg[shouPaiLen].indexOf(i) != -1) {
                ...
                // if(UserPlayer.getPaiType(player.shoupai[count]) == UserPlayer.dingQue){
                //     this._dingQue = true;
                //     this._dingQueList.push(this.shouPai[i]);
                // }
        ...
        if (player.hasMoPai()) {//说明手牌中是包含
            ...
            if (player.state != PlayerState.HUPAI && player.state != PlayerState.TINGPAI && player.state != PlayerState.SC_HUPAI) {
                cc.find("Canvas/game_menu_list").getComponent("scmj_game_menu_list").setMenus(player);
            }

            // if(UserPlayer.getPaiType(player.shoupai[shouPaiLen]) == UserPlayer.dingQue){
            //     this._dingQue = true;
            //     this._dingQueList.push(this.modepai);
            // }
        } else {
            this.modepai.node.active = false;
        }
        ...
        if (this.chupai_prompt) {
            this.chupai_prompt.active = player.hasMoPai() && DeskData.Instance().isGameStart && !playerMgr.Instance().playing_fapai_ani && !player.isBaoTing && !DeskData.Instance().waitDingQue && !this._inHuan3Zhang && player.state != PlayerState.HUPAI && !DeskData.Instance().isFenZhang;
        }

        this.hideShouPaiInFaPaiAction();

        // this._setPaiTouch(this._shouPaiTouch);
        this.checkDingQue();
        ...
        if (this.canCradList && this.canCradList.length > 0 && this.touchCardMode == TouchCardMode.DA_TING_PAI) {
            for (var i = 0; i < this.canCradList.length; ++i) {
                var shoupai = this.getShouPai(this.canCradList[i]);
                if (shoupai) {
                    shoupai.setTouchAble(true);
                    shoupai.setTingPai(true);
                } else {
                    cc.error("【UI】" + "用户玩家中不存在牌:" + pai3d_value.desc[this.canCradList[i]]);
                }
            }
        }

        if (this._inHuan3Zhang && cc.dd._.isArray(this._huan3Zhang_pai) && this._huan3Zhang_pai.length > 0) {
            for (let i = 0; i < this._huan3Zhang_pai.length; i++) {
                let shoupai = this.getShouPai(this._huan3Zhang_pai[i]);
                if (shoupai) {
                    shoupai.selected = true;
                    shoupai.node.y = restPt_y() + chupai_offset;
                }
            }
        }
    ...
    checkDingQue(value) {
        // if(cc.dd._.isNull(value) || cc.dd._.isUndefined(value)){
        //     value = true;
        // }

        if (cc.dd._.isArray(UserPlayer.dingQueList) && UserPlayer.dingQueList.length > 0) {
            ...
            for (let i = 0; i < UserPlayer.dingQueList.length; i++) {
                let jlmj_pai = this.getShouPai(UserPlayer.dingQueList[i]);
                if (jlmj_pai) {
                    jlmj_pai.setTouchAble(true);
                    jlmj_pai.setTouch(true);
                }
            }
            DeskED.notifyEvent(DeskEvent.SHOW_DA_PAI_PROMPT, [3, true]);
        } else {
        ...
        //     this.modepai.setTouchAble(false);
        //
        //     for(let i = 0; i < this._dingQueList.length; i++){
        //         this._dingQueList[i].setTouchAble(true);
        //         this._dingQueList[i].setTouch(true);
        //     }
        //     DeskED.notifyEvent(DeskEvent.SHOW_DA_PAI_PROMPT, [3, true]);
        // }else{
        //     DeskED.notifyEvent(DeskEvent.SHOW_DA_PAI_PROMPT, [-2,false]);
    ...
    touchStart: function (event) {
        ...
        if (DeskData.Instance().waitDingQue) {
            this.resetSelected();
            this.yidong_pai.node.active = false;
            if (this.pai_touched) {
        ...
        if (pai_touched) {
            if (this._inHuan3Zhang) {
                let _idx = this._huan3Zhang_pai.indexOf(pai_touched.cardId);
                if (_idx != -1) {
                    pai_touched.selected = false;
                    pai_touched.node.y = restPt_y();
                    this._huan3Zhang_pai.splice(_idx, 1);
                } else {

                    if (this._huan3Zhang_pai.length > 0 && UserPlayer.getPaiType(this._huan3Zhang_pai[0]) != UserPlayer.getPaiType(pai_touched.cardId)) {
                        this._huan3Zhang_pai = [];
                        this.resetSelected();
                    }

                    pai_touched.selected = true;
                    pai_touched.node.y = restPt_y() + chupai_offset;
                    this._huan3Zhang_pai.push(pai_touched.cardId);
                }
            ...
            if (this.touchCardMode == TouchCardMode.DA_TING_PAI) {
                this.closeJiaoInfo();
                cc.log("scmj touchStart openJiaoInfo");
    ...
    touchMove: function (event) {
        ...
        if (pai_touched) {//找到选择的牌
            ...
            let lastID = -1;
            if (this.pai_touched) {
                lastID = this.pai_touched.cardId
                this.pai_touched.node.active = true;
                this.yidong_pai.node.active = false
            }
            ...
            if (pai_touched && this.touchCardMode == TouchCardMode.DA_TING_PAI && lastID != -1 && pai_touched.cardId != lastID) {
                this.closeJiaoInfo();
                this.openJiaoInfo(pai_touched.cardId);
            }
        }
    ...
    touchEnd: function (event) {
        ...
        if (this._inHuan3Zhang && this.pai_touched) {
            let list = [];
            for (let i = 0; i < this._huan3Zhang_pai.length; i++) {
                list.push({
                    id: this._huan3Zhang_pai[i],
                    type: UserPlayer.getPaiType(this._huan3Zhang_pai[i])
                })
            }
            UserPlayer.setHuan3Zhang(list);
            DeskED.notifyEvent(DeskEvent.CHECK_HUAN_3_ZHANG);
            this.pai_touched = null;
            return;
        }
        ...
        if (!this.pai_touched) {
            ...
            if (this.touchCardMode == TouchCardMode.TING_PAI) {
                this.closeJiaoInfo();
                this.openJiaoInfo(this.pai_touched.cardId);
            }
        }
        ...
        if (this.yidong_pai.node.active) {
            if (this.yidong_pai.node.y > -260 * this._node_scale_y) {
                ...
                if (UserPlayer.dingQueList.length > 0 && UserPlayer.dingQueList.indexOf(this.yidong_pai.cardId) == -1) {
                    this.resetSelected();
                    return;
                }
                ...
                if (UserPlayer.isTempBaoTing) {
                    ...
                    if (UserPlayer.isTempPengTing) {
                        tingType = 4;
                    }
                    if (UserPlayer.isTempGangTing) {
                        tingType = 5;
                    }
            ...
            if (UserPlayer.dingQueList.length > 0 && UserPlayer.dingQueList.indexOf(pai_touched.cardId) == -1) {
                this.resetSelected();
                return;
            }
    ...
    biaojiPai: function (selectId) {
        ...
        for (var i in userList) {
            var com = userList[i].getComponent('scmj_player_down_ui');
    ...
    sendTingPai: function (playCardId, tingType) {
        ...
        //     chi_card.setId(UserPlayer.chi_pai);
        //
        //     var chi_option = UserPlayer.chitingPaiId;
        //     var card_option = [];
        ...
        //     });
        //
        //     var msg = new cc.pb.xuezhanmj.xzmj_req_chiting();
        ...
        //     //return;
        //
        //     cc.gateNet.Instance().sendMsg(cc.netCmd.xuezhanmj.cmd_xzmj_req_chiting, msg, "cmd_xzmj_req_chiting");
        //     cc.dd.NetWaitUtil.net_wait_start();
        // }else if(tingType == 4) {
        //     var msg = new cc.pb.xuezhanmj.xzmj_req_pengting();
        //
        //     msg.setOutcard(out_card);
        //     msg.setIsxiaosa(UserPlayer.isTempXiaosa);
        //
        //     cc.gateNet.Instance().sendMsg(cc.netCmd.xuezhanmj.cmd_xzmj_req_pengting, msg, "cmd_xzmj_req_pengting");
        //     cc.dd.NetWaitUtil.net_wait_start();
        //
        // }else if(tingType == 5) {
        //
        ...
        //     msg.setIsxiaosa(UserPlayer.isTempXiaosa);
        //
        //     cc.gateNet.Instance().sendMsg(cc.netCmd.xuezhanmj.cmd_xzmj_req_tingpai_out_card, msg, "cmd_xzmj_req_tingpai_out_card");
        //     cc.dd.NetWaitUtil.net_wait_start();
        // }
    },
    ...
    sendOutCard: function (cardID) {
        ...
        this.quxiaoBiaoji();
        var msg = new cc.pb.xuezhanmj.xzmj_req_game_send_out_card();
        msg.setCardid(cardID);
        cc.gateNet.Instance().sendMsg(cc.netCmd.xuezhanmj.cmd_xzmj_req_game_send_out_card, msg, "xzmj_req_game_send_out_card");
        cc.dd.NetWaitUtil.net_wait_start();
    },
    ...
    dapai: function (userplayer, chupai_idx_in_shoupai) {
        ...
        cc.log("check chupai idx = ", idx, " view = ", this.viewIdx, " chupai = ", userplayer.chupai.length);
    ...
    openJiaoInfo: function (out_id) {
        ...
        cc.dd.UIMgr.openUI(jlmj_prefab.SCMJ_JIAOPAI_INFO, function (ui) {
            var play_list = cc.find('Canvas/player_list').getComponent('scmj_player_list');
            var mj_jiao_info = ui.getComponent('scmj_jiao_info');
    ...
    closeJiaoInfo: function () {
        cc.dd.UIMgr.closeUI(jlmj_prefab.SCMJ_JIAOPAI_INFO);
    },
    ...
    showDaPaiTing: function (isShow, canCradList, type) {
        ...
        isShow = isShow || false;
        if (isShow && canCradList) {//可以操作canCardList中牌
            this.canCradList = canCradList.slice();
    ...
    clear: function (data) {
        ...
        this.head.ding_que.node.active = false;
        this.head.tips.node.active = false;
    ...
    huan3Zhang: function (paiTips) {
        ...
        this._inHuan3Zhang = true;

        this._huan3Zhang_pai = [];

        for (let i = 0; i < this.shouPai.length; i++) {
            this.shouPai[i].setTouch(true);
        }
        cc.log("Down huan3Zhang");
        this.setShoupaiTingbiaoji(false);
        for (let i = 0; i < paiTips.length; ++i) {
            let shoupai = this.getShouPai(paiTips[i].id);
            if (shoupai) {
                shoupai.selected = true;
                shoupai.node.y = restPt_y() + chupai_offset;
                this._huan3Zhang_pai.push(shoupai.cardId);
            } else {
                cc.error("【UI】" + "用户玩家中不存在牌:" + pai3d_value.desc[paiTips[i]]);
                this._inHuan3Zhang = false;
        ...
        let _tongList = [];
        for (let i = 0; i < this.shouPai.length; i++) {
            switch (UserPlayer.getPaiType(this.shouPai[i].cardId)) {
                case 1:
                    _wanList.push(this.shouPai[i]);
                    break;
                case 2:
                    _tiaoList.push(this.shouPai[i]);
                    break;
                case 3:
                    _tongList.push(this.shouPai[i]);
                    break;
            }
        }

        if (this.modepai && this.modepai.node.active) {
            switch (UserPlayer.getPaiType(this.modepai.cardId)) {
                case 1:
                    _wanList.push(this.modepai);
                    break;
                case 2:
                    _tiaoList.push(this.modepai);
                    break;
                case 3:
                    _tongList.push(this.modepai);
                    break;
            }
        }

        if (_wanList.length < paiTips.length) {
            _wanList.forEach((shoupai) => {
                shoupai.setTouchAble(false);
            })
        }

        if (_tiaoList.length < paiTips.length) {
            _tiaoList.forEach((shoupai) => {
                shoupai.setTouchAble(false);
            })
        }

        if (_tongList.length < paiTips.length) {
            _tongList.forEach((shoupai) => {
                shoupai.setTouchAble(false);
            })
        }
    },
    ...
    change3Zhang(data) {
        ...
        //     let huanPai = huanPais[i];
        //     if (shoupai) {
        //         let pos = shoupai.node.position;
        //         let originPos = this._changePos[choosePais[i].id];
        //
        ...
        //                     if(_use2D){
        //                         shoupai.frame.spriteFrame = shoupai.atlas_2d.getSpriteFrame("ed_shoupai_ziji_li");
        //                     }else{
        //                         shoupai.frame.spriteFrame = shoupai.atlas.getSpriteFrame("d_shoupai_xia_dao_zhengmian");
        //                     }
        //
        //                     shoupai.setValue(huanPai.id);
        //                     shoupai.value.node.active = true;
        //                     shoupai.node.position = originPos;
        //                 }
        //             }),
        //             // cc.delayTime(1),
        //             // cc.moveTo(0.1, originPos)
        //         ));
        //     }
        // }
        this._huan3Zhang_pai = [];
        this.node.runAction(cc.sequence(
            ...
            cc.callFunc(() => {
                ...
                for (let i = 0; i < huanPais.length; i++) {
                    let shoupai = this.getShouPai(huanPais[i].id);
                    if (shoupai) {
                        shoupai.node.y += 25;
                    }
                }
            ...
            cc.callFunc(() => {
                if (this.node && this.node.isValid) {
                    this._inHuan3Zhang = false;
                    this._huan3Zhang_pai = null;
                    this._changePos = null;

                    var menu_list = cc.find("Canvas/scmj_huan3zhang");
                    if (menu_list) {
                        menu_list.getComponent("scmj_huan3zhang").huan3ZhangHide();
                    }
    ...
    move3Zhang(data) {
        ...
        //     if (shoupai) {
        //         let pos = shoupai.node.position;
        //         this._changePos[choosePais[i].id] = pos;
        //
        //         shoupai.value.node.active = false;
        //
        //         if(_use2D){
        //             shoupai.frame.spriteFrame = shoupai.atlas_2d.getSpriteFrame("ed_penpai_jifang_kou");
        //             shoupai.node.width = 97;
        //             shoupai.node.height = 138;
        //         }else{
        //             shoupai.frame.spriteFrame = shoupai.atlas.getSpriteFrame("c_shoupai_xia_beimian");
        //             shoupai.node.width = 90;
        //             shoupai.node.height = 131;
        //         }
        //
        //         shoupai.setTouch(false);
        //         shoupai.node.runAction(cc.moveTo(0.1, (i-1)*90, -142));
        //     }
        // }
        this._huan3Zhang_pai = [];
        this.updateShouPai();
    ...
    //  */
    // setPaiTouch(value){
    //     this._shouPaiTouch = value;
    // },

    // _setPaiTouch(value){
    //     if(cc.dd._.isNull(value) || cc.dd._.isUndefined(value)){
    //         this.checkDingQue(true);
    //         return;
    //     }
    //     this.shouPai.forEach(function (pai) {
    //         pai.setTouch(value);
    //     });
    //
    //     this.modepai.setTouch(value);
    //
    //     this.checkDingQue(value);
    // },
        ...
        pais.forEach(function (jlmj_pai, idx) {
            ...
            if (baipai.type == BaipaiType.PENG) {
                if (showIndex == idx) {
                    ...
                    if (baipai.viewIdx == 0) {
                        jiantou.rotation = 0;
                        jiantou.scaleX = jlmj_pai.node.scaleX > 0 ? 1.1 : -1.1;
                        jiantou.scaleY = 1.1;
                    } else if (baipai.viewIdx == 1) {
                        jiantou.rotation = -90;
                        jiantou.scaleX = 1.1;
                        jiantou.scaleY = jlmj_pai.node.scaleX > 0 ? 1.1 : -1.1;
