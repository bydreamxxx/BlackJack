...
var PlayerHead = cc.Class({
    ...
    properties: {
        ...
        tiao: cc.SpriteFrame,
        tong: cc.SpriteFrame,
        dingque: cc.SpriteFrame,
        huan3zang: cc.SpriteFrame,
        waiting: cc.SpriteFrame,
        tips: cc.Sprite,
    },
    ...
    onLoad: function () {
        ...
        this.ding_que = cc.find('head_node/dingque', this.node).getComponent(cc.Sprite);

        this.setTips(false);
        this.ding_que.node.active = false;
    ...
    initUI: function (player) {
        ...
        if (player.coin == 0 && DeskData.Instance().isJBC() && !this.pochanNode.node.active) {
    ...
    playMagicProp: function (idx, fromId, toId) {
        ...
        let heads = cc.find("Canvas").getComponentsInChildren("scmj_playerHead");
    ...
    setPoChan: function (isshow) {
        ...
        if (RoomMgr.gameId != cc.dd.Define.GameType.XZMJ_FRIEND && RoomMgr.gameId != cc.dd.Define.GameType.XLMJ_FRIEND) {
    ...
    setDingQue(type) {
        switch (type) {
            case 1:
                this.ding_que.spriteFrame = this.wan;
                break;
            case 2:
                this.ding_que.spriteFrame = this.tiao;
                break;
            case 3:
                this.ding_que.spriteFrame = this.tong;
                break;
        }
        let issanfang = RoomMgr.Instance()._Rule && RoomMgr.Instance()._Rule.usercountlimit != 4 && !RoomMgr.Instance()._Rule.issanfang ? false : true
        this.ding_que.node.active = (type == 1 || type == 2 || type == 3) && issanfang;
    },

    setTips(value) {
        if (value) {
            if (value == 'huan3zhang') {
                this.tips.spriteFrame = this.huan3zang;
            } else if (value == 'dingque') {
                this.tips.spriteFrame = this.dingque;
            } else if (value == 'waiting') {
                this.tips.spriteFrame = this.waiting;
            }
