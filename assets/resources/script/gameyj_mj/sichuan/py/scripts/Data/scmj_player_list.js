...
cc.Class({
    ...
    properties: {
        player_ui_arr: { default: [], type: [require('scmj_player_base_ui')], tooltip:"玩家ui 顺序为下右上左", override: true},
    },
    initUI: function () {
    ...
    playerExit: function (player) {
        ...
        } else if( RoomMgr.Instance().gameId == cc.dd.Define.GameType.XZMJ_FRIEND ||  RoomMgr.Instance().gameId == cc.dd.Define.GameType.XLMJ_FRIEND) {
            if( player.userId === cc.dd.user.id ) {
                // 返回俱乐部
                scmj_util.enterHall();
            }
