...
var SCMJUserPlayerData = cc.Class({
    ...
    statics: {
        ...
        Instance: function () {
            if(!this.s_userPlayer){
                this.s_userPlayer = new SCMJUserPlayerData();
            }
    ...
    mopai: function (msg) {
        ...
        if(this.getPaiType(card.id) == this.dingQue && this.dingQueList.indexOf(card.id) == -1){
            this.dingQueList.push(card.id);
        }
        ...
        if(this.cangang || this.canbugang){
            ...
            if(!DeskData.Instance().hasRemainPai()){
                ...
                if( gangOption.cardList.length == 1 ) {
                    ...
                    const gang_Analysts = require('scmj_pai_analysts');
        ...
        if(msg.chitingjiaoinfosList){
            ...
            for(let i = 0, len = msg.chitingjiaoinfosList.length; i < len; ++i){
                ...
                for(let id = 0, len = chijiao_info.length; id < len; ++id){
                    ...
                    jiaoInfo_list.push(new JiaoInfo(out_id,jiaoPai_list,jiaoInfo.angang));
                }
                this.chi_jiaoInfo_list.push(jiaoInfo_list);
            }
        }
    ...
    setDapaiTing:function (msg) {
        if(!msg.jiaoinfosList.length || this.state == PlayerState.TINGPAI){
            return;
        }
        this.jiaoInfo_list = [];
        var list = [];
        ...
        cc.log("【数据】"+"玩家:"+this.userId+" 座位号:"+this.idx+" 打牌后可以下叫");
    },
});

module.exports = SCMJUserPlayerData;
