...
var PlayerMgr = cc.Class({
    ...
    ctor: function () {
        ...
        this.require_PlayerData = require(mjComponentValue.playerData).PlayerData;
    ...
    updatePlayerNum: function (num) {
        ...
        this.require_UserPlayer.clear();
        cc.log("清空玩家列表 初始化");
        if(this.isFriend()){    //朋友场
    ...
    playerEnter: function (role_info) {
        ...
        if(role_info.userId == cc.dd.user.id){
            var player = this.require_UserPlayer;
        }else{
            var player = this.getPlayer(role_info.userId);
            if(!player){
                player = new this.require_PlayerData();
            }
        }
        player.clear();
        player.setBaseData(role_info);
        if(this.isFriend()){
            player.coin = this.getDefaultFriendScore();
        }
        //设置是否在线
        ...
        if(player.isUserPlayer()){
            ...
            this.setThreeSeat(player);
        }else {
            //四人麻将 视觉座位号
            if(player.idx>this.require_UserPlayer.idx){
                player.viewIdx = player.idx - this.require_UserPlayer.idx;
            }else{
                player.viewIdx = 3 - this.require_UserPlayer.idx + player.idx + 1;
            }
        }
    ...
    gamePlayerEnter: function (playerMsg) {
        ...
        if(playerMsg.userid == cc.dd.user.id){
            var pd = this.require_UserPlayer;
        }else{
            var pd = this.getTempBasePlayer(playerMsg.userid);
            if(!pd){
                pd = new this.require_PlayerData();
            }
        }
        ...
        if(pd.isUserPlayer()){
            ...
            this.setThreeSeat(pd);
        }else {
            //四人麻将 视觉座位号
            if(pd.idx>this.require_UserPlayer.idx){
                pd.viewIdx = pd.idx - this.require_UserPlayer.idx;
            }else{
                pd.viewIdx = 3 - this.require_UserPlayer.idx + pd.idx + 1;
            }
        }
    ...
    requireDeskData:function(){
        this.DeskED = require(mjComponentValue.deskData).DeskED;
        this.DeskEvent = require(mjComponentValue.deskData).DeskEvent;
    },
    ...
    //三人麻将 视觉座位号
    setThreeSeat(player){
        var offset = this.require_UserPlayer.idx - player.idx;
        if(offset == -1){
            player.viewIdx = 1;
    ...
    getDefaultFriendScore(){
        cc.log("-----------------------no implements base_mj_player_mgr getDefaultFriendScore-----------------------")
        return 0;
    },

    isFriend(){
        cc.log("-----------------------no implements base_mj_player_mgr isFriend-----------------------")
        return false;
    },

    initMJComponet(){
        cc.log("-----------------------no implements base_mj_player_mgr initMJComponet-----------------------")
