...
var PlayerEvent = cc.Enum({
    ...
    LIANGXISTATE:            'fzmj_liangxi_state',                       //方正麻将亮喜状态
    BUHUA:                   'wdmj_buhua',                               //乌丹麻将补花
    SHOW_BU_HUA:             'wdmj_show_buhua',                          //乌丹麻将显示补花
    SHOW_TING_TIPS:          'hlmj_show_ting_tips',                      //和龙麻将显示听牌提示
    SHOW_HUANG_ZHUANG_TIPS:          'hlmj_show_haung_zhuang_tips',      //和龙麻将显示黄庄提示
});
...
var PlayerData = cc.Class({
    ...
    ctor: function () {
        ...
        this.require_PlayerEvent = PlayerEvent;
        this.require_PlayerED = PlayerED;
        this.require_PlayerState = PlayerState;
    ...
    transBaiPaiType: function( baipaiType ) {
        switch( baipaiType ) {
            ...
            case BAIPAI_TYPE.C_BUHUA:
                return BaipaiType.BUHUA;
            default:
                break;
        }
    },
    ...
    mopai: function (msg) {
        ...
        if(this._isUserPlayer){
            ...
            //   this.require_PlayerED.notifyEvent(this.require_PlayerEvent.AUTO_CHU_PAI,[this, this.modepai]);
    ...
    dapai: function (id, isQuick) {
        ...
        this.isQuick = cc.dd._.isBoolean(isQuick) ? isQuick : true;
        ...
        if(this._isUserPlayer){
            ...
            this.paixu();
            this.chupai.push(id);
            cc.log("【数据】"+"玩家:"+this.userId+" 座位号:"+this.idx+" 打牌:" + pai3d_value.desc[id]+" 快出牌:"+this.isQuick);
     ...
     * 补花
     */
    buhua: function (buhuaID) {
        if(!cc.dd._.isNumber(buhuaID) || buhuaID < 0){
            cc.error("【数据】"+"补花牌为空");
            return;
        }

        this.delShouPai([buhuaID],1);
        cc.log("【数据】"+"玩家:"+this.userId+" 座位号:"+this.idx+" 补花:"+pai3d_value.descs([buhuaID]));

        if(cc.dd._.isNull(this.buhua_data)){
            this.buhua_data = new BaipaiData();
            this.buhua_data.index = this.baipai_data_list.length;

            this.buhua_data.mj_index = 0;
            this.buhua_data.type = BaipaiType.BUHUA;
            this.buhua_data.cardIds = [buhuaID];
        }else{
            this.buhua_data.cardIds.push(buhuaID);
        }

        cc.log("【数据】"+"玩家:"+this.userId+" 座位号:"+this.idx+" 摆牌:");
        cc.log(this.buhua_data.toString());


        this.resetBaiPaiIndex();
        PlayerED.notifyEvent(PlayerEvent.BUHUA,[this,this.buhua_data]);
    },
    ...
    gang: function (msg) {
        switch (msg.gangtype) {
            ...
            case 9: //一杠
                ...
                break;
            case 18:
                this.angang_ming(msg.gangcardList, true);    //暗杠
                break;
            case 19:
     ...
     * 已开门暗杠，展示牌值
     */
    angang_ming: function (gangcardList, play_audio) {
        if(!gangcardList){
            cc.error("【数据】"+"开门暗杠杠牌列表为空");
            return;
        }
        let shoupaiGangCount = 0;
        var gangIds = [];
        gangcardList.forEach(function (card) {
            if(card){
                ...
                cc.error("【数据】"+"开门暗杠杠牌列表项为空");
            }
        }.bind(this));

        if(gangIds.length != 4){
            cc.error("【数据】"+"开门暗杠杠牌数目 != 4");
            return;
        }
        if(shoupaiGangCount >= 4){
            cc.error("【数据】"+"开门暗杠手牌数目不对"+shoupaiGangCount);
            return;
        }
        this.delShouPai(gangIds, 4);
        cc.log("【数据】"+"玩家:"+this.userId, '开门暗杠杠列表',pai3d_value.descs(gangIds));
        var baipai_data = new BaipaiData();
        baipai_data.type = BaipaiType.DIANGANG;
        baipai_data.down_pai_num = 3;
        baipai_data.cardIds = gangIds;
    ...
    kaipai: function(holdCardList, mopai, hupaiId, duibao) {
        ...
        cc.log("【数据】" + "玩家:" + this.userId + " 座位号:" + this.idx + " 开牌" + pai3d_value.descs(this.shoupai));
    ...
    setCpgth: function (msg, isGangTing) {
        this.setCtrlStatus(msg, isGangTing);
        this.setCPGTFunc(msg);
        ...
        if(this.cangang || this.canbugang){
            ...
            if(!DeskData.Instance().hasRemainPai()){
                ...
                if( gangOption.cardList.length == 1 ) {
                    ...
                    const gang_Analysts = require(mjComponentValue.paiAnalysts);
                    gang_Analysts.Instance().getAllGangGroup(gangOption.cardList, gangOptionList, havexiaoji, this.isGangTing);
    ...
    guo: function () {
        ...
        this.isTempBaoTing = false;
        this.require_PlayerED.notifyEvent(this.require_PlayerEvent.GUO,[this]);
    },
    ...
    setJiaoPaiMsg:function (jiaopai) {
        ...
        for(let  i = 0; i < jiaopai.length; i++){
    ...
    diffHoldCard: function( data ) {
        ...
        if( serverShouPai.length != clientShouPai.length ) {
            ...
            // dd.DialogBoxUtil.show( 999, "服务器的牌 和 本地手牌 长度不同");
            return ;
        }
        for( var i = 0; i < serverShouPai.length; ++i ) {
            ...
            if( !flag ) {
                // dd.DialogBoxUtil.show( 999, "没有找到id = " + serverShouPai[i].id + " 的服务器数据");
                return;
            }
        }
    },
     ...
     */
    hasCaozuo: function () {
        cc.log("-----------------------no implements base_mj_player_data hasCaozuo-----------------------")
        return false;
    },
    ...
    clearCtrlStatus: function() {
        cc.log("-----------------------no implements base_mj_player_data clearCtrlStatus-----------------------")
    },
    ...
    setCtrlStatus: function(msg, isGangTing) {
        cc.log("-----------------------no implements base_mj_player_data setCtrlStatus-----------------------")
    },
     ...
     * @param msg
     */
    setCPGTFunc: function(msg) {
        cc.log("-----------------------no implements base_mj_player_data setCHIPENGGANGTING-----------------------")
    },

    initMJComponet(){
        cc.log("-----------------------no implements base_mj_player_data initMJComponet-----------------------")
