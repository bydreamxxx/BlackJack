...
var userList = cc.Class({
    ...
    properties: {
        player_ui_arr: { default: [], type: [require('base_mj_player_base_ui')], tooltip:"玩家ui"},
    },
    ...
    initUI: function () {
        this.require_playerMgr.Instance().playerList.forEach(function(player){
            if(player){
    ...
    playerExit: function (player) {
        ...
        this.require_DeskData.Instance().closePopTips();
    ...
    removeAllPlayer: function() {
        for( var i = 0; i < this.require_playerMgr.Instance().playerList.length; ++i ) {
    ...
    updatePlayerList: function( playerList ) {
        this.require_DeskData.Instance().isGameStart = false;
        this.removeAllPlayer();
        this.initUI();
    },

    playerNumChanged:function(){
        var num = this.require_playerMgr.Instance().playerList.length;
        this.require_DeskED.notifyEvent(this.require_DeskEvent.UPDATE_PLAYER_NUM,num);
        cc.log('更新玩家数量:'+num);
    },
            ...
            for (var i = 0; i < gpsList.length - 1; i++) {
                for (var j = i; j < gpsList.length - 1; j++) {
                    if (this.getDistance(gpsList[i].location, gpsList[j + 1].location) < 100) {
                        this.player_ui_arr[gpsList[i].viewIdx].head.gps_warn.active = true;
                        this.player_ui_arr[gpsList[j + 1].viewIdx].head.gps_warn.active = true;
                    }
                }
            }
        }
    ...
    closeGPSWarn: function () {
        ...
        this.player_ui_arr.forEach(function (player_ui) {
            player_ui.head.gps_warn.active = false;
        });
    },
    ...
    on_player_location_change: function(msg) {
        ...
        if (UI) {
            UI.getComponent('user_info_view').setGpsData(this.require_playerMgr.Instance().playerList);
        }
    },
    ...
    onEventMessage:function (event,data) {
        ...
        switch (event){
            case this.require_PlayerEvent.ENTER:
                this.playerEnter(player);
                this.playerNumChanged();
                break;
            case this.require_PlayerEvent.EXIT:
                this.playerExit(player);
                break;
            case RoomEvent.room_recover:
                this.updatePlayerList();
                break;
            case this.require_DeskEvent.BIAOJI_BAOPAI:
    ...
    isClub(){
        cc.log("-----------------------no implements base_mj_player_list isClub-----------------------")
        return false;
    },

    initMJComponet(){
        cc.log("-----------------------no implements base_mj_player_list initMJComponet-----------------------")
