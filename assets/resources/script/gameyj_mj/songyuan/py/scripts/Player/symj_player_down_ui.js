...
cc.Class({
    extends: require('symj_player_base_ui'),
    ...
    onLoad: function () {
        cc.log('symj_player_down_ui onLoad');
    ...
    updateShouPai: function (player) {
        ...
        if (player.hasMoPai()) {//说明手牌中是包含
            ...
            if (player.state != PlayerState.HUPAI && player.state != PlayerState.TINGPAI) {
                cc.find("Canvas/game_menu_list").getComponent("symj_game_menu_list").setMenus(player);
            }
        } else {
            this.modepai.node.active = false;
        }
    ...
    touchStart: function (event) {
        ...
        if (pai_touched) {
            ...
            if (this.touchCardMode == TouchCardMode.DA_TING_PAI) {
                this.closeJiaoInfo();
                cc.log("symj touchStart openJiaoInfo");
    ...
    biaojiPai: function (selectId) {
        ...
        for (var i in userList) {
            var com = userList[i].getComponent('symj_player_down_ui');
    ...
    sendTingPai: function (playCardId, tingType) {
        ...
        if (tingType == 3) {
            ...
            var msg = new cc.pb.songyuanmajiang.sy_req_chiting();
            ...
            cc.gateNet.Instance().sendMsg(cc.netCmd.songyuanmajiang.cmd_sy_req_chiting, msg, "cmd_sy_req_chiting");
            cc.dd.NetWaitUtil.net_wait_start();
        } else if (tingType == 4) {
            var msg = new cc.pb.songyuanmajiang.sy_req_pengting();

            msg.setOutcard(out_card);
            msg.setIsxiaosa(UserPlayer.isTempXiaosa);

            cc.gateNet.Instance().sendMsg(cc.netCmd.songyuanmajiang.cmd_sy_req_pengting, msg, "cmd_sy_req_pengting");
            cc.dd.NetWaitUtil.net_wait_start();
            ...
            cc.gateNet.Instance().sendMsg(cc.netCmd.songyuanmajiang.cmd_sy_req_tingpai_out_card, msg, "cmd_sy_req_tingpai_out_card");
            cc.dd.NetWaitUtil.net_wait_start();
        }
    },
    ...
    sendOutCard: function (cardID) {
        ...
        this.quxiaoBiaoji();
        var msg = new cc.pb.songyuanmajiang.sy_req_game_send_out_card();
        msg.setCardid(cardID);
        cc.gateNet.Instance().sendMsg(cc.netCmd.songyuanmajiang.cmd_sy_req_game_send_out_card, msg, "sy_req_game_send_out_card");
        cc.dd.NetWaitUtil.net_wait_start();
    },
    ...
    ting: function (player) {
        ...
        this.closeJiaoInfo();
        var desk_info = cc.find('Canvas/desk_info').getComponent('symj_desk_info');
