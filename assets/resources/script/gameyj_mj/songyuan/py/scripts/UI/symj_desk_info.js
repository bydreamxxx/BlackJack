...
var baseDeskInfo = cc.Class({
    ...
    initLocalData:function() {
        ...
        //桌布
        var json = cc.sys.localStorage.getItem('symj_zhuobu_set_' + cc.dd.user.id);
        if (json) {
            var sprite = null;
            ...
            if (sprite) {
                ...
                cc.sys.localStorage.setItem('symj_zhuobu_set_' + cc.dd.user.id, this.zhuomianImg[0]._name);
            }
        }
        else {
            this.deskImage.spriteFrame = this.zhuomianImg[0];
    ...
    initGuiZeInfo:function () {
        ...
        var cc_txt = RoomMgr.Instance()._Rule.symjtype==1?dd.Text.TEXT_PY_RULE_45:dd.Text.TEXT_PY_RULE_44;
        cur_rule.roomId = RoomMgr.Instance().roomId;

        this.gz_arr_box = [];
        this.gz_arr_box.push(cc_txt.format([RoomMgr.Instance()._Rule.usercountlimit]));
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.is37jia?       Text.TEXT_PY_RULE_46:'');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.issiqing?      Text.TEXT_PY_RULE_47:'');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.iskuaibao?     Text.TEXT_PY_RULE_48:'');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.iskandb?       Text.TEXT_PY_RULE_49:'');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isganghua?     Text.TEXT_PY_RULE_50:'');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.istongbaofb?   Text.TEXT_PY_RULE_51:'');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isbujiabuhu?   Text.TEXT_PY_RULE_52:'');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.iszhanlihu?   Text.TEXT_PY_RULE_71:'');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.ispiaohuduan19?   Text.TEXT_PY_RULE_72:'');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isqingyise?   Text.TEXT_PY_RULE_29:'');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isbutingdpbsj? Text.TEXT_PY_RULE_53:'');
        ...
        this.gz_arr_info.push({str:cc_txt.format([RoomMgr.Instance()._Rule.usercountlimit]),nodetype:0});
        this.gz_arr_info.push({str:juquan_txt.format([RoomMgr.Instance()._Rule.boardscout]),nodetype:0});
        this.gz_arr_info.push({str:RoomMgr.Instance()._Rule.is37jia?       Text.TEXT_PY_RULE_46:'',nodetype:1});
        this.gz_arr_info.push({str:RoomMgr.Instance()._Rule.issiqing?      Text.TEXT_PY_RULE_47:'',nodetype:1});
        this.gz_arr_info.push({str:RoomMgr.Instance()._Rule.iskuaibao?     Text.TEXT_PY_RULE_48:'',nodetype:1});
        this.gz_arr_info.push({str:RoomMgr.Instance()._Rule.iskandb?       Text.TEXT_PY_RULE_49:'',nodetype:1});
        this.gz_arr_info.push({str:RoomMgr.Instance()._Rule.isganghua?     Text.TEXT_PY_RULE_50:'',nodetype:1});
        this.gz_arr_info.push({str:RoomMgr.Instance()._Rule.istongbaofb?   Text.TEXT_PY_RULE_51:'',nodetype:1});
        this.gz_arr_info.push({str:RoomMgr.Instance()._Rule.isbujiabuhu?   Text.TEXT_PY_RULE_52:'',nodetype:1});
        this.gz_arr_info.push({str:RoomMgr.Instance()._Rule.iszhanlihu?   Text.TEXT_PY_RULE_71:'',nodetype:1});
        this.gz_arr_info.push({str:RoomMgr.Instance()._Rule.ispiaohuduan19?   Text.TEXT_PY_RULE_72:'',nodetype:1});
        this.gz_arr_info.push({str:RoomMgr.Instance()._Rule.isqingyise?   Text.TEXT_PY_RULE_29:'',nodetype:1});
        this.gz_arr_info.push({str:RoomMgr.Instance()._Rule.isbutingdpbsj? Text.TEXT_PY_RULE_53:'',nodetype:1});
    ...
    initZhiNan:function () {
        if(this.zhinan.active && DeskData.Instance().zhiNan_site != null){
            ...
            this.zhinan.getComponent("symj_zhinan_ui").initsetZhiNan(site);
    ...
    recoverDesk: function () {
        cc.log("symj 朋 友 recover_desk------ 接收1");
        cc.log("------恢复桌子------py");
        this.updateDesk();
        DeskData.Instance().enterSceneRecoverDesk(this.sendReadyOK.bind(this));
        if (this.fenzhang) {
    ...
    sendReadyOK: function () {
        ...
        var msg = new cc.pb.songyuanmajiang.sy_req_reloading_ok();
        cc.gateNet.Instance().sendMsg(cc.netCmd.songyuanmajiang.cmd_sy_req_reloading_ok,msg,"sy_req_reloading_ok");
        this.initGuiZeInfo();
    },
    ...
    jiesuan:function (data) {
        ...
        this.jiesuan_TimeID = setTimeout(function () {
            ...
            if (cc.director.getScene().name != 'symj_py_game' ||
                cc.director.getScene().name != 'symj_jbc_game') {
                cc.dd.UIMgr.destroyUI(jlmj_prefab.JLMJ_JIAOPAI_INFO);
    ...
    leave_tips: function (data) {
        ...
        if(DeskData.Instance().isFriend()){//朋友场
            ...
            }else if(symj_desk_data_jbc.getInstance().getIsMatching() || DeskData.Instance().player_read_gamne){//取消匹配状态
                txt = Text.TEXT_LEAVE_ROOM_3;
                callfunc = this.jbc_matching_leave_room_req;
            }else if( symj_desk_data_jbc.getInstance().getIsStart()){//游戏中退出游戏
    ...
    py_dissolve_room_req:function() {
        ...
        var msg = new cc.pb.songyuanmajiang.sy_req_sponsor_dissolve_room();
        msg.setSponsorid(dd.user.id);
        cc.gateNet.Instance().sendMsg(cc.netCmd.songyuanmajiang.cmd_sy_req_sponsor_dissolve_room,msg,"sy_req_sponsor_dissolve_room");
    },
        ...
        if( msg.retCode !== 0 ) {
            ...
            switch (msg.retCode) {
                case 1:
                    ...
                    this.popupEnterHall( str, symj_util.enterHall.bind(symj_util) );
                    break;
                case 2:
    ...
    py_ready:function () {
        ...
        var msg = new cc.pb.songyuanmajiang.sy_req_ready();
        cc.gateNet.Instance().sendMsg(cc.netCmd.songyuanmajiang.cmd_sy_req_ready,msg,"sy_req_ready");
    },
    ...
    update_player_info:function () {
        ...
        this.zhinan.getComponent("symj_zhinan_ui").initDirection();
        var play_list = cc.find('Canvas/player_list').getComponent('symj_player_list');
    ...
    getTextByWanFa: function() {
        ...
        this.gz_arr_box = [];
        this.gz_arr_box.push(cc_txt.format([RoomMgr.Instance()._Rule.usercountlimit]));
        RoomMgr.Instance()._Rule.is37jia?       this.gz_arr_box.push(Text.TEXT_PY_RULE_46):'';
        RoomMgr.Instance()._Rule.issiqing?      this.gz_arr_box.push(Text.TEXT_PY_RULE_47):'';
        RoomMgr.Instance()._Rule.iskuaibao?     this.gz_arr_box.push(Text.TEXT_PY_RULE_48):'';
        RoomMgr.Instance()._Rule.iskandb?       this.gz_arr_box.push(Text.TEXT_PY_RULE_49):'';
        RoomMgr.Instance()._Rule.isganghua?     this.gz_arr_box.push(Text.TEXT_PY_RULE_50):'';
        RoomMgr.Instance()._Rule.istongbaofb?   this.gz_arr_box.push(Text.TEXT_PY_RULE_51):'';
        RoomMgr.Instance()._Rule.isbujiabuhu?   this.gz_arr_box.push(Text.TEXT_PY_RULE_52):'';
        RoomMgr.Instance()._Rule.iszhanlihu?    this.gz_arr_box.push(Text.TEXT_PY_RULE_71):'';
        RoomMgr.Instance()._Rule.ispiaohuduan19?   this.gz_arr_box.push(Text.TEXT_PY_RULE_72):'';
        RoomMgr.Instance()._Rule.isqingyise?    this.gz_arr_box.push(Text.TEXT_PY_RULE_29):'';
        RoomMgr.Instance()._Rule.isbutingdpbsj? this.gz_arr_box.push(Text.TEXT_PY_RULE_53):'';
    ...
    on_show_result_view: function () {
        ...
        cc.dd.UIMgr.openUI(jlmj_prefab.MJ_ZHANJITONGJI, function(ui) {
            var fj_arr = [];
            var juquan_txt = RoomMgr.Instance()._Rule.usercountlimit==2?Text.TEXT_PY_RULE_10:Text.TEXT_PY_RULE_11;
            fj_arr.push(Text.TEXT_PY_RULE_46.format([RoomMgr.Instance()._Rule.usercountlimit]));
    ...
    onEventMessage: function (event, data) {
        ...
        switch (event) {
            ...
            case HallCommonEvent.HALL_NO_RECONNECT_GAME:
                cc.log("symj  响应------> HallCommonEvent.HALL_NO_RECONNECT_GAME");
            ...
            case DeskEvent.RECOVER_DESK://恢复桌子
                cc.log("symj 朋 友 recover_desk------ 开始0");
                this.recoverDesk();
                break;
    ...
    _change2D(){
        ...
        cc.find("Canvas/desk_node/jlmj_player_left_ui").getComponent('symj_player_left_ui').resetConfig();
        cc.find("Canvas/desk_node/jlmj_player_down_ui").getComponent('symj_player_down_ui').resetConfig();
        cc.find("Canvas/desk_node/jlmj_player_right_ui").getComponent('symj_player_right_ui').resetConfig();
        cc.find("Canvas/desk_node/jlmj_player_up_ui").getComponent('symj_player_up_ui').resetConfig();
    },
