...
cc.Class({
    ...
    onLoad: function () {
        ...
        this.p_wf[9] = cc.find('gameScrollView/view/content/toggle_group_wanfa/layout/toggle10', this.node).getComponent(cc.Toggle);
        ...
        switch (data) {
            case "rs1":
                ...
                var conf = cost_room_cards_config.getItemList(function (item) {
                    return item.id == cc.dd.Define.GameType.SYMJ_FRIEND && item.player_num == 4;
                }.bind(this));
            ...
            case "rs2":
                ...
                var conf = cost_room_cards_config.getItemList(function (item) {
                    return item.id == cc.dd.Define.GameType.SYMJ_FRIEND && item.player_num == 3;
                }.bind(this));
            ...
            case "rs3":
                ...
                var conf = cost_room_cards_config.getItemList(function (item) {
                    return item.id == cc.dd.Define.GameType.SYMJ_FRIEND && item.player_num == 2;
                }.bind(this));
    ...
    onCreate: function (event, custom) {
        /************************游戏统计 start************************/
        cc.dd.Utils.sendClientAction(cc.dd.clientAction.SYMJ_KT_FRIEND, cc.dd.clientAction.T_HALL.CREATE_ROOM);
        /************************游戏统计   end************************/
    ...
    sendCreateRoom: function (data, isProxy) {
        ...
        cc.sys.localStorage.setItem(cc.dd.user.id + '_symj_kt_create', JSON.stringify(data));
    ...
    initData: function () {
        ...
        this.m_objRuleIndex = {
            ...
            rs: 4,
            // 玩法
            wf: [true, true, true, true, true, true, true, true, true, true],
            // 防作弊
            fzb: true,
            //俱乐部限定
            club: false,
        ...
        let result = cc.sys.localStorage.getItem(cc.dd.user.id + '_symj_kt_create');
    ...
    getRules(data, isProxy){
        ...
        var conf = cost_room_cards_config.getItemList(function (item) {
            return item.id == cc.dd.Define.GameType.SYMJ_FRIEND && item.player_num == this.m_objRuleIndex.rs;
        }.bind(this));

        qsOrJs = conf[this.m_objRuleIndex.qs].circle_num

        var gameReq = new cc.pb.room_mgr.msg_create_game_req();
        var pbData = new cc.pb.room_mgr.sy_req_createdesk();
        ...
        pbData.setSymjtype(1);

        pbData.setIs37jia(data.wf[0]);
        pbData.setIssiqing(data.wf[1]);
        pbData.setIskuaibao(data.wf[2]);
        pbData.setIskandb(data.wf[3]);
        pbData.setIsganghua(data.wf[4]);
        pbData.setIstongbaofb(data.wf[5]);
        pbData.setIsbutingdpbsj(data.wf[6]);
        pbData.setIszhanlihu(data.wf[7]);
        pbData.setIspiaohuduan19(data.wf[8]);
        pbData.setIsqingyise(data.wf[9]);

        pbData.setGps(data.fzb);
        pbData.setIsyuyin(data.yy);

        commonGame.setGameType(62);
        if (isProxy) {
    ...
    setRoomRule(rule){
        ...
        let _rule = rule.rule.mjSongyuanRule;

        let qsOrJs = 0;
        var conf = cost_room_cards_config.getItemList(function (item) {
            return item.id == cc.dd.Define.GameType.SYMJ_FRIEND && item.player_num == _rule.usercountlimit;
        }.bind(this));
        ...
        this.m_objRuleIndex = {
            ...
            // 玩法
            wf: [_rule.is37jia, _rule.issiqing, _rule.iskuaibao, _rule.iskandb, _rule.isganghua, _rule.istongbaofb, _rule.isbutingdpbsj, _rule.iszhanlihu, _rule.ispiaohuduan19, _rule.isqingyise],
            // 防作弊
            fzb: _rule.gps,
            //俱乐部限定
            club: _rule.gps,
