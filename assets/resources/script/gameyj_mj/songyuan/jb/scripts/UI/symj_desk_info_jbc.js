var baseDeskInfo = require( "symj_desk_info" );
...
var baseDeskInfoJB = cc.Class({
    ...
    onLoad:function () {
        cc.log("symj_desk_info_jbc onLoad");
        this._super();
        this.initDeskDataUI();
        this.initDeskData();
    },
    ...
    initDeskData: function() {
        var deskDataJBC= symj_desk_data_jbc.getInstance();
    ...
    start: function () {
        ...
        if( symj_desk_data_jbc.getInstance().getIsReconnect() ) {
    ...
    onDestroy: function() {
        ...
        symj_desk_data_jbc.getInstance().destroy();
        this._super();
        RoomMgr.Destroy();
    },
    ...
    on_room_replace:function (msg) {
        if( msg.retCode !== 0 ) {
            ...
            this.playMatchAnim();
            symj_desk_data_jbc.getInstance().setIsMatching(true);
            this.tuo_guan(false);
            cc.find("Canvas/desk_node/jlmj_zhishiqi").active = false;
        }
    },
    ...
    on_room_enter:function () {
        cc.log("symj jbc desk on_room_enter--->"+(DeskData.Instance().isGameStart));
    ...
    recover_desk:function () {
        cc.log("symj jbc desk recover_desk------执行0");
        this.on_room_enter();
        this.recoverDesk();
    },
        ...
        if( msg.retCode !== 0 ) {
            ...
            switch (msg.retCode) {
                case 1:
                    ...
                    this.popupEnterHall( str, symj_util.enterRoomList.bind(symj_util) );
                    break;
                case 2:
    ...
    tuo_guan: function (isShow) {
        if(isShow){
            ...
            if(this.menu_list){
                this.menu_list.getComponent("symj_game_menu_list").closeMenuAndOptions();
            }
    ...
    clean_tuo_guan:function () {
        if(DeskData.Instance().isoffline) {
            const req = new cc.pb.songyuanmajiang.sy_req_update_deposit();
            req.setIsdeposit(false);
            cc.gateNet.Instance().sendMsg(cc.netCmd.songyuanmajiang.cmd_sy_req_update_deposit, req, "sy_req_update_deposit", true);
        }
    },
    ...
    onEventMessage: function (event, data) {
        ...
        switch (event) {
            ...
            case DeskEvent.RECOVER_DESK:
                cc.log("symj jbc recover_desk------ 开始0");
                this.recover_desk();
                break;
