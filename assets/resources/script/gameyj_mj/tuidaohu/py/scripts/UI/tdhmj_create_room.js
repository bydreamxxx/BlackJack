...
cc.Class({
        ...
        switch (data) {
            case "rs1":
                ...
                var conf = cost_room_cards_config.getItemList(function (item) {
                    return item.id == cc.dd.Define.GameType.TDHMJ_FRIEND && item.player_num == 4;
                }.bind(this));
            ...
            case "rs2":
                ...
                var conf = cost_room_cards_config.getItemList(function (item) {
                    return item.id == cc.dd.Define.GameType.TDHMJ_FRIEND && item.player_num == 3;
                }.bind(this));
            ...
            case "rs3":
                ...
                var conf = cost_room_cards_config.getItemList(function (item) {
                    return item.id == cc.dd.Define.GameType.TDHMJ_FRIEND && item.player_num == 2;
                }.bind(this));
    ...
    onCreate: function (event, custom) {
        /************************游戏统计 start************************/
        cc.dd.Utils.sendClientAction(cc.dd.clientAction.TDHMJ_FRIEND, cc.dd.clientAction.T_HALL.CREATE_ROOM);
        /************************游戏统计   end************************/
    ...
    sendCreateRoom: function (data, isProxy) {
        ...
        cc.sys.localStorage.setItem(cc.dd.user.id + '_tdhmj_create', JSON.stringify(data));
    ...
    initData: function () {
        ...
        this.m_objRuleIndex = {
            ...
            rs: 4,
            // 玩法
            wf: [true, true, true, true, true, false, false, false, false],
            // 防作弊
            fzb: true,
            //俱乐部限定
            club: false,
        ...
        let result = cc.sys.localStorage.getItem(cc.dd.user.id + '_tdhmj_create');
    ...
    getRules(data, isProxy){
        ...
        var conf = cost_room_cards_config.getItemList(function (item) {
            return item.id == cc.dd.Define.GameType.TDHMJ_FRIEND && item.player_num == data.rs;
        }.bind(this));

        qsOrJs = conf[data.qs].circle_num

        var gameReq = new cc.pb.room_mgr.msg_create_game_req();
        var pbData = new cc.pb.room_mgr.neimenggu_req_createdesk();
        ...
        pbData.setIspiaohu(data.wf[0]);
        pbData.setIsliangxi(data.wf[1]);
        pbData.setIsqidui(data.wf[2]);
        pbData.setIsqingyise(data.wf[3]);
        pbData.setIsyitiaolong(data.wf[4]);
        pbData.setIsyipaoduoxiang(data.wf[5]);
        pbData.setIscanchi(data.wf[6]);
        pbData.setIspaofen(data.wf[7]);
        pbData.setIslianzhuang(!data.wf[8]);

        pbData.setGps(data.fzb);
        pbData.setIsyuyin(data.yy);

        commonGame.setGameType(cc.dd.Define.GameType.TDHMJ_FRIEND);
        if (isProxy) {
        ...
        rule.setMjNeimengguRule(pbData);
    ...
    setRoomRule(rule){
        ...
        let _rule = rule.rule.mjNeimengguRule;

        let qsOrJs = 0;
        var conf = cost_room_cards_config.getItemList(function (item) {
            return item.id == cc.dd.Define.GameType.TDHMJ_FRIEND && item.player_num == _rule.usercountlimit;
        }.bind(this));
        ...
        this.m_objRuleIndex = {
            ...
            // 玩法
            wf: [_rule.ispiaohu, _rule.isliangxi, _rule.isqidui, _rule.isqingyise, _rule.isyitiaolong, _rule.isyipaoduoxiang, _rule.iscanchi, _rule.ispaofen, _rule.islianzhuang],
            // 防作弊
            fzb: _rule.gps,
            //俱乐部限定
            club: _rule.gps,
