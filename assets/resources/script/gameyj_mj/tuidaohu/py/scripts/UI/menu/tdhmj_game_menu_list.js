...
var hupai_type = require("tdhmj_game_menu").hupai_type;

var menu_type = require("tdhmj_game_menu").menu_type;
...
cc.Class({
    ...
    addMenu: function (type) {
        ...
        var menu = menu_node.getComponent("tdhmj_game_menu");
        if(!menu){
            cc.error("tdhmj_game_menu预制没有挂在tdhmj_game_menu组件");
            return;
        }
        menu.type = type;
    ...
    setMenus: function(player){
        ...
        if(player.canhu == hupai_type.CAN_HU_TRUE_DUIBAO || player.canhu == hupai_type.CAN_HU_TRUE_MOBAO){
            ...
            if(player.canxi){
                this.onShow_list.push(this.getMenu(this.menu_list,menu_type.LIANGXI));
            }
        }
