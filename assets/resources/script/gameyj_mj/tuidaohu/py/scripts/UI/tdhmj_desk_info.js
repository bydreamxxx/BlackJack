...
var tdhmj_util = require("tdhmj_util");
var tdhmj_desk_data_jbc = require("tdhmj_desk_data_jbc");
...
var UserPlayer = require("tdhmj_userPlayer_data");
...
var baseDeskInfo = cc.Class({
    ...
    onLoad: function() {
        cc.log("tdhmj_desk_info onLoad");
        this.logo = cc.find("Canvas/desk_node/c-logo-tuidaohu").getComponent(cc.Sprite);
        ...
        this.run_score = cc.find("Canvas/tdhmj_run_score");
    ...
    initLocalData: function() {
        ...
        //桌布
        var json = cc.sys.localStorage.getItem('tdhmj_zhuobu_set_' + cc.dd.user.id);
        if (json) {
            var sprite = null;
            ...
            if (sprite) {
                ...
                cc.sys.localStorage.setItem('tdhmj_zhuobu_set_' + cc.dd.user.id, this.zhuomianImg[0]._name);
            }
        }
        else {
            this.deskImage.spriteFrame = this.zhuomianImg[0];
    ...
    initGuiZeInfo: function() {
        ...
        this.gz_arr_box = [];
        this.gz_arr_box.push(cc.dd.Text.TEXT_PY_RULE_112.format([RoomMgr.Instance()._Rule.usercountlimit]));
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.ispiaohu ? Text.TEXT_PY_RULE_113 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isliangxi ? Text.TEXT_PY_RULE_114 : '');
        ...
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isyitiaolong ? Text.TEXT_PY_RULE_115 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isyipaoduoxiang ? Text.TEXT_PY_RULE_116 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.iscanchi ? Text.TEXT_PY_RULE_117 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.ispaofen ? Text.TEXT_PY_RULE_118 : '');
        this.gz_arr_box.push(!RoomMgr.Instance()._Rule.islianzhuang ? Text.TEXT_PY_RULE_119 : '');
        ...
        this.gz_arr_info.push({ str: Text.TEXT_PY_RULE_112.format([RoomMgr.Instance()._Rule.usercountlimit]), nodetype: 0 });
        this.gz_arr_info.push({ str: juquan_txt.format([RoomMgr.Instance()._Rule.boardscout]), nodetype: 0 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.ispiaohu ? Text.TEXT_PY_RULE_113 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.isliangxi ? Text.TEXT_PY_RULE_114 : '', nodetype: 1 });
        ...
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.isyitiaolong ? Text.TEXT_PY_RULE_115 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.isyipaoduoxiang ? Text.TEXT_PY_RULE_116 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.iscanchi ? Text.TEXT_PY_RULE_117 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.ispaofen ? Text.TEXT_PY_RULE_118 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: !RoomMgr.Instance()._Rule.islianzhuang ? Text.TEXT_PY_RULE_119 : '', nodetype: 1 });
    ...
    initZhiNan: function() {
        if (this.zhinan.active && DeskData.Instance().zhiNan_site != null) {
            ...
            this.zhinan.getComponent("tdhmj_zhinan_ui").initsetZhiNan(site);
    ...
    recoverDesk: function() {
        cc.log("tdhmj 朋 友 recover_desk------ 接收1");
        cc.log("------恢复桌子------py");
        this.updateDesk();
        DeskData.Instance().enterSceneRecoverDesk(this.sendReadyOK.bind(this));
        if (this.fenzhang) {
        ...
        if(this.run_scoreTips.active || this.run_score.active){
            cc.find("Canvas/desk_node").getComponent('tdhmj_scene').jlmjPlayerDownSortShouPai();
        }
        this.run_scoreTips.active = false;
        this.run_score.active = false;
    },
    ...
    sendReadyOK: function() {
        ...
        var msg = new cc.pb.neimenggumj.neimenggu_req_reloading_ok();
        cc.gateNet.Instance().sendMsg(cc.netCmd.neimenggumj.cmd_neimenggu_req_reloading_ok, msg, "neimenggu_req_reloading_ok");
        this.initGuiZeInfo();
    },
    ...
    jiesuan: function(data) {
        ...
        this.jiesuan_TimeID = this.scheduleOnce(function() {
            ...
            if (cc.director.getScene().name == 'tdhmj_py_game' ||
                cc.director.getScene().name == 'tdhmj_jbc_game') {
                cc.dd.UIMgr.destroyUI(jlmj_prefab.JLMJ_JIAOPAI_INFO);
    ...
    leave_tips: function(data) {
        ...
        if (DeskData.Instance().isFriend()) {//朋友场
            ...
            } else if (tdhmj_desk_data_jbc.getInstance().getIsMatching() || DeskData.Instance().player_read_gamne) {//取消匹配状态
                txt = Text.TEXT_LEAVE_ROOM_3;
                callfunc = this.jbc_matching_leave_room_req;
            } else if (tdhmj_desk_data_jbc.getInstance().getIsStart()) {//游戏中退出游戏
    ...
    py_dissolve_room_req: function() {
        ...
        var msg = new cc.pb.neimenggumj.neimenggu_req_sponsor_dissolve_room();
        msg.setSponsorid(dd.user.id);
        cc.gateNet.Instance().sendMsg(cc.netCmd.neimenggumj.cmd_neimenggu_req_sponsor_dissolve_room, msg, "neimenggu_req_sponsor_dissolve_room");
    },
        ...
        if (msg.retCode !== 0) {
            ...
            switch (msg.retCode) {
                case 1:
                    ...
                    this.popupEnterHall(str, tdhmj_util.enterHall.bind(tdhmj_util));
                    break;
                case 2:
    ...
    py_ready: function() {
        ...
        var msg = new cc.pb.neimenggumj.neimenggu_req_ready();
        cc.gateNet.Instance().sendMsg(cc.netCmd.neimenggumj.cmd_neimenggu_req_ready, msg, "neimenggu_req_ready");
    },
    ...
    update_player_info: function() {
        ...
        this.zhinan.getComponent("tdhmj_zhinan_ui").initDirection();
        var play_list = cc.find('Canvas/player_list').getComponent('tdhmj_player_list');
    ...
    getTextByWanFa: function() {
        ...
        this.gz_arr_box = [];
        this.gz_arr_box.push(cc.dd.Text.TEXT_PY_RULE_112.format([cur_rule.usercountlimit]));
        this.gz_arr_box.push(cur_rule.ispiaohu ? Text.TEXT_PY_RULE_113 : '');
        this.gz_arr_box.push(cur_rule.isliangxi ? Text.TEXT_PY_RULE_114 : '');
        this.gz_arr_box.push(cur_rule.isqidui ? Text.TEXT_PY_RULE_27 : '');
        this.gz_arr_box.push(cur_rule.isqingyise ? Text.TEXT_PY_RULE_29 : '');
        this.gz_arr_box.push(cur_rule.isyitiaolong ? Text.TEXT_PY_RULE_115 : '');
        this.gz_arr_box.push(cur_rule.isyipaoduoxiang ? Text.TEXT_PY_RULE_116 : '');
        this.gz_arr_box.push(cur_rule.iscanchi ? Text.TEXT_PY_RULE_117 : '');
        this.gz_arr_box.push(cur_rule.ispaofen ? Text.TEXT_PY_RULE_118 : '');
        this.gz_arr_box.push(!cur_rule.islianzhuang ? Text.TEXT_PY_RULE_119 : '');
    ...
    onInvaite: function(event, custom) {
        ...
        jlmj_audio_mgr.com_btn_click();
        var title = Text.TEXT_PY_RULE_112.format(playerMgr.Instance().playerList.length);
    ...
    on_show_result_view: function() {
        ...
        cc.dd.UIMgr.openUI(jlmj_prefab.MJ_ZHANJITONGJI, function(ui) {
            var fj_arr = [];
            var juquan_txt = RoomMgr.Instance()._Rule.usercountlimit != 4 ? Text.TEXT_PY_RULE_10 : Text.TEXT_PY_RULE_11;
            fj_arr.push(Text.TEXT_PY_RULE_112.format([RoomMgr.Instance()._Rule.usercountlimit]));
    ...
    onEventMessage: function(event, data) {
        ...
        switch (event) {
            ...
            case HallCommonEvent.HALL_NO_RECONNECT_GAME:
                cc.log("tdhmj  响应------> HallCommonEvent.HALL_NO_RECONNECT_GAME");
            ...
            case DeskEvent.RECOVER_DESK://恢复桌子
                cc.log("tdhmj 朋 友 recover_desk------ 开始0");
                this.recoverDesk();
                break;
    ...
    _change2D() {
        ...
        cc.find("Canvas/desk_node/jlmj_player_left_ui").getComponent('tdhmj_player_left_ui').resetConfig();
        cc.find("Canvas/desk_node/jlmj_player_down_ui").getComponent('tdhmj_player_down_ui').resetConfig();
        cc.find("Canvas/desk_node/jlmj_player_right_ui").getComponent('tdhmj_player_right_ui').resetConfig();
        cc.find("Canvas/desk_node/jlmj_player_up_ui").getComponent('tdhmj_player_up_ui').resetConfig();
    },
    ...
    onClickRunScore(event, data){
        ...
        var msg = new cc.pb.neimenggumj.neimenggu_req_paofen();
        msg.setScore(Number(data));
        cc.gateNet.Instance().sendMsg(cc.netCmd.neimenggumj.cmd_neimenggu_req_paofen, msg, "neimenggu_req_paofen", true);
