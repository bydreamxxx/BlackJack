...
var TDHMJUserPlayerData = cc.Class({
    ...
    statics: {
        ...
        Instance: function () {
            if(!this.s_userPlayer){
                this.s_userPlayer = new TDHMJUserPlayerData();
            }
    ...
    setCpgth: function (msg, isGangTing) {
        ...
        this.canxi = this.checkXi(msg);
        this.cangang = this.checkGang(msg);
        var caozuoDes = function (canchi,canpeng,cangang,canbugang,canting,canhu,canxi) {
            var des = "[";
            if(canchi)  { des+=" 吃 " };
            if(canpeng) { des+=" 碰 " };
            if(cangang) { des+=" 杠 " };
        ...
        cc.log("【数据】"+"玩家:"+this.userId+" 座位号:"+this.idx+" 操作菜单:"+caozuoDes(this.canchi,this.canpeng,this.cangang,this.canbugang,this.canting,this.canhu,this.canxi));
        ...
        if(this.cangang || this.canbugang){
            if(!DeskData.Instance().hasRemainPai()){
                ...
                if( gangOption.cardList.length == 1 ) {
                    ...
                    const gang_Analysts = require('tdhmj_pai_analysts');
     ...
     */
    hasCaozuo: function () {
        cc.log('----判断关闭吃碰杠菜单开始---'+this.canchi+"  "+this.canpeng+"  "+this.cangang+"  "+this.canting+"  "+this.canhu+"  "+this.canxi);
        return this.canchi||this.canpeng||this.cangang||this.canting||this.canhu||this.canxi;
    },
    ...
    checkXi(msg){
        ...
        for(let i = msg.gangcards.gangcardList.length - 1; i >= 0; i--){
            ...
            if(info.id == 108 || info.id == 109 || info.id == 110 || info.id == 111){
                this.xiList.zhong.push(info.id);
            }else if(info.id == 112 || info.id == 113 || info.id == 114 || info.id == 115){
                this.xiList.fa.push(info.id);
            }else if(info.id == 116 || info.id == 117 || info.id == 118 || info.id == 119){
                this.xiList.bai.push(info.id);
            }
        }
        cc.log("【数据】"+"玩家:"+this.userId+" 座位号:"+this.idx+" 喜牌选项"+":"+pai3d_value.descs(this.xiList.zhong)+pai3d_value.descs(this.xiList.fa)+pai3d_value.descs(this.xiList.bai));

        return this.xiList.zhong.length != 0 && this.xiList.fa.length != 0 && this.xiList.bai.length != 0
    },
    ...
    checkGang(msg){
        ...
        groupList.fa = [];
        groupList.bai = [];
        for(let i = 0; i < msg.gangcards.gangcardList.length; i++){
            let item = msg.gangcards.gangcardList[i];
            if(item.type != 4){
                return true;
            }else{
                if(item.id == 108 || item.id == 109 || item.id == 110 || item.id == 111){
                    groupList.zhong.push(item.id);
                }else if(item.id == 112 || item.id == 113 || item.id == 114 || item.id == 115){
                    groupList.fa.push(item.id);
                }else if(item.id == 116 || item.id == 117 || item.id == 118 || item.id == 119){
                    groupList.bai.push(item.id);
                }
            }
        }

        return groupList.zhong.length == 4 || groupList.fa.length == 4 || groupList.bai.length == 4
    },
});

module.exports = TDHMJUserPlayerData;
