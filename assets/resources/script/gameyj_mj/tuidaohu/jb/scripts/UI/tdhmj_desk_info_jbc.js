var baseDeskInfo = require("tdhmj_desk_info");
...
var baseDeskInfoJB = cc.Class({
    ...
    onLoad: function () {
        cc.log("tdhmj_desk_info_jbc onLoad");
        this._super();
    ...
    initDeskData: function () {
        if (DeskData.Instance().isJBC()) {
            var deskDataJBC = tdhmj_desk_data_jbc.getInstance();
    ...
    start: function () {
        ...
        if (tdhmj_desk_data_jbc.getInstance().getIsReconnect()) {
    ...
    onDestroy: function () {
        ...
        tdhmj_desk_data_jbc.getInstance().destroy();
        this._super();
        //RoomMgr.Destroy();
    },
    ...
    on_room_replace: function (msg) {
        if (msg.retCode !== 0) {
            ...
            this.playMatchAnim();
            tdhmj_desk_data_jbc.getInstance().setIsMatching(true);
            this.tuo_guan(false);
            cc.find("Canvas/desk_node/jlmj_zhishiqi").active = false;
        }
    },
    ...
    on_room_enter: function () {
        cc.log("tdhmj jbc desk on_room_enter--->" + (DeskData.Instance().isGameStart));
    ...
    recover_desk: function () {
        cc.log("tdhmj jbc desk recover_desk------执行0");
        this.on_room_enter();
        //this.recoverDesk();
    },
        ...
        if (msg.retCode !== 0) {
            ...
            switch (msg.retCode) {
                case 1:
                    ...
                    this.popupEnterHall(str, tdhmj_util.enterRoomList.bind(tdhmj_util));
                    break;
                case 2:
    ...
    tuo_guan: function (isShow) {
        if (isShow) {
            ...
            if (this.menu_list) {
                this.menu_list.getComponent("tdhmj_game_menu_list").closeMenuAndOptions();
            }
    ...
    clean_tuo_guan: function () {
        if (DeskData.Instance().isoffline) {
            const req = new cc.pb.neimenggumj.neimenggu_req_update_deposit();
            req.setIsdeposit(false);
            cc.gateNet.Instance().sendMsg(cc.netCmd.neimenggumj.cmd_neimenggu_req_update_deposit, req, "neimenggu_req_update_deposit", true);
        }
    },
    ...
    game_opening: function () {
        ...
        tdhmj_desk_data_jbc.getInstance().setIsStart(true);
        tdhmj_desk_data_jbc.getInstance().setIsMatching(false);
    ...
    onEventMessage: function (event, data) {
        ...
        switch (event) {
            ...
            case DeskEvent.RECOVER_DESK:
                cc.log("tdhmj jbc recover_desk------ 开始0");
                this.recover_desk();
                break;
