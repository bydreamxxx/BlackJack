...
var hupai_type = require("ccmj_game_menu").hupai_type;

var menu_type = require("ccmj_game_menu").menu_type;
...
cc.Class({
    ...
    addMenu: function (type) {
        ...
        var menu = menu_node.getComponent("ccmj_game_menu");
        if(!menu){
            cc.error("ccmj_game_menu预制没有挂在ccmj_game_menu组件");
            return;
        }
        menu.type = type;
    ...
    setMenus: function(player){
        ...
        if(player.canhu == hupai_type.CAN_HU_TRUE_DUIBAO || player.canhu == hupai_type.CAN_HU_TRUE_MOBAO){
            ...
            // if(player_list.length == 2) {
                if(DeskData.Instance().isFriend() && player.canhu){
