...
var playerED = require("ccmj_player_data").PlayerED;
var PlayerEvent = require("ccmj_player_data").PlayerEvent;
...
var UserPlayer = require("ccmj_userPlayer_data");
...
var baseDeskInfo = cc.Class({
    ...
    onLoad: function() {
        cc.log("ccmj_desk_info onLoad");
        this.logo = cc.find("Canvas/desk_node/c-logo-changchun").getComponent(cc.Sprite);
        ...
        this.new_2D = cc.find("Canvas/desk_node/new_2D");
        cc.find("Canvas/desk_node/new_2D/left").active = false;
        cc.find("Canvas/desk_node/new_2D/quan").active = false;
        this.new_25D = cc.find("Canvas/desk_node/new_baopai");
        this.baopai_2D = cc.find("Canvas/desk_node/new_2D/baopai").getComponent("jlmj_pai");
        cc.find("Canvas/desk_node/new_2D/baopai_bg").active = true;
        this.baopai_2D.node.active = false;

        this.RecordButton = cc.find("Canvas/toppanel/RecordButton");
    ...
    initDeskData: function() {
        ...
        cc.find("Canvas/desk_node/new_2D/quan/count").getComponent(cc.Label).string = currValue+'/'+totalValue;
    },
    ...
    initLocalData: function() {
        ...
        if (this.use2D) {
            ...
            this.new_25D.active = false;
            this.new_2D.active = true;
            return;
        }

        this.new_25D.active = true;
        this.new_2D.active = false;

        this.logo.node.scaleX = 1;
        this.logo.node.scaleY = 1;

        //桌布
        var json = cc.sys.localStorage.getItem('ccmj_zhuobu_set_' + cc.dd.user.id);
        if (json) {
            var sprite = null;
            ...
            if (sprite) {
                ...
                cc.sys.localStorage.setItem('ccmj_zhuobu_set_' + cc.dd.user.id, this.zhuomianImg[0]._name);
            }
        }
        else {
            this.deskImage.spriteFrame = this.zhuomianImg[0];
    ...
    initGuiZeInfo: function() {
        ...
        this.gz_arr_box = [];
        this.gz_arr_box.push(cc.dd.Text.TEXT_PY_RULE_20.format([RoomMgr.Instance()._Rule.usercountlimit]));
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isxiadanzhanli ? Text.TEXT_PY_RULE_14 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isxiaojifeidan ? Text.TEXT_PY_RULE_6 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.issifenggang ? Text.TEXT_PY_RULE_18 : Text.TEXT_PY_RULE_17);
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isquemen ? Text.TEXT_PY_RULE_15 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.istongbaofanfan ? Text.TEXT_PY_RULE_16 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isbujiabuhu ? Text.TEXT_PY_RULE_87 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.ishaoqifanbei ? Text.TEXT_PY_RULE_107 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.ismingdanzhanli ? Text.TEXT_PY_RULE_161 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.islixiantuoguan !== '0' ? Text.TEXT_PY_RULE_162.format([RoomMgr.Instance()._Rule.islixiantuoguan]) : Text.TEXT_PY_RULE_163);
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.pqsanse ? Text.TEXT_PY_RULE_19 : '');
        this.gz_arr_box.push(RoomMgr.Instance()._Rule.isdianpaosanjia ? Text.TEXT_PY_RULE_108 : '');
        ...
        this.gz_arr_info.push({ str: Text.TEXT_PY_RULE_20.format([RoomMgr.Instance()._Rule.usercountlimit]), nodetype: 0 });
        this.gz_arr_info.push({ str: juquan_txt.format([RoomMgr.Instance()._Rule.boardscout]), nodetype: 0 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.isxiadanzhanli ? Text.TEXT_PY_RULE_14 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.isxiaojifeidan ? Text.TEXT_PY_RULE_6 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.issifenggang ? Text.TEXT_PY_RULE_18 : Text.TEXT_PY_RULE_17, nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.isquemen ? Text.TEXT_PY_RULE_15 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.istongbaofanfan ? Text.TEXT_PY_RULE_16 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.isbujiabuhu ? Text.TEXT_PY_RULE_87 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.ishaoqifanbei ? Text.TEXT_PY_RULE_107 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.ismingdanzhanli ? Text.TEXT_PY_RULE_161 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.islixiantuoguan !== '0' ? Text.TEXT_PY_RULE_162.format([RoomMgr.Instance()._Rule.islixiantuoguan]) : Text.TEXT_PY_RULE_163, nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.pqsanse ? Text.TEXT_PY_RULE_19 : '', nodetype: 1 });
        this.gz_arr_info.push({ str: RoomMgr.Instance()._Rule.isdianpaosanjia ? Text.TEXT_PY_RULE_108 : '', nodetype: 1 });
        mj_guize.addGuize(this.gz_arr_info, RoomMgr.Instance()._Rule.gps);

        cc.find("Canvas/desk_node/new_2D/quan/quan").getComponent(cc.Label).string = RoomMgr.Instance()._Rule.usercountlimit == 2 ? '局数:' : '圈数:';
    ...
    initZhiNan: function() {
        if (this.zhinan.active && DeskData.Instance().zhiNan_site != null) {
            ...
            this.zhinan.getComponent("ccmj_zhinan_ui").initsetZhiNan(site);
    ...
    clear: function(data) {
        ...
        cc.find("Canvas/desk_node/new_baopai/baopai").active = false;
        cc.find("Canvas/desk_node/new_2D/baopai_bg").active = true;
        cc.find("Canvas/desk_node/new_2D/baopai").active = false;
        cc.find("Canvas/desk_node/jlmj_player_down_ui/tingButton").active = false;
        cc.find("Canvas/desk_node/new_2D/left").active = false;
        cc.find("Canvas/desk_node/new_2D/quan").active = false;
        this.closeResponseDissolveView();
    ...
    recoverDesk: function() {
        cc.log("ccmj 朋 友 recover_desk------ 接收1");
        cc.log("------恢复桌子------py");
        this.updateDesk();
        DeskData.Instance().enterSceneRecoverDesk(this.sendReadyOK.bind(this));
        if (this.fenzhang) {
    ...
    update_game_status: function(status) {
        ...
        if (status == 1) {
            ...
            if (DeskData.Instance().isFriend()) {
                ...
                cc.find("Canvas/room_num").active = false;
                if(this.RecordButton){
                    this.RecordButton.active = RoomMgr.Instance()._Rule.isyuyin;
                }
            }
            this.zhinan.active = true;
            this.initZhiNan();
    ...
    sendReadyOK: function() {
        ...
        var msg = new cc.pb.changchunmajiang.p16_req_reloading_ok();
        cc.gateNet.Instance().sendMsg(cc.netCmd.changchunmajiang.cmd_p16_req_reloading_ok, msg, "p16_req_reloading_ok");
        this.initGuiZeInfo();
    },
    ...
    jiesuan: function(data) {
        ...
        this.jiesuan_TimeID = this.scheduleOnce(function() {
            ...
            if (cc.director.getScene().name == 'ccmj_py_game' ||
                cc.director.getScene().name == 'ccmj_jbc_game') {
                cc.dd.UIMgr.destroyUI(jlmj_prefab.JLMJ_JIAOPAI_INFO);
                    ...
                    if (DeskData.Instance().isJBC()) {
                        ...
                        if (room_item && HallPropData.getCoin() < room_item.entermin) {
                            cc.dd.UIMgr.openUI(jlmj_prefab.JLMJ_XIAOQIAN, function(ui) {
                                ...
                                ui.setLocalZOrder(UIZorder.MJ_LAYER_TOP);
    ...
    leave_tips: function(data) {
        ...
        if (DeskData.Instance().isFriend()) {//朋友场
            ...
            } else if (ccmj_desk_data_jbc.getInstance().getIsMatching() || DeskData.Instance().player_read_gamne) {//取消匹配状态
                txt = Text.TEXT_LEAVE_ROOM_3;
                callfunc = this.jbc_matching_leave_room_req;
            } else if (ccmj_desk_data_jbc.getInstance().getIsStart()) {//游戏中退出游戏
    ...
    py_dissolve_room_req: function() {
        ...
        var msg = new cc.pb.changchunmajiang.p16_req_sponsor_dissolve_room();
        msg.setSponsorid(dd.user.id);
        cc.gateNet.Instance().sendMsg(cc.netCmd.changchunmajiang.cmd_p16_req_sponsor_dissolve_room, msg, "p16_req_sponsor_dissolve_room");
    },
    ...
    sponsorDissolveRoom: function(msg) {
        ...
        if (!jiesan_ui && !mj_zhangji) {
            cc.dd.UIMgr.openUI(jlmj_prefab.JLMJ_JIESAN, function(ui) {
                ui.setLocalZOrder(UIZorder.MJ_LAYER_POPUP);
            });
    ...
    open_bao_pai: function() {
        ...
        if (baoPaiValue >= 0) {
            cc.find("Canvas/desk_node/new_baopai/baopai").active = true;
            this.baopai_2D.node.active = true;
            this.baopai.node.active = true;
            cc.find("Canvas/desk_node/new_2D/baopai_bg").active = false;
            this.baopai_2D.frame.spriteFrame = this.spf_baopai_ming_2d;
            ...
            this.baopai_2D.value.node.active = true;
            this.baopai_2D.setValue(baoPaiValue);
        }
    },
        ...
        if (msg.retCode !== 0) {
            ...
            switch (msg.retCode) {
                case 1:
                    ...
                    this.popupEnterHall(str, ccmj_util.enterHall.bind(ccmj_util));
                    break;
                case 2:
    ...
    tuo_guan: function (data) {
        var play_list = cc.find('Canvas/player_list').getComponent('ccmj_player_list');
        if (play_list) {
            for (var i = 0; i < play_list.player_ui_arr.length; ++i) {
                var headinfo = play_list.player_ui_arr[i];
                if (headinfo && headinfo.head.player && headinfo.head.player.userId == data[1]) {
                    headinfo.head.setTuoguan(data[0]);
                    break;
                }
            }
        }

        if(cc.dd.user.id == data[1]){
            if(data[0]){
            ...
            cc.find("Canvas/toppanel/layer_tuo_guan").active = data[0];
        }
    },
    ...
    py_ready: function() {
        ...
        var msg = new cc.pb.changchunmajiang.p16_req_ready();
        cc.gateNet.Instance().sendMsg(cc.netCmd.changchunmajiang.cmd_p16_req_ready, msg, "p16_req_ready");
    },
    ...
    update_bao_pai: function() {
        ...
        if (baoPaiValue == -2) {
            ...
            cc.find("Canvas/desk_node/new_2D/baopai_bg").active = false;
            this.baopai_2D.node.active = true;
            this.baopai_2D.frame.spriteFrame = this.spf_baopai_an_2d;
            this.baopai_2D.value.node.active = false;
    ...
    update_remain_card: function(cardNum) {
        ...
        if (DeskData.Instance().isGameStart) {
            ...
            cc.find("Canvas/desk_node/new_2D/left/left").getComponent(cc.Label).string = cardNum;
        }
    },
    ...
    update_curr_round: function(currValue) {
        ...
        cc.find("Canvas/desk_node/new_2D/quan/count").getComponent(cc.Label).string = currValue+'/'+DeskData.Instance().totalPlayCount;
    ...
    update_total_round: function(totalValue) {
        ...
        cc.find("Canvas/desk_node/new_2D/quan/count").getComponent(cc.Label).string = DeskData.Instance().currPlayCount+'/'+totalValue;
    ...
    getTextByWanFa: function() {
        ...
        this.gz_arr_box = [];
        this.gz_arr_box.push(cc.dd.Text.TEXT_PY_RULE_20.format([cur_rule.usercountlimit]));
        this.gz_arr_box.push(cur_rule.isxiadanzhanli ? Text.TEXT_PY_RULE_14 : '');
        this.gz_arr_box.push(cur_rule.isxiaojifeidan ? Text.TEXT_PY_RULE_6 : '');
        this.gz_arr_box.push(cur_rule.issifenggang ? Text.TEXT_PY_RULE_18 : Text.TEXT_PY_RULE_17);
        this.gz_arr_box.push(cur_rule.isquemen ? Text.TEXT_PY_RULE_15 : '');
        this.gz_arr_box.push(cur_rule.istongbaofanfan ? Text.TEXT_PY_RULE_16 : '');
        this.gz_arr_box.push(cur_rule.isbujiabuhu ? Text.TEXT_PY_RULE_87 : '');
        this.gz_arr_box.push(cur_rule.pqsanse ? Text.TEXT_PY_RULE_19 : '');
        this.gz_arr_box.push(cur_rule.ishaoqifanbei ? Text.TEXT_PY_RULE_107 : '');
        this.gz_arr_box.push(cur_rule.isdianpaosanjia ? Text.TEXT_PY_RULE_108 : '');
        this.gz_arr_box.push(cur_rule.ismingdanzhanli ? Text.TEXT_PY_RULE_161 : '');
        this.gz_arr_box.push(cur_rule.islixiantuoguan !== '0' ? Text.TEXT_PY_RULE_162.format([cur_rule.islixiantuoguan]) : Text.TEXT_PY_RULE_163);
    ...
    onInvaite: function(event, custom) {
        ...
        jlmj_audio_mgr.com_btn_click();
        var title = Text.TEXT_PY_RULE_20.format(playerMgr.Instance().playerList.length);
        title += " " + "房间号:" + (RoomMgr.Instance().roomId || 888888);
        var content = '共' + RoomMgr.Instance()._Rule.boardscout + '' + (RoomMgr.Instance()._Rule.usercountlimit == 2 ? '局' : '圈');
    ...
    on_show_result_view: function() {
        ...
        cc.dd.UIMgr.openUI(jlmj_prefab.MJ_ZHANJITONGJI, function(ui) {
            var fj_arr = [];
            var juquan_txt = RoomMgr.Instance()._Rule.usercountlimit == 2 ? Text.TEXT_PY_RULE_10 : Text.TEXT_PY_RULE_11;
            fj_arr.push(Text.TEXT_PY_RULE_20.format([RoomMgr.Instance()._Rule.usercountlimit]));
    ...
    onEventMessage: function(event, data) {
        ...
        switch (event) {
            ...
            case HallCommonEvent.HALL_NO_RECONNECT_GAME:
                cc.log("ccmj  响应------> HallCommonEvent.HALL_NO_RECONNECT_GAME");
            ...
            case DeskEvent.RECOVER_DESK://恢复桌子
                cc.log("ccmj 朋 友 recover_desk------ 开始0");
                this.recoverDesk();
                break;
    ...
    _change2D() {
        ...
        // cc.find("Canvas/desk_node/jlmj_player_left_ui").getComponent('ccmj_player_left_ui').resetConfig();
        // cc.find("Canvas/desk_node/jlmj_player_down_ui").getComponent('ccmj_player_down_ui').resetConfig();
        // cc.find("Canvas/desk_node/jlmj_player_right_ui").getComponent('ccmj_player_right_ui').resetConfig();
        // cc.find("Canvas/desk_node/jlmj_player_up_ui").getComponent('ccmj_player_up_ui').resetConfig();
        var play_list = cc.find('Canvas/player_list').getComponent('ccmj_player_list');
