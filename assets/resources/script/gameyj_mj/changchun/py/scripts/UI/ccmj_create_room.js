...
cc.Class({
    ...
    properties: {
        ...
        p_tg: { default: [], type: cc.Toggle, tooltip: '离线托管', },
        ...
        switch (data) {
            case "rs1":
                ...
                var conf = cost_room_cards_config.getItemList(function (item) {
                    return item.id == cc.dd.Define.GameType.CCMJ_FRIEND && item.player_num == 4;
                }.bind(this));
                ...
                this.setEnableToggle(this.p_wf[8], true);
                this.p_wf[8].node.active = true;
                break;
            case "rs2":
                ...
                var conf = cost_room_cards_config.getItemList(function (item) {
                    return item.id == cc.dd.Define.GameType.CCMJ_FRIEND && item.player_num == 3;
                }.bind(this));
            ...
            case "rs3":
                ...
                var conf = cost_room_cards_config.getItemList(function (item) {
                    return item.id == cc.dd.Define.GameType.CCMJ_FRIEND && item.player_num == 2;
                }.bind(this));
                ...
                this.setEnableToggle(this.p_wf[8], false);
                this.p_wf[8].node.active = false;
                break;
            case "qs1":
    ...
    onCreate: function (event, custom) {
        /************************游戏统计 start************************/
        cc.dd.Utils.sendClientAction(cc.dd.clientAction.CCMJ_FRIEND, cc.dd.clientAction.T_HALL.CREATE_ROOM);
        /************************游戏统计   end************************/
    ...
    sendCreateRoom: function (data, isProxy) {
        ...
        cc.sys.localStorage.setItem(cc.dd.user.id + '_ccmj_create', JSON.stringify(data));
    ...
    initData: function () {
        ...
        this.m_objRuleIndex = {
            ...
            rs: 4,
            // 玩法
            wf: [true, true, false, true, true, false, false, true, true, false],
            //离线托管
            tg: '0',
            // 防作弊
            fzb: true,
        ...
        let result = cc.sys.localStorage.getItem(cc.dd.user.id + '_ccmj_create');
    ...
    getRules(data, isProxy){
        ...
        var conf = cost_room_cards_config.getItemList(function (item) {
            return item.id == cc.dd.Define.GameType.CCMJ_FRIEND && item.player_num == data.rs;
        }.bind(this));

        qsOrJs = conf[data.qs].circle_num

        var gameReq = new cc.pb.room_mgr.msg_create_game_req();
        var pbData = new cc.pb.room_mgr.p16_req_createdesk();
        ...
        pbData.setIsdianpaosanjia(data.wf[8] && data.rs != 2);
        pbData.setIsyaojiusanse(1);
        pbData.setIsuncheat(data.fzb);

        pbData.setIsxiadanzhanli(data.wf[0]);
        pbData.setIsxiaojifeidan(data.wf[1]);
        pbData.setIssifenggang(!data.wf[2]);
        pbData.setIsquemen(data.wf[3]);
        pbData.setIstongbaofanfan(data.wf[4]);
        pbData.setPqsanse(data.wf[5]);
        pbData.setIsbujiabuhu(data.wf[6]);
        pbData.setIshaoqifanbei(data.wf[7]);
        pbData.setReservedList([String(data.wf[9]), String(data.tg)]);

        pbData.setGps(data.fzb);
        pbData.setIsyuyin(data.yy);

        commonGame.setGameType(10);
        if (isProxy) {
        ...
        rule.setMjChangchunRule(pbData);
    ...
    setRoomRule(rule){
        ...
        let _rule = rule.rule.mjChangchunRule;
        _rule.ismingdanzhanli = _rule.reservedList[0] === 'true';
        _rule.islixiantuoguan = _rule.reservedList[1] === 'true' ? '45' : _rule.reservedList[1];

        let qsOrJs = 0;
        var conf = cost_room_cards_config.getItemList(function (item) {
            return item.id == cc.dd.Define.GameType.CCMJ_FRIEND && item.player_num == _rule.usercountlimit;
        }.bind(this));
        ...
        this.m_objRuleIndex = {
            ...
            // 玩法
            wf: [_rule.isxiadanzhanli, _rule.isxiaojifeidan, !_rule.issifenggang, _rule.isquemen, _rule.istongbaofanfan, _rule.pqsanse, _rule.isbujiabuhu, _rule.ishaoqifanbei, _rule.isdianpaosanjia, _rule.ismingdanzhanli],
            //离线托管
            tg: _rule.islixiantuoguan,
            // 防作弊
            fzb: _rule.gps,
            //俱乐部限定
            club: _rule.gps,
