...
cc.Class({
     ...
     */
    initDlGame: function () {
        var logonode = cc.find('c-logo-changchun', this.node).getComponent(cc.Sprite);
    ...
    autoScaleWin: function () {
        ...
        let new_2D = cc.find("Canvas/desk_node/new_2D");
        if (canvas_node.width / canvas_node.height >= desk_node.width / desk_node.height) {
            new_2D.scaleX = 1 / c_scale;
            new_2D.scaleY = 1 / c_scale;
        }
        new_2D.height = zhuozi.height * zhuozi.scaleY;
        new_2D.width = canvas_node.width;
        let widgetList_2D = new_2D.getComponentsInChildren(cc.Widget);
        widgetList_2D.forEach((widget) => {
            if (widget) {
                widget.updateAlignment();
            }
        })
    ...
    playerHideModePai: function () {
        var jlmj_player_list = cc.find("Canvas/player_list").getComponent('ccmj_player_list');
