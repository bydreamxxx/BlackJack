...
cc.Class({
    extends: require('ccmj_player_base_ui'),
    ...
    onLoad: function () {
        cc.log('ccmj_player_down_ui onLoad');
    ...
    updateShouPai: function (player) {
        ...
        if (player.hasMoPai()) {//说明手牌中是包含
            ...
            if (player.state != PlayerState.HUPAI && player.state != PlayerState.TINGPAI) {
                cc.find("Canvas/game_menu_list").getComponent("ccmj_game_menu_list").setMenus(player);
            }
        } else {
            this.modepai.node.active = false;
        }
    ...
    touchStart: function (event) {
        ...
        if (pai_touched) {
            ...
            if (this.touchCardMode == TouchCardMode.TING_PAI) {
                this.closeJiaoInfo();
                cc.log("ccmj touchStart openJiaoInfo");
    ...
    touchMove: function (event) {
        ...
        if (pai_touched) {//找到选择的牌
            ...
            if (pai_touched && this.touchCardMode == TouchCardMode.TING_PAI) {
                this.closeJiaoInfo();
                this.openJiaoInfo(this.pai_touched.cardId);
            }
        }
    ...
    touchEnd: function (event) {
        ...
        if (!this.pai_touched) {
            ...
            if (this.touchCardMode == TouchCardMode.TING_PAI) {
                this.closeJiaoInfo();
                cc.log("ccmj touchEnd openJiaoInfo");
                this.openJiaoInfo(this.pai_touched.cardId);
            }
        }
    ...
    biaojiPai: function (selectId) {
        ...
        for (var i in userList) {
            var com = userList[i].getComponent('ccmj_player_down_ui');
    ...
    sendTingPai: function (playCardId, tingType) {
        ...
        cc.gateNet.Instance().sendMsg(cc.netCmd.changchunmajiang.cmd_p16_req_tingpai_out_card, msg, "cmd_p16_req_tingpai_out_card");
    ...
    sendOutCard: function (cardID) {
        ...
        this.quxiaoBiaoji();
        var msg = new cc.pb.changchunmajiang.p16_req_game_send_out_card();
        msg.setCardid(cardID);
        cc.gateNet.Instance().sendMsg(cc.netCmd.changchunmajiang.cmd_p16_req_game_send_out_card, msg, "p16_req_game_send_out_card");
        cc.dd.NetWaitUtil.net_wait_start();
    },
    ...
    showHideClickCard: function (isShow, canCradList, type) {
        ...
        if (isShow && canCradList) {//可以操作canCardList中牌
            ...
            if (this.modepai) {
                this.modepai.setTouchAble(true);
                this.modepai.setTouch(!UserPlayer.isBaoTing);
            }
        }
    },
    ...
    ting: function (player) {
        ...
        this.closeJiaoInfo();
        var desk_info = cc.find('Canvas/desk_info').getComponent('ccmj_desk_info');
