...
var ccmj_cmd = require('c_msg_changchunmajiang_cmd');
...
cc.Class({
    ...
    getSelfMsgList: function () {
        ...
        for(var i=0; i<msg_list.length; ++i){
            //过滤解散消息
            if(msg_list[i].id == ccmj_cmd.cmd_p16_ack_sponsor_dissolve_room){
                continue;
            }
            if(msg_list[i].id == ccmj_cmd.cmd_p16_ack_response_dissolve_room){
                continue;
            }
            if(msg_list[i].id == ccmj_cmd.cmd_p16_ack_dissolve_room){
                continue;
            }
    ...
    updateGameUI: function () {
        ...
        var desk_info_ui = cc.find('Canvas/desk_info').getComponent('ccmj_desk_info_replay');
        desk_info_ui.updateDesk();
    },

    updateGameUIBeforeMsg: function (id, msg) {
        //非菜单消息,关闭菜单UI
        if(id != ccmj_cmd.cmd_p16_ack_game_overturn){
            var menu_list = cc.find("Canvas/game_menu_list").getComponent("ccmj_game_menu_list");
            menu_list.closeMenuAndOptions();
            for(var i=1; i<4; ++i){
                var menu_list = cc.find("Canvas/game_menu_list_"+i).getComponent("ccmj_game_menu_list");
                menu_list.closeMenuAndOptions();
            }
        }
        //玩家摸牌消息时,关闭菜单UI
        if(id == ccmj_cmd.cmd_p16_ack_game_overturn && msg.acttype == 1){
            var menu_list = cc.find("Canvas/game_menu_list").getComponent("ccmj_game_menu_list");
            menu_list.closeMenuAndOptions();
            for(var i=1; i<4; ++i){
                var menu_list = cc.find("Canvas/game_menu_list_"+i).getComponent("ccmj_game_menu_list");
        ...
        //非打牌消息,停止打牌动画
        if(id != ccmj_cmd.cmd_p16_ack_game_send_out_card){
            var play_list = cc.find('Canvas/player_list');
            if(play_list){
                play_list.getComponent('ccmj_player_list').playerStopChuPaiAni();
                play_list.getComponent('ccmj_player_list').playerUpdateZsq();
            }
        }
    },
