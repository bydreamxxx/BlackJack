...
cc.Class({
    ...
    showLangCard: function (cardlist) {
        ...
        for (var i = 0; i < cards.length; i++) {
            ...
            card.y = 0;
            if (i >= one_lene_num) {
                var moveTo = cc.moveTo(duration, cc.v2((i - one_lene_num) * CARD_OFFSET, -50));
    ...
    showOutCard: function (cardlist, isabnormal) {
        ...
        if (cardlist.length == 0) {
            ...
            var cardWidth = 169;
            var cardHeight = 233;
            var player = PY_Data.getInstance().getPlayerByViewID(1);
            var lastnum = 0;
            if (player)
                lastnum = player.out_poker ? player.out_poker.length : 0;
            if (lastnum > 0) {
                if (lastnum + cardlist.length >= 11) {
                    var erNode = cc.find('eroutCards', this.node);
                    if (erNode) {
                        var scale = 0.3;
                        this.out_node.y = erNode.y - 30;
                    }
                }
                else
                    this.out_node.y = -173;
            ...
            if (!isabnormal) {
                ...
                cc.log('上上家剩余牌数:' + cardNum);
                this.setRemainCardNum(cardNum, cardHeight);
            }
        }
        cc.log('下家出牌完成');
    },
