...
cc.Class({
    ...
    initHistoryList(data){
        ...
        let historyNum = this.historyList.length
        if(historyNum > this.spawnCount){
            historyNum = this.spawnCount;
        }
        for (let i = 0; i < historyNum; i++) {
            j = Math.floor(i / this.row);
            k = i % this.row;
            var item = cc.instantiate(this.prefabItem);
            item.getComponent('chifeng_hall_MyExchangeHistoryItem').initHistoryInfo(i, this.historyList[i]);
            item.index = i;
        ...
        let count = Math.ceil(this.historyList.length / this.row)
        ...
        for (let i = 0; i < items.length; ++i) {
            ...
            if (isDown) {
                ...
                if (viewPos.y < -this.bufferZone && newY < 0) {
                    ...
                    items[i].getComponent('chifeng_hall_MyExchangeHistoryItem').initHistoryInfo(itemId, this.historyList[itemId]);
