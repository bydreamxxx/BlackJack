...
cc.Class({
    ...
    properties: {
        ...
        selfRank: require('klb_hall_iphone_item'),
        winRanks: [require('klb_hall_iphone_item')],
    ...
    show(msg) {
        ...
        this.endTime = new Date(msg.endTime * 1000);
        // this.time.string = '活动时间\n' + startTime.format("yyyy-MM-dd") + '·' + endTime.format("yyyy-MM-dd");
        let date = this.endTime-new Date().getTime();
        if(date <= 0){
            this.time.string = '0天0小时0分';
            this.endTime = null;
        }else{
            this.setTime(date);
        }

        this.initPlayerList(msg.rankDetail.rankListList.concat());
    ...
    update(dt){
        if(this.endTime){
            ...
            if(this.tmpTime >= 1){
                ...
                let date = this.endTime-new Date().getTime();  //时间差的毫秒数
                if(date <= 0){
                    this.time.string = '0天0小时0分';
                    this.endTime = null;
                    return;
...
//计算相差分钟数
        ...
        this.time.string = `${days}天${hours}小时${minutes}分`;
    },
    ...
    onClickRule(){
        hall_audio_mgr.com_btn_click();
        cc.dd.UIMgr.openUI(hall_prefab.KLB_HALL_IPHONE_RULE);
    },
    ...
    initPlayerList(data) {
        ...
        let windata = data.splice(0, 3);
        for(let i = 0; i < this.winRanks.length; i++){
            if(windata[i]){
                this.winRanks[i].getComponent('klb_hall_iphone_item').initPlayerInfo(windata[i]);
            }else{
                this.winRanks[i].init();
            }
        }

        this.playerList = data;//.splice(0, 7);
        ...
        for (let i = 0; i < playerNum; i++) {
            ...
            item.getComponent('klb_hall_iphone_item').initPlayerInfo(this.playerList[i]);
            item.index = i;
        ...
        for (let i = 0; i < items.length; ++i) {
            ...
            if (isDown) {
                ...
                if (viewPos.y < -this.bufferZone && newY < 0) {
                    ...
                    items[i].getComponent('klb_hall_iphone_item').initPlayerInfo(this.playerList[itemId]);
    ...
    onEventMessage(event, data) {
        switch (event) {
            case Hall.HallEvent.RANK_ACTIVITY_ADDRESS:
                cc.dd.UIMgr.openUI(hall_prefab.KLB_HALL_IPHONE_USERINFO, (ui) => {
