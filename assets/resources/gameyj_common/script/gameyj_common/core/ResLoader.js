...
var ResLoader = {
    ...
    loadDynamicRes:function (dynamic_load_assets, onCompleted) {
        ...
        dynamic_load_assets.forEach(function(cell){
            cc.resources.load(cell, function (err, item) {
                    progress++;
    ...
    preloadSceneStaticRes: function (path,type,onload) {
        TimeTake.start(path);
        cc.resources.load(path, type, function (err, item) {
            if (err) {
    ...
    loadFont: function(path,onload) {
        cc.resources.load(path, cc.Font, function (err, font) {
            if (err!=null) {
                cc.error(err.message || err);
            }
            else{
    ...
    loadAnimation: function(path,onload) {
        cc.resources.load(path, cc.AnimationClip, function (err, animation) {
            if (err!=null) {
                cc.error(err.message || err);
            }
            else{
    ...
    loadAudio: function(path,onload) {
        cc.resources.load(path, cc.AudioClip, function (err, audio) {
            if (err!=null) {
                cc.error(err.message || err);
            }
            else{
    ...
    preloadAudioList:function (audioList, cb) {
        if( audioList!=null && typeof audioList!='undefined' && audioList.length>0 ){
            ...
            audioList.forEach(function (path) {
                cc.resources.load(path, function () {
                    load++;
                    if(total == load){
                        cb();
                    }
