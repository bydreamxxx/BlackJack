...
////////////////////////////////////////////玩家操作通信信息枚举begin/////////////////////////////
var DSZ_PlayerEvent = cc.Enum({
    DSZ_PLAYER_COME_BACK: 'dsz_player_come_back', //玩家重连消息
    DSZ_PLAYER_ENTER: 'dsz_player_enter', //玩家进入游戏
    DSZ_PLAYER_READY: 'dsz_player_ready', //玩家准备消息
    DSZ_PLAYER_INIT_DATA: 'dsz_player_init_data', //实例化玩家游戏数据
    DSZ_PLAYER_OP_STATE: 'dsz_player_op_state', //玩家操作/状态更新
    DSZ_PLAYER_WATCH_POKER: 'dsz_player_watch_poker', //玩家看牌
    DSZ_PLYER_EXIT: 'dsz_player_exit', //玩家离开房间
    PLAYER_ISONLINE: 'dsz_player_isonline', //玩家在线
    DSZ_PLAYER_CLEAR: 'dsz_ player_clear', //游戏清除
});
////////////////////////////////////////////玩家操作通信信息枚举end/////////////////////////////

var DSZ_PlayerED = new cc.dd.EventDispatcher();

////////////////////////////////////////////玩家基础信息Begin///////////////////////////////
var DSZ_PlayerData = cc.Class({
    properties:{
        userId: 0, //玩家id
    ...
    setPlayerGameInfo: function(roleInfo){
        ...
        DSZ_PlayerED.notifyEvent(DSZ_PlayerEvent.DSZ_PLAYER_INIT_DATA, this.userId);
    ...
    setPlayerPokers: function(pokers){
        ...
        DSZ_PlayerED.notifyEvent(DSZ_PlayerEvent.DSZ_PLAYER_WATCH_POKER, this.userId);
    },
    ...
    updatePlayerState: function(state){
       ...
       // DSZ_PlayerED.notifyEvent(DSZ_PlayerEvent.DSZ_PLAYER_OP_STATE, this.userId);
    },
     ...
     */
    comeBack: function(){
        DSZ_PlayerED.notifyEvent(DSZ_PlayerEvent.DSZ_PLAYER_COME_BACK, this.userId);
    },
     ...
     */
    enterGame: function(){
        DSZ_PlayerED.notifyEvent(DSZ_PlayerEvent.DSZ_PLAYER_ENTER, this.userId);
    },
    /**
     * 玩家准备
     */
    setReady: function(isReady){
        ...
        DSZ_PlayerED.notifyEvent(DSZ_PlayerEvent.DSZ_PLAYER_READY, this.userId);
    },
    ...
    setOnLine: function(isOnline){
        DSZ_PlayerED.notifyEvent(DSZ_PlayerEvent.PLAYER_ISONLINE, [this, isOnline]);
    },
     ...
     */
    exit: function(){
        DSZ_PlayerED.notifyEvent(DSZ_PlayerEvent.DSZ_PLYER_EXIT, this.playerCommonInfo);
    },
    /////////////////////////////玩家通信操作end///////////////////////////////////////
});
////////////////////////////////////////////玩家基础信息End///////////////////////////////
...
///////////////////////////////////////////////玩家管理器Begin/////////////////////////////
var DSZ_PlayerMgr = cc.Class({
    s_dsz_player_data: null,
    statics: {
        Instance: function () {
            if (!this.s_dsz_player_data) {
                this.s_dsz_player_data = new DSZ_PlayerMgr();
            }
            return this.s_dsz_player_data;
        },

        Destroy: function () {
            if (this.s_dsz_player_data) {
                this.s_dsz_player_data.clear();
                this.s_dsz_player_data = null;
            }
        },
    },
    ...
    genPlayerData: function(role_info){
        ...
        var playerData = new DSZ_PlayerData;
    ...
    updatePlayerNum: function () {
        ...
        if(roomMgr.gameId == Define.GameType.DSZ_FRIEND)
        ...
        DSZ_PlayerED.notifyEvent(DSZ_PlayerEvent.DSZ_PLAYER_CLEAR);
                ...
                if(player){
                    ...
                    if(player.userId != cc.dd.user.id){
                        if(own && this.playerInfo[index].getPlayerCommonInfo().pos == null){
                            this.playerInfo[index].getPlayerCommonInfo().pos = (index - own.getPlayerCommonInfo().seat + this.gamePlayerNum) % this.gamePlayerNum;
                            var pos = this.playerInfo[index].getPlayerCommonInfo().pos;
                            this.playerInfo[index].enterGame();
                        }
    ...
    checkPlayerAllReady: function(){
        var isReadyAll = true;
        this.playerInfo.forEach(function(player){
            if(player && !player.isRoomer && !player.getPlayerCommonInfo().isReady)
                isReadyAll = false;
        });
        return isReadyAll;
    },
    ...
    checkPlayerAllGameReady: function(){
        var isReadyAll = true;
        this.playerInfo.forEach(function(player){
            if(player && !player.isRoomer && player.getPlayerGameInfo().userState != 11)
                isReadyAll = false;
        });
        return isReadyAll;
    },
    ...
    playerExit: function (userId) {
        ...
        //     return;
        // }
        //var view = this.findPlayerByUserId(userId);
        for (var i = 0; i < this.playerInfo.length; i++) {
            if (this.playerInfo[i] && this.playerInfo[i].userId == userId) {
    ...
    //玩家进入
    playerRecordEnter: function (role_info) {
        if(this.findPlayerByUserId(role_info.userId)){//判定玩家是否为断线重连
            this.findPlayerByUserId(role_info.userId).comeBack();
            return;
        }
        ...
        this.playerInfo[index] = player;
        this.sortRecordPlayersPos();
    },

    //重新安排玩家的座位
    sortRecordPlayersPos: function(){
        var own = this.findPlayerByUserId(cc.dd.user.id);
        if(own){
            this.playerInfo.forEach(function(player){
                if(player){
...
module.exports = {
    DSZ_PlayerMgr : DSZ_PlayerMgr,
    DSZ_PlayerED : DSZ_PlayerED,
    DSZ_PlayerEvent: DSZ_PlayerEvent,
};
