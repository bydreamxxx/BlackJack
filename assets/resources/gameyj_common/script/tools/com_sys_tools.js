...
var SysTools = {
    setLandscape: function () {
        if (cc.sys.isNative) {
            if (cc.dd.native_systool.setLandscape()) {
            ...
            this.adjustDesignRatio(design_ratio, true);
            cc.view.setDesignResolutionSize(design_ratio.width, design_ratio.height, cc.ResolutionPolicy.SHOW_ALL);
        } else if (cc.sys.platform == cc.sys.MOBILE_BROWSER) {
            cc.view.setOrientation(cc.macro.ORIENTATION_LANDSCAPE);
            var design_ratio = { width: DESIGN_RATIO_LAND.width, height: DESIGN_RATIO_LAND.height };
            this.adjustDesignRatioH5(design_ratio);
            cc.view.setDesignResolutionSize(design_ratio.width, design_ratio.height, cc.ResolutionPolicy.SHOW_ALL);
    ...
    setPortrait: function () {
        if (cc.sys.isNative) {
            if (cc.dd.native_systool.setPortrait()) {
                var size = cc.view.getFrameSize();
                cc.view.setFrameSize(size.height, size.width);
            }
            var design_ratio = { width: DESIGN_RATIO_PORT.width, height: DESIGN_RATIO_PORT.height };
            this.adjustDesignRatio(design_ratio, false);
            cc.view.setDesignResolutionSize(design_ratio.width, design_ratio.height, cc.ResolutionPolicy.SHOW_ALL);
        } else if (cc.sys.platform == cc.sys.MOBILE_BROWSER) {
    ...
    adjustDesignRatio: function (design_ratio, isSetLand) {
        var screen_ratio = this.getScreenRatio(isSetLand);
        ...
        if (screen_aspect > design_aspect) {
            design_ratio.width *= screen_aspect / design_aspect;
        } else if (screen_aspect < design_aspect) {
            design_ratio.height *= design_aspect / screen_aspect;
        }
        cc.log("调整设计分辨率=", design_ratio.width, ",", design_ratio.height);
    },

    adjustDesignRatioH5: function (design_ratio) {
    ...
    //根据屏幕真实大小缩放ui, 此ui是后面添加到场景上面的。 否则会被缩放2次。(场景加载时会缩放一次)
    resizeByScreenSize: function (ui, isSetLand) {
        if (cc.sys.isNative) {
            var screen_ratio = cc.dd.SysTools.getScreenRatio(isSetLand);
            var screen_aspect = screen_ratio.width / screen_ratio.height;
            var ui_aspect = ui.width / ui.height;
    ...
    getScreenRatio: function (isSetLand) {
        var string_ratio = cc.dd.native_systool.getScreenRatio();
        var ratios = string_ratio.split(",");
        var width = 0;
        var height = 0;
        if (isSetLand) {
            width = Math.max(Number(ratios[0]), Number(ratios[1]));
            height = Math.min(Number(ratios[0]), Number(ratios[1]));
        } else {
            width = Math.min(Number(ratios[0]), Number(ratios[1]));
            height = Math.max(Number(ratios[0]), Number(ratios[1]));
        }
        cc.log("屏幕分辨率=", width, " : ", height);
    ...
    */
    ...
    _loadWxheadH5: function (sprite, url, sex) {
        ...
        //头像为空,直接返回
        if (cc.dd._.isUndefined(url) || url.length == 0 || url.indexOf("$#XLYX_") != -1) {

            let pic = '2';
            if (cc.dd._.isString(url) && url.indexOf("$#XLYX_") != -1) {
                pic = url.replace("$#XLYX_", "");
            } else {
                let HallCommonData = require('hall_common_data').HallCommonData;

                let _sex = cc.dd._.isNumber(sex) ? sex : HallCommonData.getInstance().sex;
        ...
        if (url.indexOf('.jpg') != -1) {
            ...
            //微信头像
            url = cc.dd.Utils.getWX64Url(url);
        }
        //重置头像
        if (cc.isValid(sprite) && sprite instanceof cc.Sprite) {
            if (sprite._headTag) {
                if (sprite._headTag != url) {
    ...
    enableLog: function () {
        cc.dd.dumper = require('dumper').dumper;
        cc.dd.obj2string = require('dumper').obj2string;
        cc.log = function () {
    ...
    disableLog: function () {
        if (cc.sys.isNative) {
            //屏蔽dumper obj2string
            cc.dd.dumper = function () { };
            cc.dd.obj2string = function () { return '' };
            //屏蔽log
            cc.log = function () { };
        }
    },
    ...
    encode64: function (input) {
        input = this.strUnicode2Ansi(input);
    ...
    captureScreen: function (fileName, node, callback) {
        if (!node) {
            cc.log("error node");
            return;
        }
        ...
        var renderTexture = cc.RenderTexture.create(cc.winSize.width, cc.winSize.height, cc.Texture2D.PIXEL_FORMAT_RGBA8888, gl.DEPTH24_STENCIL8_OES);
    ...
    captureScreenVertical: function (fileName, node, callback) {
        if (!node) {
            cc.log("error node");
            return;
        }
        // 创建 renderTexture
        //如果待截图的场景中含有 mask，请使用下面注释的语句来创建 renderTexture
        var renderTexture = cc.RenderTexture.create(244, 244, cc.Texture2D.PIXEL_FORMAT_RGBA8888, gl.DEPTH24_STENCIL8_OES);
        // var renderTexture = cc.RenderTexture.create(432,768);
        ...
        // renderTexture.x = node.x;
        // renderTexture.y = node.y;
     ...
     * 节点必须位于左下角  且在屏幕内
     */
    captureNode: function (fileName, node, callback) {
        if (!node) {
            cc.log("error node");
            return;
        }
    ...
    captureScreenToPhotoAlbum: function () {
        if (!cc.sys.isNative) {
            cc.dd.PromptBoxUtil.show('网页不能截图!');
            return;
        }
        var file_name = '巷乐棋牌-' + new Date().format("yyyy-MM-dd-HH-mm-ss") + '.png';
        cc.dd.SysTools.captureScreen(file_name, cc.find('Canvas'), function () {
            cc.dd.native_systool.captureScreenToPhotoAlbum(jsb.fileUtils.getWritablePath() + '/' + file_name);
    ...
    captureCustomNode:function(fileName, node, callback){
        ...
        let originX = node.x;
        let originY = node.y;
        node.anchorX_origin = node.anchorX;
        node.anchorY_origin = node.anchorY;
        node.anchorX = 0;
        node.anchorY = 0;
        node.x = 0;
        node.y = 0;
        ...
        renderTexture.saveToFile(fileName, cc.ImageFormat.JPG, true, function () {
            ...
            node.anchorX = node.anchorX_origin;
            node.anchorY = node.anchorY_origin;
            node.x = originX;
            node.y = originY;

            resetNodeOffset(node.children, node);
    ...
    keepNetOk: function (net_ok_callback) {
        ...
        if (!cc.dd.native_systool.isNetAvailable() && cc.sys.isNative && cc.sys.isMobile) {
            func();
        } else {
            net_ok_callback();
        }
    ...
    genQRCode: function (url, node) {
        ...
        var gW = tileW * qrcode.getModuleCount();
        var off = (node.parent.width - gW) / 2
        node.width = gW;
        node.height = gW;
        ...
        for (var row = 0; row < qrcode.getModuleCount(); row++) {
            for (var col = 0; col < qrcode.getModuleCount(); col++) {
                if (qrcode.isDark(row, col)) {
                    ...
                    ctx.fillRect(off + Math.round(col * tileW), off + Math.round(row * tileH), w, h);
                    // ctx.rect(-gW/2 + Math.round(col * tileW), -gW/2 + Math.round(row * tileH), w, h);
                    ctx.fill();
                }
    ...
    uploadLog(data, url) {
        if (cc.sys.OS_ANDROID == cc.sys.os && cc.dd.native_systool.isNetAvailable()) {
            if (this.isUploading) {
                cc.log("正在上传日志，请稍后重试");
                return;
            }
            ...
            // formData.append('file', data, cc.dd.user.id+'.log');
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState == 4) {
                    ...
                    if (xhr.status >= 200 && xhr.status < 300) {
                        ...
                        if (jsonData.code == 1) {
                            cc.log('http success:uploadLog');
                        }
                        cc.log('http upload ' + jsonData.msg);
                    }
                    return;
                }
            }.bind(this);
            xhr.ontimeout = function () {
                cc.log('http timeout:uploadLog');
                this.isUploading = false;
            }.bind(this);
            xhr.onerror = function () {
                cc.log('http error:uploadLog');
                this.isUploading = false;
            }.bind(this);
            ...
            let datas = []
            let gameID = cc.dd._.isNumber(cc.dd.AppCfg.GAME_ID) ? cc.dd.AppCfg.GAME_ID : 0;
            let filename = gameID + '_' + cc.dd.user.id + '.log';
            datas.push(`--${boundary}\r\n`)
            datas.push(`Content-Disposition: form-data; name="file"; filename="${filename}"\r\n`)
            datas.push(`Content-Type: application/octstream\r\n`)
            datas.push("\r\n")
            datas.push(data)
            datas.push("\r\n")
            datas.push(`--${boundary}--`)
    ...
    async array2array(datas){
        ...
        for (let v of datas) {
            if (typeof v == "string") {
                bytes.push(this.string2utf8(v))
            } else if (v instanceof ArrayBuffer) {
                bytes.push(this.arraybuffer2array(v));
        ...
        let ab = this.array2arraybuffer(_data);
        return ab;
    }
};
