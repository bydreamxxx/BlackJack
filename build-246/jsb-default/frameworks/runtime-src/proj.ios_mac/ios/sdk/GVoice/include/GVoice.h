...
/**
 * Callback when called CheckDeviceMuteState interface function
 **/
- (void)onMuteSwitchResult:(int) nState;
 ...
 * @param openID : player's openID from QQ or Wchat. or a unit role ID.
 * @return : if success return GCLOUD_VOICE_SUCC, failed return other errno @see GCloudVoiceErrno
 * @see : GCloudVoiceErrno
 */
 ...
 */
-(enum GCloudVoiceDeviceState) getAudioDeviceConnectionState;

/*
 * enable BluetoothSCO
 * @return :  void
 */
-(void) EnableBluetoothSCO:(bool) enable;

/**
 * check mute switch state; iPhone is valiable; iOS simulator will return non-mute
 *
 * @return: if success return GCLOUD_VOICE_SUCC, failed return other errno @see GCloudVoiceErrno
 * @see :
 - GCloudVoiceErrno
 - void OnMuteSwitchResult(int nState);
 **/
- (enum GCloudVoiceErrno) CheckDeviceMuteStat;
 ...
 * @see : GCloudVoiceErrno
 */
- (enum GCloudVoiceErrno) SetAudience:(int *_Nullable) members  count:(int)count roomName: (const char *) roomName;
