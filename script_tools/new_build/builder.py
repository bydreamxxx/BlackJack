...
#
#
#     path = buildCfg.H5_PATH + difDir+'/assets/resources/native'
#     path_temp_ori = buildCfg.H5_TEMP_PATH + tempPath +'/last_ori/assets/resources/native'
#     path_temp_compress = buildCfg.H5_TEMP_PATH+ tempPath  + '/last_compress/assets/resources/native'
#     logFile = buildCfg.H5_TEMP_PATH+ tempPath  + '/../log.txt'
#
...
# # 压缩图片
# def tinify_images():
#     pro_path = buildCfg.NATIVE_PATH + '/res/raw-assets/resources/'
#     #os.system('rm -rf {0}*'.format(buildCfg.TINIFY_PATH))
#     #os.system('cp -rf {0}* {1}'.format(pro_path, buildCfg.TINIFY_PATH))
#     shutil.rmtree(buildCfg.TINIFY_PATH)
#     shutil.copytree(pro_path, buildCfg.TINIFY_PATH)
#     for dir_path, dir_names, file_names in os.walk(buildCfg.TINIFY_PATH):
#         for file in file_names:
#             file_path = os.path.join(dir_path, file)
#             if buildCfg.TINIFY_SKIP_LIST.count(file_path) > 0:
...
#     pro_dir_path = buildCfg.NATIVE_PATH + '/res/raw-assets/resources/' + dir + '/'
#     tinify_dir_path = buildCfg.TINIFY_PATH + dir + '/'
...
#     pro_path = buildCfg.NATIVE_PATH + '/res/raw-assets/resources/'
#     #os.system('cp -r {0}* {1}'.format(buildCfg.TINIFY_PATH, pro_path))
#     if os.path.exists(pro_path):
#         shutil.rmtree(pro_path)
#     shutil.copytree(buildCfg.TINIFY_PATH, pro_path)
#     return
#
#
# # 构建空资源工程
...
def copy_main_code_to_project():
    # if os.path.exists(buildCfg.NATIVE_PATH + '/main.jsc'):
    #     os.remove(buildCfg.NATIVE_PATH + '/main.jsc')
    # if os.path.exists(buildCfg.NATIVE_PATH + '/main.js'):
    #     os.remove(buildCfg.NATIVE_PATH + '/main.js')
    ...
    mainjs = getFileContent(buildCfg.NATIVE_PATH + '/main.js')
    if re.search(r"""if \(window.jsb\) \{
        var isRuntime = \(typeof loadRuntime === \'function\'\);""", mainjs):
        mainjs = re.sub(r"""if \(window.jsb\) \{
        var isRuntime = \(typeof loadRuntime === \'function\'\);""", """if (window.jsb) {
        var manifestUrl = "assets/main/project.manifest"
        var storagePath = jsb.fileUtils.getWritablePath()+"blackjack";
    // 创建 AssetsManager
    ...
    writeFileContent(buildCfg.NATIVE_PATH + '/frameworks/runtime-src/proj.android-studio/settings.gradle',settings)

    if os.path.exists(buildCfg.NATIVE_PATH + '/frameworks/runtime-src/proj.android-studio/instantapp/'):
        shutil.rmtree(buildCfg.NATIVE_PATH + '/frameworks/runtime-src/proj.android-studio/instantapp/')
    if os.path.exists(buildCfg.NATIVE_PATH + '/frameworks/runtime-src/proj.android-studio/game/'):
        shutil.rmtree(buildCfg.NATIVE_PATH + '/frameworks/runtime-src/proj.android-studio/game/')
    return
...
#     content = getFileContent(buildCfg.NATIVE_PATH + '/frameworks/runtime-src/proj.android-studio/local.properties',)
#     lines = content.splitlines()
#     new_content = ''
#     for i in range(len(lines)):
#         if "ndk.dir=" in lines[i]:
#             lines[i] = 'ndk.dir=' + buildCfg.NDK_PATH
#         new_content += lines[i]
#         new_content += '\n'
#     writeFileContent(buildCfg.NATIVE_PATH + '/frameworks/runtime-src/proj.android-studio/local.properties', new_content)
#     return
#
#
# 生成apk
def generate_apk(output_path):
    ...
    os.chdir(buildCfg.NATIVE_PATH + '/frameworks/runtime-src/proj.android-studio/')
    ...
    # #    buildCfg.NATIVE_PATH + '/frameworks/runtime-src/proj.android-studio/app/build/outputs/apk/xlqp-release.apk'), './xlqp-release.apk'))
    # shutil.copyfile(buildCfg.NATIVE_PATH + '/frameworks/runtime-src/proj.android-studio/app/build/outputs/apk/release/blackjack-release.apk', './blackjack-release.apk')
    return
...
def gen_scene_dir_cfg():
    path = buildCfg.COCOS_PROJECT_PATH+'/assets/resources/script/scene_dir_cfg.js'
    template = """
var scene_dir_cfg = {
%s
}
...
"""
    ...
    content_attr = ''
    for dir_path, dir_names, file_names in os.walk(buildCfg.COCOS_PROJECT_PATH):
        for file in file_names:
