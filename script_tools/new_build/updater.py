...
def gen_local_version(platform, game):
    ...
    #     builder.build_hall_pro(platform.local_dir + game.game_dir)
    # else:
    #     builder.build_empty_pro(platform.local_dir + game.game_dir, game.res_dir)

    if os.path.exists(platform.local_dir + game.game_dir):
        shutil.rmtree(platform.local_dir + game.game_dir, True)
    if os.path.exists(buildCfg.NATIVE_PATH + '/assets' + game.res_dir) and game.res_dir != "":
        shutil.copytree(buildCfg.NATIVE_PATH + '/assets' + game.res_dir, platform.local_dir + game.game_dir)
    if not os.path.exists(platform.local_dir + game.game_dir):
        os.mkdir(platform.local_dir + game.game_dir)
    ...
    # 生成项目游戏版本目录
    if not os.path.exists(buildCfg.NATIVE_VERSIONS_PATH):
        os.mkdir(buildCfg.NATIVE_VERSIONS_PATH)
    if not os.path.exists(buildCfg.NATIVE_VERSIONS_PATH + game.game_dir):
        os.mkdir(buildCfg.NATIVE_VERSIONS_PATH + game.game_dir)

    # 拷贝本地版本到项目
    shutil.copyfile(platform.local_dir + game.game_dir + "/project.manifest",
                    buildCfg.NATIVE_VERSIONS_PATH + game.game_dir + "/project.manifest")
    shutil.copyfile(platform.local_dir + game.game_dir + "/version.manifest",
                    buildCfg.NATIVE_VERSIONS_PATH + game.game_dir + "/version.manifest")
    return
...
#         builder.build_hall_pro(platform.local_dir + game.game_dir)
#     else:
#         builder.build_empty_pro(platform.local_dir + game.game_dir)
#
#     # 生成本地版本
#     version = platform.empty_version
#     if game.res_dir == '':
...
def gen_remote_version(platform, game):
    ...
    # if game.game_dir == '/hall':
    #     builder.build_hall_pro(platform.remote_dir + game.game_dir)
    # else:
    #     builder.build_empty_pro(platform.remote_dir + game.game_dir)

    if os.path.exists(platform.remote_dir + game.game_dir):
        shutil.rmtree(platform.remote_dir + game.game_dir, True)
    if os.path.exists(buildCfg.NATIVE_PATH + '/assets' + game.res_dir) and game.res_dir != "":
        shutil.copytree(buildCfg.NATIVE_PATH + '/assets' + game.res_dir, platform.remote_dir + game.game_dir)
    if not os.path.exists(platform.remote_dir + game.game_dir):
        os.mkdir(platform.remote_dir + game.game_dir)
    ...
    builder.make_zip(platform.remote_dir + game.game_dir, platform.remote_dir + '/zip/' + game.zip_name)
